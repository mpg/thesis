...
\begin{proof}
  ...
  \( \vai(\Cv) \) tels que \( \distv x \vai = \distv{ x }{ x' } \) et
  \( \distv y \vai = \distv{ y }{ y' } \). L'hypothèse~\eqref{e:Mapx} donne
  alors
  \begin{equation}
    ...
    \pmm.
  \end{equation}
  Comme \( \vai \) est un sous-groupe, \( x'_v - y'_v \in \vai \), donc la
  proposition~\ref{p:addsub-dv} donne
  \begin{equation}
    \distv z \vai
    \le
    H^{-\wtapx \expapx}
  ...
  archimédiennes et toutes celles où \( \hmclab* > 1 \).  La normalisation \(
    \sum\placerange \wtapx \degv \) permet alors de conclure.
\end{proof}
...
\begin{proof}[Démonstration (du théorème~\ref{t:mumford}).]
  ...
  contredit alors~\eqref{e:Mbig}. En effet, l'inégalité de \bsc{Liouville}
  ...
  \begin{equation}
    ...
    - ld \ln(3d)
  \end{equation}
  puis, en substituant les inégalités précédentes :
  \begin{equation}
