...
En revanche, dans le cas général, il ne semble \lat{a priori} pas exclu dans
...
Dans un registre différent, le lemme suivant montre qu'il ne semble pas
possible dans le théorème~\vref{t:mumford-gen} (ni dans la conjecture
ci-dessus) de prendre \( \grp = \va(\Qbar) \) plutôt qu'on sous-groupe de rang
fini.

\begin{lem}
  Supposons que le théorème~\vref{t:mumford-gen} soit vrai avec \( \grp =
    \va(\Qbar) \) et \( \avar \) une courbe de genre au moins \( 2 \). Alors,
  pour tout \( x \in \va(\Qbar) \)
  \begin{equation}
    \prod\placerange \distv x \oa ^\degv
    \le
    \hautm2 x ^{-\eps}
    \implies
    \hautn x \le \todots
  \end{equation}
  où \( \oa \) est l'origine de \( \va \).
\end{lem}

Avant de passer à la preuve, introduisons quelques notations : on note
\begin{align}
  \apx(\avar, \eps)
  & =
  \set{
  ...
  \set{
    ...
    \hautn{ z - x } \le \gamma^2 \hautn x
  }
  \pmm.
\end{align}
On utilisera de plus le lemme de géométrie euclidienne suivant.

\begin{lem}
  Deux points quelconques \( y \) et \( z \) de \( B(x, \gamma) \), satisfont
  toujours
  \begin{align}
    \cos(y, z) & \le 1 - \phi
    \\
    \hautn z \le (1 + \rho) \haut y
  \end{align}
  dès que \( \gamma \le \todots \).
\end{lem}
