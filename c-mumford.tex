...
\begin{thm} \label{t:mumford-gen}
  Soit \( \avar \) une sous-variété de \( \va \) ; on note \( \adeg = \deg
    ...
    \subset \va(\Qbar) \) de rang fini \( r \), puis on
  pose
  \begin{equation}
    m
    =
    \bigl(
    2^{34} \cdot h_0(\va) \cdot \deg X
    \bigr)^{ (r+1) g^{ 5(\dim X + 1)^2 } }
    + 1
    \pmm.
  \end{equation}
  ...
  \begin{align}
    ...
    \Bigl(
      \hautl[\htpph] X
      + 4 D m^2 \bigl( \hlclab + \htcmp + \ln(n+1) \bigr)
    \Bigr)
    \quad
    \label{e:Mbig-gen}
    \\
  ...
  translaté est contenue dans \( \avar \) et deux indices distincts \( i \)
...
\begin{prop} \label{p:liouville}
  Soit \( V \) une sous-variété de \( \projd \), de dimension \( \adim \) et de
  degré \( \adeg \). Pour tout point \( x \in \projd(\Qbar) \), on a soit \( x \in
  V(\Qbar) \) soit
  \begin{equation}
...
général en utilisant la définition, on obtient \( \adeg (\adim+1) \) comme exposant de
...
\begin{proof}[\proofname{} de la proposition~\vref{p:liouville}]
  ...
  donne une forme \( G \) homogène de degré \( \adeg \), contenant \( V \) mais
  pas \( x \), telle que
  \begin{equation}
    \hautm[1] G
    \le
  ...
  lemme précédent à \( \zeros G \) et de remarquer que \( 17/8 \cdot (\adeg+1)^2
    \le (3\adeg)^2 \) pour conclure.
\end{proof}
...
\begin{proof}
  ...
  \cite{remgdmp}, appliqué avec \( q = 2 \) et \( \adeg = (1,1) \), ainsi que le
...
Soit \( \avar \) une sous-variété de \( \va \) ; on note \( \adeg = \adeg \) et \( \adim
  = \adim \). Remarquons de suite que le degré de \( \avar \) est nécessairement
au moins \( 2 \), car si \( \avar \) était linéaire, elle contiendrait une droite
...
\begin{lem}
  ...
  \end{equation}
  où \( \delta \colon \avar \to \avar^m \) est le plongement diagonal. En particulier,
  si \( S_\avar = \bigcap_{i=1}^m \avar - x_i \), ces inclusions sont des égalités.
\end{lem}

\begin{proof}
  Par définition de \( s_m \), il est clair que \( x + \delta(A) \subset
    s_m^{-1} ( s_m(x) ) \). Par ailleurs, la définition de \( S_\avar \) donne
  clairement \( x + \delta(S_\avar) \subset \avar \), ce qui établit l'inclusion
  annoncé.
...
\begin{proof}
  ...
  Plus précisément, on note \( Y_j = \bigcap_{i=1}^{j+1} \avar - x_i \) le fermé de
  ...
  Il est clair que \( SH(Y_1, \adeg) = SH(\avar, \adeg) = \adeg^{\adim+1} \). Par ailleurs, comme
  chaque \( Y_j \) peut être défini (ensemblistement) par des équations de
  degré au plus \( \adeg \), on peut appliquer à ces équations la proposition~3.3
  de la référence citée, ce qui garantit que \( SH(Y_{j+1}, \adeg) \le SH(Y_j, \adeg)
  \) et donne immédiatement par récurrence \( SH(Y_j, \adeg) \le \adeg^{\adim+1} \).

  Par définition de \( SH \), on voit donc que pour tout \( k \), le nombre de
  composantes de \( Y_j \) de dimension \( \adim - k \) est au plus \( \adeg^{k+1} \).
  On peut être un peu plus précis pour \( Y_1 \) et voir que toutes ses
  composantes sont de dimension au plus \( \adim - 1 \) car \( \avar - x_1 \) n'a
  qu'une composante. Ainsi, en choisissant successivement les \( x_i \) de
  façon à ce que \( \avar - x_{j+2} \) coupe strictement une des composantes de \(
    Y_j \) de dimension maximale parmi celles qui ne sont pas composantes de \(
    S_\avar \), on voit qu'il existe un \( j_1 \le 1 + \adeg^2 \) tel que \( Y_j \) n'ai
  plus de composante de dimension \( \adim - 1 \) à part celles de \( S_\avar \), et
  en continuant jusqu'à avoir éliminé les composantes indésirables de
  dimension \( 0 \), qu'il existe un \( j_\adim \le 1 + \adeg^2 + \adeg^3 + \dots +
    \adeg^{\adim+1} \) tel que \( Y_{j_\adim} \) n'ait plus aucune composante autre que
  celles de \( S_\avar \), c'est-à-dire tel que \( Y_{j_\adim} = S_\avar \).
  ...
  Par ailleurs, on constate qu'une fois qu'on a choisi \( x_1, \dots,
    x_{j_\adim+1} \) tels que \( Y_{j_\adim} = S_\avar \), on peut rajouter des \( x_i \)
  arbitraires pour \( i > j_\adim + 1 \) et on aura toujours \( S_\avar = \bigcap_i
    \avar - x_i \). Ainsi, pour tout \( m \ge M \) il existe une famille \(
    (x_i)_i \in \avar^m \) possédant la propriété voulue.
\end{proof}
...
on peut calculer la dimension de \( Z = s_m(\avar^m) \) puis estimer son degré et
sa hauteur.
...
\begin{proof}
  ...
  \( S_\avar = \bigcap_{i=1}^m \avar - x_i \), ce qui est possible d'après le choix de
  \( m \). La fibre de \( {s_m}_{|\avar} \) en \( s_m(x) \) est alors isomorphe à
  \( S_\avar \) et en particulier, de même dimension.
  ...
  points où elle est égale à \( \dim S_\avar \) est donc un ouvert. D'après le
...
\begin{proof}
  On note \( \adim' = \dim Z = mu - \dim S_\avar \) et on choisit des hyperplans de \(
    \proj N \) qu'on notera \( H_1, \dots, H_{\adim'} \) et qu'on supposera en
  ...
  choisit un point \( x_j \in s_m^{-1}(y_j) \cap \avar^m \). Pour tout \( j \in
  ...
  la fibre \( s_m^{-1}(y_j) \cap \avar^m \) est un translaté de \( \delta(S_\avar) \)
  et est une composante isolée de l'intersection \( \avar^m \cap \bigcap_i \tilde
  ...
  d'indice \( (\adim, \dots, \adim) \) et qu'il vaut \( \adeg^m \),  de sorte que l'on a
  \begin{equation}
    SH( \avar^m ; 2, \dots, 2, 2(m-1) )
    =
    \adeg^m
  ...
  les multidegrés de \( \delta(S_\avar) \) sont égaux à \( \deg S_\avar \), ce qui
  donne
  \begin{align}
...
\begin{proof}
  ...
  Notons \( F \) l'ensemble des points de \( x \in \avar \) tels que \( \hautl[2]
    x \le 2 \miness(\avar) \) ; par définition \( F \) est dense dans \( \avar \),
  donc \( F^m \) est dense dans \( \avar^m \) et \( s_m(F^m) \) est dense dans \(
...
une famille \( x = (x_1, \dots, x_m) \in \va(\Qbar)^m \) (et non plus \( \avar^m
...
On rappelle la version effective de l'ex-conjecture de \bsc{Mordell-Lang}
obtenue par \bsc{Rémond}.

\begin{fact}
  \worknote{On doit pouvoir mettre une inégalité stricte, non ?}
  Soient \( \avar \subset \va \) une sous-variété et \( \Gamma \subset \va(\Qbar)
  \) un sous-groupe de rang fini \( r \). Alors il existe un entier naturel
  \begin{equation}
    S
    \le
    \bigl(
  ...
  sous-variétés abéliennes \( B_1, \dots, B_s \) de \( \va \) telles que \(
    x_i + B_i \subset \avar \) pour tout \( i \in \set{1, \dots, S} \) et
  \begin{equation}
    \avar(\Qbar) \cap \Gamma
    =
    \bigcup_{i=1}^{S} (x_i + B_i)(\Qbar) \cap \Gamma
    \pmm.
  \end{equation}
\end{fact}

\begin{proof}
  \worknote{Introduire \( h_0(\va) \) et vérifier les hypothèses sur le
    plongement.}
...
\begin{proof}[\proofname{} du théorème~\vref{t:mumford-gen}.]
  ...
  \begin{equation}
    ...
    =
    \bigcup_{k=1}^{S} (z_k + B_k)(\Qbar) \cap \Gamma
  \end{equation}
  où l'égalité de droite découle du fait précédent, avec \( z_k \in \avar - y_m \)
  et \( z_k + B_k \in \avar \), où \( B_k \subset \va \) est une sous-variété
  abélienne, pour tout \( k \), et \( S < m \).

  Ainsi, il existe un indice \( k \in \set{1, \dots, S} \) et deux indices
  distincts \( i \) et \( j \) tels que \( y_i - y_m \in z_k + B_k \) et \(
    y_j - y_m \in z_k + B_k \). Par différence, on a alors
  \begin{equation}
    y_i - y_j = x_i - x_j \in B_k
  \end{equation}
  et \( z_k + y_m + B_k \subset \avar \), ce qui achève la preuve.
\end{proof}
