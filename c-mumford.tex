...
\begin{lem} \label{l:dv-p-alg-gen}
  Pour toute variété \( V \subset \Proj^n \) et tous points \( x \in
    \Proj^n(\C_v) \) et \( y \in V(\C_v) \), on a \( \distalgv x V  \le
    \cst{CPonctuelAlg}\, \distv x y ^m \) avec
  \begin{equation}
    \newcst{CPonctuelAlg}
    =
    \left( 2 (n+1)^{3m/2} \left(
        \frac{2n^2+1}{n^2}
        \exp\bigl(\gamma_{\frac{(n+1)n}2} + \gamma_{n+1}\bigr) \right)^{ld}
    \right)^\dv
    \pmm,
  \end{equation}
  où \( l = \dim V + 1 \), \( d = \deg V \) et \( m = m_{y}(V) \) est la
  multiplicité de \( V \) en \( y \).
\end{lem}
...
\begin{proof}
  ...
  \begin{align*}
    \frac {\nv2{R_{k, \alpha}}} {\nv2 x ^{ld}}
    & \le
    \Onnv[v, 2]{ \frac1{\alpha!} \frac{\partial^kg}{\partial X^\alpha} }
    \nv2 y^{ld-k}
    \nv2{x \wedge y}^k
    / \nv2 x^{ld}
    \\ & \le
    ...
    / \nv2 x^{ld}
    \\ & \le
    \left( \frac{2n^2+1}{n^2} \right)^{ld\dv}
    \Onnv[v, 2] g
    \distv x y ^k
    (\sqrt{n+1})^{k\dv}
    \pmm.
  ...
  \begin{equation}
    ...
    \pmm,
  \end{equation}
  puis, si \( v \) est finie, \( \nv2 {g(x)} / \nv2 x ^{ld} \le \Onnv[v, 2]g
    \distv x y ^m \) par l'inégalité ultramétrique ; et si \( v \) est
  infinie :
  \begin{align*}
    \frac{\nv2{g(x)}}{\nv2 x^{ld}}
    & \le
    \left(\frac{2n^2+1}{n^2}\right)^{ld} \Onnv[v, 2]g
    \sum_{k=m}^{ld}
  ...
  \begin{equation}
    ...
    \nv2{\md_x f}
    = \nv2{g(x)}
    \stackrel{(b)}{\ll} \Onnv[v, 2] g
    = \nv2{\md f}
    \stackrel{(c)}{\ll} \mv{\md f}
    \stackrel{(d)}{\ll} \mv f
    \pmm,
  \end{equation}
...
géométrie de \( V \) plongée. En effet, \bsc{Rémond} a donné
...
de \( V \) recherchées. Dans notre cas, il faudra comme chez \bsc{Rémond}
...
partir de la définition que \( \mu \) envoie la forme de \bsc{Chow} d'une
...
\begin{proof}
  \worknote{Dupliqué par le lemme \ref{l:cramer} qui donne aussi une
    estimation de hauteur, qui serait d'ailleurs peut-être utile ici.}
...
\begin{proof}
  ...
  \bsc{Veronese} de degré \( b \). Pour chaque \( w \in \mathcal C(D) \), on
...
\begin{proof}[Démonstration du lemme~\ref{lProjection}]
  ...
  \( m \).  D'après \bsc{Rémond}, il existe un polynôme \( P_z \) homogène de
...
\begin{proof}
  ...
  également le fait qu'une forme de \bsc{Chow} de \( \pi_* V \) s'obtient en
  spécialisant une forme de \bsc{Chow} de \( V \) en des formes linéaires à
