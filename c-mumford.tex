...
\begin{thm} \label{t:mumford-gen}
  ...
  \bsc{Mumford} modifié associé à une polarisation principale.
...
la constante de \bsc{Lipschitz} en fonction des constantes introduites à la
section~\vref{sec:vaemb}.  L'énoncé suivant concerne la soustraction, mais il
est clair qu'un résultat identique pour l'addition peut être obtenu de la même
façon.
...
\begin{proof}
  ...
  lien entre indices (d'une forme de \bsc{Chow} ou, par extension, d'une
  distance) et plongements de \bsc{Segre} (et de \bsc{Veronese} remodelé), on
  a
  ...
  On va utiliser un développement de \bsc{Taylor} au voisinage de \( z \) de
  ...
  Le développement de \bsc{Taylor} de \( F \) s'écrit alors :
  \begin{equation}
    F(z')
    =
     F(z)
  ...
  \(
    ...
    \distv{ z }{ z' }
  \)
  pour estimer \( R_i \) en utilisant~\eqref{e:pm-loc2} et l'hypothèse faite
  sur \( \nv2{ z' } \) :
  \begin{align}
    \nv2{R_i}
    & \le
...
\begin{proof}
  ...
  \begin{equation}
    ...
    \le
    \exp(-\wtapx \eps \hautn{ x } + \htcmp)
    \cdot \hmpm* \bigl( 5 \sqrt2 (n + 1)^3 \bigr)^\dv
    \pmm.
  \end{equation}
...
\begin{proof}
  ...
  \begin{align}
    ...
    (\dim Z + 1) \deg Z
    \\ & \le
    \Bigl( 8 \miness(\avar) + \hlpm + \frac12 \ln(n+1) \Bigr)
    (m-1) (\adim m + 1) \deg Z
    \\ & \le
    \Bigl( \frac8\adeg \hautl\htpph \avar + \frac12 \ln(n+1) + \hlpm \Bigr)
    \adim m^2 \deg Z
    \\ & \le
    \Bigl(
...
\begin{proof}
  ...
  entre indices d'une distance et plongement de \bsc{Segre}), en notant \( y_v
...
\begin{proof}
  ...
  théorème~\oref{t:mumford-grp}. Si \( z \not\in Z \), l'inégalité de
  ...
  \bsc{Liouville} écrite en~\eqref{e:liou}, puis simplifier l'expression
