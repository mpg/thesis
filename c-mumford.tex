...
\) dans l'énoncé précédent (ou dans le théorème~\vref{t:mumford-gen}). En
effet, notons
\begin{equation}
  E(\avar, \eps)
  =
...
\end{equation}
et soit \( C \subset \va \) une courbe non elliptique ; pour tout \( \eps \)
il existerait un \( \gamma \) tel pour tout \( x \) de hauteur assez grande,
\begin{equation}
  \card E(\avar, \eps) \cap B(x, \gamma \hautn x) < \infty
\end{equation}
où \( B(x, R) \) désigne la boule centrée en \( x \) et de rayon \( R^2 \)
dans l'espace de \MoW. On prend alors \( x \in E(\set0, \eps') \) de hauteur
assez grande\footnote{Peut-on ?} et en particulier \( \hautn x > 2 \gamma^{-1}
  \minessn(C) \). Il existe par définition une infinité de points \( y \in
  C(\Qbar) \) tels que \( \hautn y \le 2 \minessn(C) \). Pour chacun de ces
points, \( x + y \in B(x, \gamma \hautn x) \) et, si \( \eps' \) a été choisi
assez petit, \( x + y \in E(C, \eps) \), ce qui donne une contradiction.
