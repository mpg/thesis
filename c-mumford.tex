...
\end{equation}
où la deuxième flèche est un plongement de \bsc{Segre}, donc \( N = \dots
\)\todo.

Soit \( X \) une sous-variété de \( \va \) ;
notons \( S_X \) le stabilisateur de \( X \).

\begin{lem}
  Il existe un entier \( M \le (\deg X)^{\dim X + 2} - 1 \) et
  une famille \( (x_1, \dots, x_M) \) de points de \( X(\Qbar) \)
  telle qu'on ait, ensemblistement :
  \begin{equation}
...
\begin{ideas}
  ...
  \end{equation}
  par définition, et par noethérieneté il est clair qu'on peut en fait prendre
  une intersection finie ; reste à quantifier ce « finie ».
  ...
  Plus précisément, si l'on note \( (Y\pexp i[k])_k \) l'ensemble des
  composantes de l'intersection partielle à l'étape \( i \), on a toujours
  \begin{equation}
    \sum_k \deg Y\pexp i[k] \cdot (\deg X)^{\dim Y\pexp i[k]}
    \le
    (\deg X)^{\dim X + 1}
    \pmm.
  \end{equation}
  En effet, c'est clairement le cas à l'étape 1, et si c'est vrai à l'étape \(
    i \), ça l'est à l'étape \( i + 1 \) par Bézout (réduit).

  On en déduit qu'à chaque étape, le nombre de composantes (qui ne sont pas
  des composante de \( S_X \)) de dimension \( \dim X - j \) est au plus
  \( (\deg X)^{j+1} \). Ainsi, il faut au plus \( (\deg X)^2 \) pour éliminer
  les composantes de dimension \( \dim X - 1 \) après la première étape (on
  suppose \( X \) irréductible au début), etc. jusqu'à \( (\deg X)^{\dim X +
      1} \) pour éliminer les composantes de dimension \( 0 \) à la fin.

  Au final le nombre d'étapes est
  \begin{equation}
    1 + (\deg X)^2 + (\deg X)^3 + \dots + (\deg X)^{\dim X + 1}
    \le
    (\deg X)^{\dim X + 2} - 1
    \pmm.
  \end{equation}
...
\begin{coro}
  Pour tout entier \( m \) supérieur à l'entier \( m \) du lemme précédent, il
  existe un ouvert dense de \( X^m \) tel
  que pour tout point \( (x_1, \dots, x_m) \) de cet ouvert, on ait
  \begin{equation} \label{e:IS}
    S_X = \bigcap_{i=1}^{m} X - x_i \tag{IS}
    \pmm.
  \end{equation}
\end{coro}

\begin{proof}
  Il est clair que si \( (x_1, \dots, x_M) \) satisfait \eqref{IS} c'est
  aussi le cas de toute extension de cette famille ; il existe donc au moins
  un point de \( X^m \) possédant la propriété voulue. Par ailleurs, il est
  clair que cette propriété est ouverte.
\end{proof}

\begin{lem}
  On choisir \( m \ge M + 1 \).
  Pour \( x \in X^m \) satisfaisant \eqref{IS}, on a
  \begin{equation}
    s_m^{-1} ( s_m(x) ) \cap X^m
    =
    x + \delta(S_X)
  \end{equation}
  où \( \delta \colon X \to X^m \) est le plongement diagonal.
\end{lem}

\begin{ideas}
  Il est clair que la fibre contient \( x + \delta(S_X) \). 

  Réciproquement, cf (2) dans le mail.
\end{ideas}
