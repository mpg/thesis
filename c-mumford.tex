...
\begin{thm} \label{t:mumford-gen}
  ...
  \begin{equation}
    ...
    \pmm.
  \end{equation}
  On choisit alors \( \expapx > 0 \) puis \( \phi > 0 \) et \( \rho > 0 \)
  tels que
  ...
  translaté est contenue dans \( \varapx \) et deux indices distincts \( i \)
...
\begin{proof}
  ...
  \begin{equation}
    ...
    \le
    D^{m-1} (2m)^{ (m+2)u } \, \hautl[\htpph] X
    + D^m (2m)^{ (m+2)u + 2} \bigl( \hlclab + \htcmp + \ln(n+1) \bigr)
  \end{equation}
...
\begin{fact}
  ...
  Soient \( X \subset \va \) une sous-variété et \( \Gamma \subset \va(\Qbar)
  ...
  \begin{equation}
    ...
    \pmm,
  \end{equation}
  des points \( x_1, \dots, x_S \) de \( X(\Qbar) \cap \Gamma \) et des
  sous-variétés abéliennes \( B_1, \dots, B_s \) de \( \va \) telles que \(
    x_i + B_i \subset X \) pour tout \( i \in \set{1, \dots, S} \) et
  \begin{equation}
    X(\Qbar) \cap \Gamma
    =
...
\begin{proof}
  \worknote{Introduire \( h_0(\va) \) et vérifier les hypothèse sur le
    plongement.}
...
\begin{proof}[Démonstration du théorème~\vref{t:mumford-gen}.]
  Soit \( x = (x_1, \dots, x_m) \) une famille de points comme dans l'énoncé.
  D'après le lemme précédent, il existe un point \( y \in \varapx(\Qbar) \)
  tel que \( x_i - x_m = y_i - y_m \) pour tout \( i \). En particulier, pour
  tout \( i \) on a
  \begin{equation}
    y_i - y_m
    \in
    (X - y_m)(\Qbar) \cap \Gamma
    =
    \bigcup_{k=1}^{S} (z_k + B_k)(\Qbar) \cap \Gamma
  \end{equation}
  où l'égalité de droite découle du fait précédent, avec \( z_k \in X - y_m \)
  et \( z_k + B_k \in X \), où \( B_k \subset \va \) est une sous-variété
  ...
  \end{equation}
  et \( z_k + y_m + B_k \subset X \), ce qui achève la preuve.
\end{proof}
