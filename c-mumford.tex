...
\begin{proof}
  ...
  compter les composantes à chaque étape, on considère la quantité (introduite
  dans \cite{XXX}\todo) :
  \begin{equation}
    SH(Y, d)
    =
    \sum_Z \deg Z d^{\dim Z}
  \end{equation}
  où la somme est prise sur l'ensemble des composantes irréductibles de \( Y
  \). Il est clair que \( SH(Y_1, D) = SH(X, D) = D^{u+1} \). Par ailleurs,
  comme chaque \( Y_j \) peut être défini (ensemblistement) par des équations
  de degré au plus \( D \), la proposition~3.3 de la référence citée
  s'applique et garantit que \( SH(Y_{j+1}, D) \le SH(Y_j, D) \) ce qui donne
  immédiatement par récurrence \( SH(Y_j, D) \le D^{u+1} \).
  ...
  Par ailleurs, on constate qu'une fois qu'on a choisit \( x_1, \dots, x_{j_u+1}
  \) tels que \( Y_{j_u} = S_X \), on peut rajouter des \( x_i \) arbitraires pour
  \( i > j_u + 1 \) et qu'on aura toujours \( S_X = \bigcap_i X - x_i \). Ainsi,
  pour tout \( m \ge M \) il existe une famille \( (x_i)_i \in X^m \)
