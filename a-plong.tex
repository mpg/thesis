...
\nomuse \coa {Coordonnées de l'origine de \( \va \) dans le plongement de
  \bsc{Mumford} modifié}
...
\begin{fact} \label{f:comp-h-hn}
  Pour tout point \( \pp \in \va(\Qbar) \), on a \(
    \abs{\hautn\pp - \hautl2\pp} \le \htcmp \) avec
  \begin{equation}
    \htcmp = (4^\genre - 1) \hautl2{\coa} + 3\genre \ln2
    \pmm.
  \end{equation}
\end{fact}
...
\begin{fact} \label{f:clab}
  ...
  \begin{enumthm}
    ...
    \item \(
        ...
        \le
        (a^2 + b^2) (2 (4^\genre-1) \hautl2{\coa} + 6\dimp)
      \).
  \end{enumthm}
\end{fact}
...
\begin{proof}
  ...
  au moment de la rédaction de \cite{remivds}. On constate par ailleurs que \(
    \hautl2{(S_{l, l'})_{l, l'}} \le 2(4^\genre-1) \hautl2{\coa} \)
