...
\begin{abstract}
  Le résultat principal de ce document est rendu obsolète par l'inégalité de
  Mumford obtenue pour les groupes (\nolinkurl{mumford-groupes.tex}).

  Cependant, le lemme~\ref{clef} n'est pas impliqué de façon tout à fait aussi
  précise par la proposition~5 (\texttt{MetricOp}) de \lat{loc. cit.} : on a
  ici du $c_v(\va)$ plutot que son cube (les autres constantes étant assez
  comparables).

  À part ça, il y a des éléments potentiellement intéressants dans les
  démonstrations des lemmes~\ref{rayonRel} et~\ref{DistRel} (utilisation de la
  formule de \bsc{Taylor} avec déshomogénéisation), ainsi que dans le
  lemme~\ref{RayonAbs} (utilisation des formules de \cite{daphimhva2}).
\end{abstract}
...
on plonge $\va$ dans $\Proj^n$, où $n=16^g d - 1$, par un plongement de
\bsc{Mumford} modifié, tel que décrit au §3.1 de \cite{daphimhva2}, que nous
noterons $\Theta$. On identifiera volontiers $\va$ et son image par $\Theta$.
...
On fixe un corps de nombres $\cdn$ contenant un corps de définition de $\va$
...
l'unité. Si $\p{x}$ est un point de $\va(\cdn)$, on notera $x = (x_0, \dots,
...
c'est-à-dire aux points $\p{x} \in \va(\cdn)$ satisfaisant $\Distv(\p{x}, \oa) \le
...
\[
  \Distv(\p{x}, \oa) \le t_v H(\p{x})^{-\lambda_v \eps} \quad
...
dimension $g$ de $\va$, de son rang $r$ sur $\cdn$, et de paramètres
diophantiens liés à $\va$ plongée, que sont la hauteur projective de $\oa$, (le
degré $d_\oa$ de son corps de définition ?), et pour chaque $v$ de $S$ les
constantes locales $c_v(\va) = \nv{\oa}\nv{\vai}$, où $\vai$, qui sera défini
...
$\va(\cdn)$, qui est par définition l'espace $\va(\cdn)\otimes_\Z \R$ muni de la
forme quadratique définie positive donnée par la hauteur normalisée de
\bsc{Néron-Tate}, notée $\Hautn$. C'est un espace vectoriel euclidien dont la
dimension est le rang de \bsc{Mordell-Weil} $r$ de $\va$ sur $\cdn$. Si $\p{x}$
est un point de $\va(\cdn)$, nous noterons encore $\p{x}$ son image dans
...
\begin{thm} \label{mumpt}
  Soient $\p{x}$ et $\p{y}$ deux points de $\va(\cdn)$ satisfaisant (\ref{hyp}),
  avec
  \[
  r_v = \av{2}^g(2^g20n(n+1)^2)^{-\dv} c_v(\va)^{-1} \pmm.
  \]
  Posons $B =
  ...
  \item $\Hautn(\p{x}) > (1+\eps)B/(\eps - (\rho^2/4) - 2\phi - \rho\phi)$,
    \label{tronk}
  \end{enumerate}
  alors $\p{x} = \p{y}$.
\end{thm}
...
\begin{lem} \label{pv}
  ...
  \va(\cdn)$ satisfaisant $\Distv(\p{x}, \oa) < \eps_0$, on a alors
...
\begin{proof}
  ...
  $\av{x_0}< \nv{\hat{x}} = \nv{x}$, il viendrait $\nv{\coa_0 \hat{x}} >
  \nv{x_0 \hat{\coa}}$ et la propriété ultramétrique appliquée au dernier
  membre de (\ref{pva}) donnerait $\eps_0 \nv{x}\nv{\coa}  \ge
  \av{\coa_0}\nv{\hat{x}}$, puis $\eps_0 \ge 1$ contrairement aux hypothèses.

  Pour le cas archimédien, l'inégalité triangulaire dans (\ref{pva}) donne
  $\eps_0 \nv{x}\nv{\coa}  \ge \av{\coa_0}\nv{\hat{x}} -
  \av{x_0}\nv{\hat{\coa}} $. En regroupant les termes et en divisant par
  $\nv{x}\nv{\coa}$, puis en remarquant que $\nv{\hat{\coa}}/\nv{\coa} \le
  1$ il vient successivement :
  \begin{align}
  \frac{\nv{\hat{\coa}}}{\nv{\coa}} \cdot \frac{\av{x_0}}{\nv{x}}
...
$\va\times\va \to \va$ dans le plongement choisi en tout point où elles
...
\begin{lem} \label{rayonRel}
  ...
  sur tout $(B_v(\oa, r_v(D)))^2 \subset \va^2$.
\end{lem}
...
\begin{proof}
  ...
  $(B_v(\oa, r_v(D)))^2$ ; on peut ainsi écrire $\max(\Distv(\p x, \oa), \Distv(\p
...
\begin{lem} \label{DistRel}
  ...
  v} < 1$. Pour tous points $\p{x}$ et $\p{y}$ de $\va(\cdn)$ satisfaisant
  \[
  \max ( \Distv(\p{x}, \oa), \Distv(\p{y}, \oa) ) < r_v(D) \cdot \eps_{1,v} \pmm{,}
  \]
  on a : $\Distv(\p{x}-\p{y}, \oa) \le \eps_{1,v}$.
\end{lem}
...
plus intrinsèques. Le candidat naturel est la constante locale $c_v(\va)$, que
...
l)$ tel que $\coa_{(a,l)} \neq 0$. On note $\mathcal{Z}_\vai$ un système complet
de représentants vérifiant cette condition, et on pose $\vai = (\coa_{(a,
l)}^{-1})_{(a, l) \in \mathcal{Z}_\vai}$. La constante $c_v(\va) =
\nv{\oa}\nv{\vai} \ge 1$ est alors bien définie ; c'est l'équivalent local de
...
\begin{proof}
  ...
  par hypothèse, et exiger $(b', k') \in \mathcal{Z}_\vai$. La proposition
  citée affirme alors que la famille $\tilde{D}$ représente le morphisme de
  différence $(\tilde\Theta(\va))^2 \to \tilde\Theta(\va)$ et qu'en
...
\begin{lem} \label{clef}
  ...
  (2^g20n(n+1)^2)^{-\dv}c_v(\va)^{-1}$. Pour tous $\p x$ et $\p y$ de
  $\va(\cdn)$ satisfaisant
  \[
  \max ( \Distv(\p{x}, \oa), \Distv(\p{y}, \oa) ) < r_v \cdot \eps_{1,v} \pmm{,}
  \]
  on a : $\Distv(\p{x}-\p{y}, \oa) \le \eps_{1,v}$.
\end{lem}
...
\bsc{Mordell-Weill} montrent que sa hauteur normalisée $\Hautn(\p{z})$ est au
contraire négligeable devant $\Hautn(\p{x})$. Il n'y aura alors plus qu'à
comparer $h$ et $\Hautn$ pour conclure. Les lemmes suivants précisent ces deux
affirmations.
...
\begin{proof}
  ...
  :
  \[
  \prod_v \Distv(\p{z}, \oa)^{e_v} \ge \prod_v \frac{\av{z_i \coa_j - \coa_i
    z_j}^{e_v}}{\nv{z}^{e_v}\cdot\nv{\coa}^{e_v}} \ge \frac{1}{H(\p{z})
    H(\oa)} \pmm{.}
  \]
  On majore ensuite $\Distv(\p{z}, \oa)$ par $\min(H(\p{x}),
...
\begin{lem} \label{pti}
  ...
  normalisée majorée par $(\rho^2/4 + 3\phi) \Hautn(\p{x})$.
\end{lem}

\begin{proof}
  On note $\left<\truc ; \truc \right>$ le produit scalaire et $\nv[N]{\truc} =
  \sqrt{\Hautn(\truc)}$ la norme dans l'espace de \bsc{Mordell-Weil}. On remarque
...
\begin{proof}[Démonstration du théorème \ref{mumpt}.]
  ...
  \cite{daphimhva2} et la remarque subséquente : $\av[]{\Hautn(\truc) -
    h(\truc)} \le (4^g-1)h(\oa) + 3g\log(2)$. On passe ainsi aux hauteurs
  normalisées dans le résultat du lemme \ref{papti} :
  \begin{align*}
  \Hautn(\p{z}) & \ge h(\p{z}) - (4^g-1)h(\oa) - 3g\log(2)\\
  & \ge \eps \min(h(\p{x}), h(\p{y})) - h(\oa) - (4^g-1)h(\oa) - 3g\log(2)
  \\
  & \ge \eps \left(\Hautn(\p{x}) - (4^g-1)h(\oa) - 3g\log(2)\right) - B \\
  & \ge \eps \Hautn(\p{x}) - (1+\eps)B \pmm{.}
  \end{align*}
  Mais le lemme \ref{pti} donne alors $((\rho^2/4) + 3\phi) \Hautn(\p{x}) \ge
  \eps \Hautn(\p{x}) - (1+\eps)B$, ce qui contredit très précisément l'hypothèse
  \ref{tronk}.
\end{proof}
