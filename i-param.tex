...
On aimerait alors pouvoir le représenter par un morphisme \( \pmor* \) faisant
...
\end{equation}
où les flèches verticales sont les projections canoniques, mais il n'est pas
clair que ceci soit possible. Nous allons en fait construire, pour chaque
\( f = F/G \in \cdn[X]_{(\ideal\anyvar)} \), une série \( \xi_{F, G} = \sum
  \xi_{F, G, \derp} \psp^\derp \) telle que \( \pi(\xi_{F, G}) = \pmor(\pi(f)) \), et dont
nous contrôlerons, pour chaque coefficient, un dénominateur ainsi que le
degré et la hauteur du numérateur correspondant. Cependant, ces
représentations ne seront pas nécessairement indépendantes de la
représentation \( f = F/G \) choisie, et on n'aura pas \( \xi_{FF',
    GG'} = \xi_{F, G} \cdot \xi_{F', G'} \) sauf dans certains cas
particuliers. De plus, on construira en fait deux séries : une dont on
contrôlera les normes aux places archimédiennes, l'autre aux places
ultramétriques.
