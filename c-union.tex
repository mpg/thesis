\chapter{Résultats de décomptes}
\label{chap:union}
...
\).  Si \( F \) est un sous-ensemble de \( \va(\Qbar) \) et \( \xi > 0 \) un
réel, on introduit une condition sur \( F \), associée à \( \xi \), de la
façon suivante.

\begin{tdef} \label{d:cond}
  On dit que \( F \) satisfait \( \overline C(\xi) \) s'il existe
  une sous-variété abélienne \( \vai \) de \( \va \) dont un translaté est
  contenu dans \( \avar \) et deux points distincts \( x \) et \( y \) dans \(
    F \), tels que \( x - y \in \vai \) et \( \hautn{ x-y } \le \xi \hautn x
  \).

  On dit que \( F \) satisfait la condition \( C(\xi) \) si \( F \) ne
  satisfait pas \( \overline C(\xi) \).
\end{tdef}
...
condition \( C(\xi) \) est vide, c'est-à-dire qu'elle est satisfaite par tout
ensemble.

Par ailleurs, la condition \( \overline C(\xi) \) est d'autant plus forte que
\( \xi \) est petit, autrement dit, plus \( \xi \) est petit, moins la
condition \( C(\xi) \) est restrictive.
...
Tout point admet au moins une variété associée : le stabilisateur de \( \avar
...
\begin{tdef}
  On dit que \( F \) satisfait \( \overline C'(\xi) \) s'il existe deux points
  distincts \( x \) et \( y \) dans \( F \) et une sous-variété abélienne
  \( \vai \) associée à \( x \) telle que \( x - y \in \vai \) et
  \( \hautn{ x-y } \le \xi \hautn x \).

  On dit que \( F \) satisfait la condition \( C'(\xi) \) si \( F \) ne
  satisfait pas \( \overline C'(\xi) \).
\end{tdef}

Il est clair que \( \overline C'(\xi) \) est une condition plus forte que \(
  \overline C(\xi) \), autrement dit que \( C'(\xi) \) est une condition plus
faible que \( C(\xi) \). Notons
\begin{equation}
  \apx(\avar, \eps)
  =
  \set{
...
abélienne, les conditions \( C'(\xi) \) et \( C(\xi) \) sont équivalentes pour
\( F \).

\medskip

Nous allons maintenant essayer de convaincre le lecteur qu'on ne peut guère
espérer majorer le cardinal d'un ensemble d'approximations exceptionnelles
sous une condition plus faible que \( C'(\xi) \) à moins de savoir majorer la
hauteur de certaines (voire toutes les) approximations exceptionnelles. On
supposera \( \eps \le 1 \) pour simplifier certaines estimations.
...
\begin{lem}
  ...
  \begin{align}
    \hautl2 x
    & >
    \frac1\eps \bigl( 11 \hlclab + 37 \ln(n+1) \bigr)
    \label{e:cont-big}
    \\
    \hautl2 y
    & \le
...
\begin{proof}
  ...
  \end{align}
  On observe maintenant, en utilisant des formules d'addition puis l'hypothèse
  sur \( \hautm2 y \), que
  \begin{equation}
    \hautm2{ x+y }
    \le
    \hmclab \hautm2 x ^2 \hautm2 y ^2
    \le
    \hmclab \hautm2 x ^4
  \end{equation}
  ...
  \begin{equation}
    \hautm2 x ^{-\eps} \cdot \hmclab^2 (n + 1)^7
    \le
    \hautm2{ x+y }^{-\eps/5} \hmclab^{\eps/5} \hautm2 x ^{-\eps/5}
    \cdot \hmclab^2 (n + 1)^7
    \pmm.
  \end{equation}
  L'hypothèse sur \( \hautm2 x \) permet alors de conclure, car on a supposé
  \( \eps \le 1 \).
\end{proof}
...
\begin{coro}
  Soient \( x \in \apx'(\avar, \eps) \cap \grp \) et \( \vai \) une variété
  associée à \( x \). On suppose que
  \begin{equation}
    \hautl2 x
    >
    \frac1\eps \bigl( 11 \hlclab + 37 \ln(n+1) \bigr) + \htcmp
  \end{equation}
  et on note \( \hmin(\vai \cap \grp) \) le minimum de \( \hautn y \) où \( y
  \) parcourt l'ensemble des points d'ordre infini de \( \vai(\Qbar) \cap \grp
  \), ou bien \( +\infty \) si cet ensemble est vide. On a alors
  \begin{equation}
    \card \bigl( \apx'(\avar, \eps/5) \cap \grp \bigr)
    \ge
    \card \bigl( \vai(\Qbar)_\torsion \cap \grp \bigr)
    + 2 \bigl( \hautn x / \hmin(\vai \cap \grp) \bigr)^{1/2} - 1
    \pmm.
  \end{equation}
\end{coro}
...
\begin{proof}
  ...
  \begin{equation}
    \set{
      y \in \vai(\Qbar)
      \text{ tel que }
      \hautl2 y \le \hautl2 x
    }
    \cap \grp
  \end{equation}
  vers \( \apx'(\avar, \eps/5) \cap \grp \), qui est évidemment injective. Vu
  l'hypothèse sur \( x \), l'ensemble de départ contient \( \vai(\Qbar) \). De
  plus, si \( z \) est un point de \( \vai(\Qbar) \cap \grp \) tel que \(
    \hautn z = \hmin(\vai \cap \grp) \), alors \( m z \) est dans l'ensemble
  de départ tant que \( m^2 \) est inférieur à \( \hautn x / \hautn z \).
\end{proof}
...
\). Nous sommes maintenant prêts combiner les inégalités de \bsc{Vojta} et de
\bsc{Mumford} pour majorer le nombre de points dans chaque cône tronqué.
...
\worknote{À choisir en fonction de ce qu'on veut présenter dans l'intro ?}
