...
\begin{sco} \label{s:nobig-liouville}
  Soit \( \avar \) une sous-variété de \( \projd \), de dimension \( \adim \) et de
  degré \( \adeg \). Il n'existe aucun point \( x \in \projd(\Qbar) \)
  tel que
  \begin{align}
    \label{e:nobig-ha}
...
\begin{lem} \label{l:big-by-cone}
  ...
  abélienne \( \vai \) de \( \va \) de degré \( \adeg \) et de dimension \( \adim \).
...
\begin{proof}
  Pour commencer, remarquons qu'on peut supposer \( \eps < \adeg + 1 \). En effet,
...
\end{proof}
  ...
  %     + g \ln 5
  %     + \ln \adeg
  %     + {\textstyle \ln \binom{n+\adeg}\adeg}
  %     - \ln \eps
  %   \right)
  %   \\ & \qquad
  %   + m \bigl(
  %     \deg\va + g (\ln2/2 + \ln(mg) + \ln \adeg)
  %   \bigr)
  %   \\ & \le
  %   2 \bigl(
  %     9/2
...
\begin{coro}
  ...
  aux deux conditions suivantes :
  \begin{align}
    0 < \prod\placerange \distv{\ex*} \avar ^\degv
    & <
...
\begin{lem}
  Soit \( \avar \) une sous-variété de \( \va \), de dimension \( \adim \) et de degré
  \( \adeg \). Si \( R \) et \( F \) sont deux réels positifs,
...
\begin{coro} \label{c:kill-small}
  Soit \( \avar \) une sous-variété de \( \va \), de dimension \( \adim \) et de degré
  \( \adeg \). Si \( R \) est un réel supérieur ou égal à
  \begin{equation}
    \frac1\eps \left(
      \hautl1{ \chow \avar }
      + \adeg (\adim+1) \ln(3\adeg)
...
\begin{proof}
  ...
  \begin{equation}
    0
    <
    \prod\placerange \distv{\ex*} \avar ^\degv
    <
    \exp\left(
...
On peut alors choisir \( m \) de façon à optimiser ce résultat dans une
