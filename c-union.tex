...
\begin{lem} \label{l:big-by-cone}
  ...
  \) satisfaisant à XXX et aux conditions suivantes, pour tous \( i \) et \( j
  \) :
  \begin{align}
    \label{e:big-ha}
    0 < \distv{x_i} \avar
...
\begin{coro} \label{c:big-gen}
  ...
  \) de rang fini \( r \), un réel \( \eps > 0 \) et un entier \( m
    ...
    \grp \) satisfaisant à XXX et aux conditions suivantes, pour tout \( i \)
  :
  \begin{align}
    0 < \distv{x_i} \avar
    & <
...
\begin{coro}
  ...
  \) de rang fini \( r \), un réel \( \eps > 0 \) et un entier \( m
    ...
    x_p) \) ; on suppose que cette famille satisfait à XXX et aux conditions
  suivantes, pour tout \( i \) :
  \begin{align}
...
\begin{coro} \label{c:big-m=2g}
  ...
  satisfait à XXX et aux conditions suivantes, pour tout \( i \) :
  \begin{align}
    0 < \prod\placerange \distv{x_i} \avar ^\degv
    & <
    \hautm2{x_i}^{-\eps}
...
on supposera toujours (au moins) \( \eps \le 1 \) pour simplifier certaines
estimations.
...
\begin{lem}
  ...
  dimension \( \adim \). On considère de plus un sous-groupe \( \grp \subset
    \va(\Qbar) \) de rang fini \( r \) et un réel \( 0 < \eps < 1 \).  Soit
  enfin une famille \( x_1, \dots, x_p \) de points de \( \grp \) satisfaisant
  à XXX et aux conditions suivantes, pour tout \( i \) :
  \begin{align}
    \label{e:big-any-ha}
    0 < \distv{x_i} \avar
    & <
...
\begin{coro}
  Soit \( \avar \) une sous-variété de \( \va \), de degré \( \adeg \) et de
  dimension \( \adim \). On considère de plus un sous-groupe \( \grp \subset
    \va(\Qbar) \) de rang fini \( r \) et un réel \( 0 < \eps < \expb^{-1} \).
  Soit enfin une famille \( x_1, \dots, x_p \) de points de \( \grp \)
  satisfaisant à XXX et aux conditions suivantes, pour tout \( i \) :
  \begin{align}
    0 < \distv{x_i} \avar
    & <
    \alpha_v^{-1}
...
\begin{proof}
  ...
  que comme \( \va \) est simple, la condition XXX est automatiquement
  satisfaite.
