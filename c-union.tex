...
\begin{lem}
  ...
  ; on pose \( f(x) = x \ln x + bx + ac / (x-c) \). Si \( ac \le 1 + b +
    \ln(1+c) \), la fonction \( f \) est croissante sur \( [c+1, +\infty
    \mathclose[ \). Sinon, il existe un entier \( x_0 \ge c + 1 \) tel que \(
    f(x_0) \le 2 ac + (c+1) \bigl( 1+b + \ln(1+c) \bigr) \).
\end{lem}
