...
Comme on l'a vu dans la section précédente, on aura intérêt dans l'inégalité
de \bsc{Vojta} (corollaire~\vref{c:vojta-gen2}) à toujours prendre l'option \(
  s = 1 \) et à poser \( m = 2g \). Par ailleurs, pour appliquer notre
inégalité de \bsc{Mumford} pour le cas général (théorème~\vref{t:mumford-gen})
nous devons nous restreindre au cas où \( \va \) est plongée dans \( \projd \)
par un plongement de \bsc{Mumford} modifié associé à une polarisation
principale ; dans ce cas nous avons \( \deg \va = N = n+1 = 16^g \).

Commençons par énoncer l'inégalité de \bsc{Vojta} sous ces conditions.

\begin{coro} \label{c:vojta-gen3}
  Supposons \( \va \) plongée dans \( \projd \) par un plongement de
  \bsc{Mumford} modifié associé à une polarisation principale ; soit
  \( \avar \) une variété de \( \projd \) de degré \( \adeg \) et de dimension
  \( \adim \), et \( 0 < \eps < 1 \) un nombre réel.  Il n'existe dans \(
    \va(\Qbar) \) aucune famille de points \( x_1, \dots, x_{2g} \)
...
Commençons par comparer les conditions des inégalités de \bsc{Mumford}
(théorème~\vref{t:mumford-gen}) et de \bsc{Vojta}
...
inégalités de \bsc{Mumford} et \bsc{Vojta} sont donc
\begin{equation}
  \cos(x, y) \ge 1 - \phi
  \quad\text{et}\quad
  \cos(x, y) \ge 1 -
...
autres conditions de l'inégalité de \bsc{Vojta} encore plus restrictives. Il
est donc avantageux de fixer \( m = g+1 \) dans l'inégalité de \bsc{Vojta}.
...
\begin{lem}
  ...
  \begin{equation}
    ...
    \Bigr)
  \end{equation}
  et \( \phi \) et \( M \) comme ci-dessus. Alors on a
  \begin{equation}
    p
    \le
...
\begin{proof}
  D'après les discussions précédentes, les conditions~\eqref{e:big-any-ha}
  et~\eqref{e:big-any-cos} impliquent les conditions correspondantes du
  théorème~\vref{t:mumford-gen} et du corollaire~\vref{c:vojta-gen3}. On voit
  facilement que c'est également le cas pour l'hypothèse~\eqref{e:big-any-big}
  : en effet, la condition~\eqref{e:Mbig-gen} est visiblement celle qui a la
  plus forte dépendance en chacun des paramètres sauf \( \eps \), où on a donc
  pris l'exposant du corollaire précédent.

  On peut donc appliquer simultanément ces deux résultats et en conclure comme
  dans la preuve du lemme~\vref{l:big-by-cone} que
  \begin{equation}
    p
    \le
    g (M-1) \left(
    ...
    2 g (M-1) \frac{ (4g)^{4g^2} \ln \Lambda_6 }{ \rho }
  \end{equation}
  où \( \rho \) et \( \Lambda_6 \) sont comme ci-dessus, compte tenu du fait
  que cette fois-ci il peut y avoir jusqu'à \( M-1 \) points par lunule.
\end{proof}
