...
sous-ensemble de \( \grp \) satisfaisant
\( \cond{ \eps / \adeg^m (2m)^{(m+1)\adim} } \), alors son intersection avec
une lunule associée aux paramètre donnés contient au plus \( m - 1 \)
approximations exceptionnelles, dans les notations de cet énoncé.
...
condition du type \( \condi\tau \) est la plus faible qu'on puisse mettre sur
...
espérer comme résultats de décomptes sans condition de type \( \condi\tau \) :
...
\bsc{Mordell}-\bsc{Lang} pour contrôler la différence, mais il nous semble
