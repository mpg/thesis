...
faible que \( C(\xi) \). Nous allons maintenant essayer de convaincre le
lecteur qu'on ne peut guère espérer majorer le cardinal d'un ensemble
d'approximations exceptionnelles sous une condition plus faible que \( C'(\xi)
\) à moins de savoir majorer la hauteur de certaines approximations
exceptionnelles. Auparavant nous introduisons les notations commodes
\begin{equation}
  \apx(\avar, \eps)
  =
  \set{
...
\end{equation}
et \( \apx'(\avar, \eps) = \apx(\avar, \eps) \cap \avar(\Qbar) \). On
supposera de plus \( \eps \le 1 \) pour simplifier certaines estimations.
...
Ainsi, on peut construire plusieurs approximations exceptionnelles à partir
d'une approximation exceptionnelle ayant une variété associée non nulle. Le
corollaire suivant donne une précision quantitative.
