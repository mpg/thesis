...
\begin{proof}
  ...
  satisfait à~\eqref{e:rho-phi}. Par ailleurs, il est assez facile de voir que la
  condition~\eqref{e:big-big} est plus forte que~\eqref{e:Mbig} : en effet \(
  ...
  On peut donc appliquer le théorème~\ref{t:mumford} et conclure que
...
\begin{thm}
  \todo \color{out}
  (Résultat final pour les grands points avec hypothèse produit.)
\end{thm}

\begin{proof}
  \color{out}
  Application du scolie précédent au théorème de la section d'avant, puis
  absorption de la constante de l'hypothèse dans la hauteur par changement
  d'epsilon.
\end{proof}
...
présente une autre approche possible pour obtenir des énoncés ne faisant pas
explicitement intervenir l'hypothèse que les points sont grands.
...
On établit ici une variante du résultat principale où l'on se dispense de
...
\begin{coro}
  ...
  théorème~\ref{t:inc-small} est au plus
  \begin{equation}
    \sqrt{\frac d \expapx}
...
\begin{proof}
  L'énoncé de la proposition~\ref{p:all-gen} est valable pour tout \( \puiss
    \ge \genre + 1 \).  Comme dans la preuve du
  théorème~\ref{t:inc-small}, on remarque la quantité \(
    \cst{ht-v-gen1} \Lambda_4^{(2\puiss\genre)^{\puiss\genre}}
  \) est minimale pour \( \puiss = \genre + 1 \) et on spécialise donc
  la proposition~\ref{p:all-gen} avec cette valeur.
\end{proof}
