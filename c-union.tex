...
\begin{tdef} \label{d:cond}
  On dit que \( F \) satisfait \( \bar C(\xi) \) s'il existe
...
Plus précisément, on remarque que la condition \( \bar C(\xi) \) est d'autant
plus forte que \( \xi \) est petit, autrement dit, plus \( \xi \) est petit,
moins la condition \( C(\xi) \) est restrictive. On remarque que les
inégalités de \bsc{Mumford} du chapitre précédent disent en fait que
l'ensemble de points considéré satisfait \( C(\xi) \) avec,
\( \xi = \rho^2/4 + 2\phi + \rho\phi \), quantité qui peut être rendu
...
avec la valeur « naturelle » de \( \xi \) pour éviter d'alourdir le texte.
...
\begin{tdef}
  On dit que \( F \) satisfait \( \bar C'(\xi) \) s'il existe deux points
...
Il est clair qu'en général \( \bar C'(\xi) \) est une condition plus forte que \(
  \bar C(\xi) \), autrement dit que \( C'(\xi) \) est une condition plus
...
hauteur de certaines (voire toutes les) approximations exceptionnelles.
...
\begin{proof}
  ...
  Il s'agit donc de montrer que cette dernière quantité est majorée par \(
    \hautm2 y ^{-\eps/5} \).  Si l'on note \( \nnt\truc \) la norme de
  l'espace de \MoW, on a
  \(
    \nnt{ x+y }
    \le
    \nnt x + \nnt y
  ...
  \end{equation}
  puis
  \( - \frac45 \eps \hautl2 x \le - \frac\eps5 \hautl2{ x+y } + \eps \htcmp \)
  ce qui donne immédiatement
  \begin{equation}
    \hautm2 x ^{-\eps} \cdot \hmclab^2 (n + 1)^7
    \le
    \hautm2{ x+y }^{-\eps/5} \expb^{\eps \htcmp} \hautm2 x ^{-\eps/5}
    \cdot \hmclab^2 (n + 1)^7
    \pmm.
  \end{equation}
  L'hypothèse sur \( \hautn x \) permet alors de conclure.
\end{proof}
...
\begin{coro}
  ...
  satisfaisant~\eqref{e:cont-big} et \( \vai \) une variété associée à \( x
  \). On note \( \hmin(\vai \cap \grp) \) le minimum de \( \hautn y \) où \( y
