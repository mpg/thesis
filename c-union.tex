...
L'autre méthode de traitement des petits points consiste à renforcer la
  ...
  \begin{equation}
    ...
    \left(
      \frac{
        5 \cst{ht-m=g+1}
        \Bigl(
          86 \nclmaps \cdot d^{\genre+1} \sbin{\dimp + d}{d}
          \, \expapx^{-1}
...
\begin{proof}
  ...
  \begin{equation}
    ...
    \pmm.
  \end{equation}
  Il suffit alors de prendre \( \puiss = \genre + 1 \) (choix qui minimise la
  quantité précédente, croissante en \( \puiss \)) pour obtenir le résultat
  annoncé.
\end{proof}
...
ou bien le décompte le plus précis possible. Dans le premier cas, on obtient
le résultat suivant.
