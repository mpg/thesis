...
\begin{proof}
  Pour chaque \( v \in \placesapx \), il existe un point \( p_v \in
    \utrans\avar\vai(\Cv) \) tel que \( \distv x{ p_v } = \distv x{
      \utrans\avar\vai } \). Le scolie~\vref{s:addsub-dv} donne alors (en
  remarquant que \( 5\sqrt2 \le (n+1)^2 \)) :
  \begin{equation}
    \distv{ x+y }{ p_v + y }
    \le
    \distv{ x }{ p_v }
...
\begin{coro} \label{c:factory}
  ...
  \( (1-\sqrt\tau)^{2} \, \hautn x \) (avec la convention \( (+\infty)^{-1} =
    0 \)). En particulier, cet ensemble contient une infinité de grands points
  (au sens précédent) dès que \( \vai(\Qbar)_\torsion \cap \grp \) est infini.
\end{coro}
...
suffit), dans le cas particulier où \( \avar = \utrans\avar\vai \), qui inclut
le cas où \( \avar \) est un translaté de \( \vai \), ou plus généralement
la somme de \( \vai \) et d'une sous-variété non abélienne, par exemple.
...
\begin{proof}
  ...
  que \( x \in E'(\eps, T, \lambda) \) pour n'importe quelle famille \(
