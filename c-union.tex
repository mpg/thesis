...
suffit), dans le cas particulier des translatés de sous-variétés abéliennes
non nulles.

\begin{coro} \label{c:count2ht}
  Soit \( \avar = z + \vai \) un translaté d'une sous-variété abélienne non
  nulle de \( \va \).  Supposons que pour un certain \( \eps_0 > 0 \) et un
  certain \( \grp \) de type fini, il existe \( R \) et \( N \) tels que
  \begin{equation}
    \card
    \set{
    ...
    \pmm.
  \end{equation}
  Alors, pour tout \( \eps > \eps_0 \) les points de \( \apx(\avar, \eps) \cap
    \grp \) sont tous de hauteur normalisée inférieure ou égale à
  \begin{equation}
    \max \Bigl(
...
\begin{lem} \label{l:big-by-cone}
  ...
  \begin{align}
    ...
    \\ & \qquad
      + \adeg^g \bigl (
        \hautl1 \avar + (\adim + 2) \log(\adeg + 1) (\adeg + n+1) + \log(m/2)
      \bigr)
    \Bigr)
  \end{align}
  et au choix :
