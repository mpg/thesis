...
Soient \( \avar \subset \va \) une sous-variété, \( F \) un sous-ensemble de
\( \va(\Qbar) \) et \( \xi > 0 \) un réel. On introduit une condition sur
\( F \), associée à \( \xi \), de la façon suivante.
...
\( \vai = \vai_0 \) dans l'énonce de la condition. À l'inverse, si \( V \) ne
...
\textcolor{out}{%
  À faire : énoncer la condition \( C'(\xi) \), expliquer pourquoi elle est
  plus faible que \( C(\xi) \) et pourquoi on ne peut guère espérer majorer le
  cardinal d'un ensemble d'approximations exceptionnelles sous une condition
  plus faible que \( C'(\xi) \) à moins de savoir majorer la hauteur des
