...
\begin{tikzpicture}[
  ...
  \matrix [row sep=1cm, column sep=2.4cm] {
      \node[result]  (var-deg)   { \( \deg Z \) };
    & \node[result2] (aux-ex)    { Existence \\ et degrés };
    & \node[result2] (ind-ok)    { Indice \\ extrapolé };
    & \node[result2] (fo-ex)     { Existence \\ et degré };
    & \node[result]  (next-deg)  { \( \deg Z' \) };
    \\
      \node[result] (var-ht)     { \( h(Z) \) };
    & \node[result] (aux-ht)     { Hauteur };
    & \node[result] (ind-ht)     { À \( h(e_1) \) fixé };
    & \node[result] (fo-ht)      { Hauteur };
    & \node[result] (next-ht)    { \( h(Z') \) };
    \\
  };

  % nœuds supplémentaires
  \node[hypo] (in-deg)     [left=of var-deg] { \( \deg A \) };
  \node[hypo] (in-ht)      [left=of var-ht]  { \( h(A) \) };

  \node[hypo] (eps-zgti)   [above=of aux-ex] { \( \eps_0 \gg \eps_1 \) };

  \node[hypo] (Vcos)       [above=of ind-ok] { \( \gamma \ll 1 \) };
  \begin{scope}[node distance=2mm]
    \node[hypo] (eps)      [right=of Vcos]   { \( \eps > 0 \) };
    \node[hypo] (eps-igtz) [left=of Vcos]    { \( \eps_1 \gg \eps_0 \) };
  \end{scope}

  \node[hypo] (Vfar)       [above=of fo-ex]  { \( \beta \gg 1 \) };

  \begin{scope}[node distance=3cm]
    \node[result] (out-ht) [below=of var-ht] { \( h(e_i) \) };
    \node[stop]   (end)    [below=of aux-ht] { Contradiction };
    \node[hypo]   (Vbig)   [below=of ind-ht] { \( \alpha \gg 1 \) };
  \end{scope}
  ...
  \draw[puiss]  (eps-zgti)   to node [swap] { \( \scriptstyle m > g \) } (eps-igtz);
  ...
  \node[above] at (rec.north)  {
    Partie itérée.
    Entrée : \( Z = A^m \) ;
    sortie : \( Z_i = \{ e_i \} \) pour un certain \( i \).
  };
