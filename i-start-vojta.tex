...
famille \( \ex[1], \dots, \ex[\puiss] \) qui le contredit. Nous
...
\begin{math}
  ...
  \cdn [ \vmp ]
\end{math}.
La famille \( \ex \) est toujours supposée satisfaire aux
...
La preuve utilisera des combinaisons linéaires de \( \ex* \) de petit
...
\begin{proof}
  ...
  \begin{alignat}{2}
    ...
    & =
    \normn{\wt* \ex*}^2 + \normn{\wt** \ex**}^2
    - 2 \scalaire{\wt* \ex*}{\wt** \ex**}
    \\
    & = \wts* \normn{\ex*}^2 + \wts** \normn{\ex**}^2
    - 2 \wt* \wt** \scalaire{\ex*}{\ex**}
    \\
    & = \left( \wt* \normn{\ex*} - \wt** \normn{\ex**} \right)^2
    + 2 \wt* \wt** \left(
      \normn{\ex*} \normn{\ex**} - \scalaire{\ex*}{\ex**}
    \right)
    \\
    & \le \left( \wt* \normn{\ex*} - \wt** \normn{\ex**} \right)^2
    + 2 \wt* \wt** \normn{\ex*} \normn{\ex**} \Vcos
...
$(\ex = \ex[1], \dots, \ex[\puiss])$
de $\var(\Qbar)$, où les $\ex*$ sont de hauteur grandes et étagées entre
...
encore $\ex*$ l'image de $\ex*$ dans cet espace).
...
\begin{equation} \label{e:def-epsiii}
  ...
  \pmm.
\end{equation}
\todo La définition de \( \wt \), les hypothèses faites sur \( \ex \) et la
...
aussi que $\wt* \ex* - \ex[\puiss]$ est de hauteur petite devant les
hauteurs des différents $\ex*$. Pour nous aider à exploiter ultérieurement
