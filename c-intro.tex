...
assez grandes, puis de façon indépendante par \bsc{Farhi}
...
À notre connaissance, à ce jour, la seule démonstration de l'ex-conjecture de
\bsc{Lang} énoncé ci-dessus est celle de \bsc{Faltings} : en particulier on
n'en connaît pas de version quantitative explicite. Les résultats obtenus ici
permettent de donner un décompte explicite des points entiers, malheureusement
pas dans le cas général, mais au moins dans un certain nombre de cas, par
exemple si \( \va \) est simple.
...
de \( N \). En particulier, les constantes de l'inégalité de \bsc{Mumford}
liées à la hauteur de \( \va \) peuvent être explicitées totalement, alors que
celles de l'inégalité de \bsc{Vojta} dépendent de formules globales de
...
\begin{rem} \label{r:ha-cdn}
  ...
  La notation \( \cdn \) désignera n'importe quel corps de nombres « assez
  grand », contenant en particulier un corps de définition des approximations
  exceptionnelles considérées. Les notations \( \placesapx \) et \( \wtapx \)
  auront toujours le sens ci-dessus ; en particulier on supposera toujours
  que~\eqref{e:wtapx-1} est satisfait sans que cela soit rappelé.
