...
suivante du théorème avait été conjecturée par \bsc{Lang}, de façon analogue à
sa généralisation de la conjecture de \bsc{Mordell}.
...
dans certains cas \cite{silqs, biluqs}.  Par contre, à ma connaissance, la
seule démonstration connue de sa généralisation est celle de \bsc{Faltings} :
en particulier on ne connaît pas de version quantitative de cette ex-conjecture
de \bsc{Lang}. Malheureusement, les résultats obtenus ici comportent une
limitation qui ne permet pas d'en déduire de version quantitative explicite de
cette extension du théorème de \bsc{Siegel}.
...
\begin{lem} \label{l:hmclab}
  ...
  \) ouverts recouvrant \( \va^2 \), et une paire de constantes \( \hmclab \)
  et \( \hlclab = \ln \hmclab \), dont la première se décompose en \( \hmclab
    = \prod_v \hmclab*^\degv \) où les \( \hmclab* \) sont des réels tous
  supérieurs et presque tous égaux à \( 1 \), possédant la propriété suivante.
...
\begin{proof}
  ...
  plongement fixées, et une famille de constantes \( (C_{2,3,v})_v \) presque
  ...
  représentant la soustraction et les constantes \( \hmpm* \) associée,
  données par le lemme suivant.
\end{proof}

\begin{lem} \label{l:hmpm}
  Il existe un entier \( \nclmaps \), un ensemble \( \clmaps \) de \( \nclmaps
  \) ouverts recouvrant \( \va^2 \), une paire de constantes \( \hmpm \)
  et \( \hlpm = \ln \hmpm \), dont la première se décompose en \( \hmpm
    = \prod_v (\hmpm*)^\degv \) où les \( \hmpm* \) sont des réels tous
  supérieurs et presque tous égaux à \( 1 \), et enfin une famille \(
    (D\pexp\clmap)_{\clmap \in \clmaps} \) de formes bihomogènes de bidegré \(
    (2, 2) \) telles que
  \begin{enumerate}
...
\begin{nota} \label{n:vaemb}
  ...
  lemmes précédent et le fait~\ref{f:htcmp}. De plus, on supposera que
  \( n \ge 2 \) et \( N \ge 3 \), et le degré de (l'image de) \( \va \) dans
  le plongement est au moins \( 3 \), ce qui est le cas dans les plongements
  usuels : plongement de \bsc{Weierstrass} pour une courbe elliptique ou en
  général, plongement de \bsc{Mumford} modifié (voir ci-dessous).
\end{nota}
...
\cite{daphimhva2}. Nous noterons \( \hautl2(\oa) \) la quantité notée \(
...
le multiplication par \( 2 \) et possédant des propriétés analogues à celles
...
\( \hmpm* \) en fonction des constantes (explicites) données par le lemme en
question.
...
un \emph{nullstellensatz} effectif, une valeur totalement explicite de \(
  \hmclab \). Par ailleurs, \bsc{Farhi} donne une valeur explicite de \(
