...
\todo[formes de Chow et hauteur d'une variété, notation \( \gamma_n \).]

\subsection{Formes homogènes et multihomogènes}

On notera généralement \( \cp = (\cp[0], \dots, \cp[n]) \) un groupe de
variables. Si \( \ip \in \N^{n+1} \) est un multiindice, on notera
\( \cp^\ip = \cp[0]^{\ip[0]} \cdots \cp[n]^{\ip[n]} \). On introduit également
...
\begin{equation}
  \frac{\partial^\ip}{\partial \cp^\ip}
  =
  \frac{ \partial^{\ip[0]} }{ \partial \cp^{\ip[0]} }
  \cdots
  \frac{ \partial^{\ip[n]} }{ \partial \cp^{\ip[n]} }
  \quad\text{et}\quad
  \der\ip
  =
  \frac1{\ip!} \frac{\partial^\ip}{\partial \cp^\ip}
  \pmm.
\end{equation}
...
\( \cmp = (\cmp[1], \dots, \cmp[m]) \) un groupe de groupe de variables, où \(
  \cmp[i] = (\cmp[i][0], \dots, \cmp[i][n]) \). On étend naturellement les
notations projectives : pour \( \imp \in \N^{(n+1)m} \) on note \( \cmp^\imp =
  (\cmp[1])^{\imp[1]} \cdots (\cmp[m])^{\imp[m]} \) ; on procède de même pour
...
autrement dit \( \av p = p^{-1} \) si \( \place \) divise un premier \( p \)
et \( \av 2 = 2 \) si \( \place \) est archimédienne.
...
\( \place \)-adiques de ses éléments. Si \( v \) est archimédienne, on
...
\begin{equation}
  ...
  =
  \bigl(
  \sum_\alpha \av{P_\alpha}^2 \cdot \binom{\delta}{\alpha}^{-1}
  \bigr)^{1/2}
  \quad\text{où }
...
enfin \( \gamma_n = \frac12 \sum_{q=1}^{n} \frac1q \). On renvoie
...
On définit maintenant la hauteur additive d'une famille finie d'éléments de \(
  \cdn \), d'un polynôme ou d'une forme (multi)homogène sur \( \cdn \) comme
\begin{equation}
  \hautl[\star]\truc
  =
  \sum_{\place}
...
ce plongement est associé à un fibré \( \fibre \) symétrique, on peut
...
sont éventuellement données pour certains types de plongement.
