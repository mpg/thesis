...
posons \( R_\ind = \poldep[][\anydim+1](1, \vaf[1], \dots, \vaf[\anydim],
  \vaf[\indi] \vafi) \). On peut alors appliquer le lemme précédent avec \( L
  = \cdn(V) \) en prenant pour \( \vaf[\indi]' \) et \( \omega \) les images
respectives de \( \vp[\indi]/\vp[0] \) et \( \vp[\anydim+1]/\vp* \) dans ce
dernier, ainsi que \( R = R_\ind \) car ce choix convient.\todo[détailler que
\( R \) convient] On note \( \tilde P_{\ind, \dv}^\derp \) les polynômes
obtenus et \( \tilde Q_\ind = \diff_\vafi R_\ind \). On a alors, dans \(
  \cdn(\anyvar) \) :
\begin{align}
  ...
  \right)^{2(2\anydeg-1)(2\lgr\derp-1)}
  \\ & =
  \frac{
    P_{\ind, \dv}^\derp
  }{
...
\end{align}
où \( P_{\ind, \dv}^\derp \) est homogène de degré \(
  2(2\anydeg-1)(2\lgr\derp-1) \) en \( \vp[0], \dots, \vp[\anydim+1] \) et \(
  Q_\ind \) homogène de degré \( 4\anydeg - 2 \) en ces mêmes variables, en
remarquant que \( \deg R = 2\anydeg \). Par ailleurs on a
\begin{equation}
  \nv1{ P_\dv^\derp }
  \le
