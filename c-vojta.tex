...
\begin{proof}[\proofname\ du théorème~\vref{t:vojta-div}]
  ...
  (\emph{arithmetic Bézout theorem}, page~87 de \cite[chap.~6]{nesphilnm}) :
  \begin{equation}
...
\begin{fact} \label{f:thm-prod}
  ...
  \begin{enumthm}
    ...
    \item la hauteur de \( T \) satisfait à
      ...
      \end{align}
      où le nombre de \bsc{Stoll} et défini par~\eqref{e:def-stoll}.
  \end{enumthm}
\end{fact}
...
remodelé, tel que défini dans \cite[p.~14]{jadotth} ou
\cite[p.~102]{nesphilnm}, dont on rappelle la définition (sur les coordonnées
homogènes) :
\begin{align}
...
\begin{proof}
  On utilise la remarque de~\cite[p.  102]{nesphilnm} pour interpréter les
  ...
  intéressent dans~\cite[p. 85-86]{nesphilnm} : les
  ...
  point~\vref{i:hs-vero-ht-va} en comparant les hauteurs utilisées comme
  en~\eqref{e:ht-pph-1} :
  \begin{align}
    \hautl{1, \vaemb'} \va
    & \le
  ...
  formule en haut de la page~89 de~\cite{nesphilnm} qu'on interprète ainsi :
  \begin{equation}
    \distv[v, \vaemb'] x{y_v}
    =
  ...
  de~\cite[p. 85-86]{nesphilnm} utilisés ci-dessus ne s'appliquent pas, car ce
