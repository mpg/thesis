...
absolue représente moralement la forme de \( \cexa* \).
...
\begin{proof} \todo[Mettre à jour, cf plongement et~\eqref{e:nv-poldep}]
  Il suffit de montrer qu'on a \( \av{ \cex** } \le \nv1{ \varfc* } \av{
      \cex*[\indv] } \) pour tout \( \ind \in \set{\vdim* + 1, \dots, \dimp}
  \).  En décomposant \( \poldep** \) suivant les puissance de \( \vmp** \),
  on a
  \begin{equation}
    (\vmp**) ^{ \vdeg* }
    =
