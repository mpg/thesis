...
Nous allons maintenant expliciter des applications linéaires \( \rfull^\beta
\) définies sur chaque partie homogène \( \cdn[ \vmp ]_\beta \) pour \( \beta
  \in \N^\puiss \) et à valeurs dans \( \cdn[ \vmp ]^{C'''} \), qui permet par
exemple de déterminer l'appartenance à \( \varida \) d'une forme homogène.
...
\nomuse \wemba [chap] {Représentation polynomiale du plongement éclatant} Une
représentation locale, définie sur un ouvert \( \clmap \),  de \( \wemb \)
dans ces plongements, est un morphisme \( \wemba \) tel que le diagramme
suivant, dont les flèches verticales sont les projections canoniques, commute :
\begin{equation}
  \xymatrix{
...
l'ouvert ouvert \( \clmap \).

Introduisons des formes
\begin{equation} \label{e:def-wembclm}
  \wembcl*_\clmap
  = L\pexp{\wti*, \wti**, \clmap}
  \bigl( \vadapt[\fcti]^{-1}(\vmp[\fcti]), \vadapt**^{-1}(\vmp[\puiss]) \bigr)
\end{equation}
où \( L\pexp{\wti*, \wti**, \clmap} \) est une famille de formes bihomogènes
donnée par le lemme~\ref{l:hclab}, de sorte que \( \wembcl*_\clmap \)
représente \( \clab[-]{\wti*}{\wti**} \) dans les plongements utilisés. Il est
alors clair que chacune des applications
\begin{align}
     \wemba^\clmap \colon \cdn [\vmp, \vmpi]
  &  \to \cdn[\vmp]
  \\ \vmp*
  &  \mapsto \vmp*
  \\ \vmpi*
...
sens précédent, où \( \clmap \in \clmaps^{\puiss-1} \). Dans la suite de cette
section, la carte \( \clmap \) choisie n'a pas d'importance, on notera donc
simplement \( \wemba \) l'un des \( \wemba^\clmap \) pour alléger.
...
allons construire une forme \( F \) sur \( \wemb(\var) \) et la tirer
localement en arrière sur \( \var \) en une famille de formes \( F'_\clmap \)
grâce aux différents morphismes \( \wemba^\clmap \) introduits à la
sous-section~\ref{sec:wemb}.

Revenons maintenant aux \( F'_\clmap \) : on ne peut brutalement exiger que
...
sous-section~\ref{sec:rfull} et exiger que \( F''_\clmap = \rfull(F'_\clmap)
...
et on rappelle que \( \delta \) désigne un grand entier tel que \( \delta
  \eps_0 \) soit aussi entier. On impose alors à \( F \) d'être
de degré \( \Dz \), de sorte que les \( F'_\clmap \) seront de degré \( \Di \)
et les \( F''_\clmap \) de degré \( \Dir = \Di + o(\delta) \). Par ailleurs,
...
dans lequel on cherchera \( F \).  L'entier \( \delta \) sera choisi assez
...
\begin{align} \label{e:def-ftarget}
  ...
  & \mapsto
  \bigl(
    \pi_{\epsi\delta} \circ \rfull^{\Di} \circ \wemba^\clmap(F)
  \bigr)_\clmap
\end{align}
...
que \( \card \clmaps = (\dimp + 1)^{\puiss-1} \) d'après le
...
\begin{align}
  ...
  \pmm.
\end{align}
\todo[reprendre avec les bonnes valeurs des \( \eps \)]%
...
Nous énonçons maintenant quelques conséquences faciles de la proposition
précédente.
...
\begin{proof}
  ...
  nombre de coefficients de \( F \), qui est au plus
  \begin{equation}
...
\begin{scho} \label{s:aux-co}
  On rappelle qu'on note \( F''_\clmap = \rfull \circ \wemba^\clmap(F) \) pour
  toute carte \( \clmap \in \clmaps \). Ces formes satisfont alors
  \begin{enumthm}
    \item \( \inda* F''_\clmap \ge \epsi \delta \) ;
    \item \( \deg F''_\clmap = \Dir \) avec
      \(
        d' = \bigl(
          \wts[1] (2 + \epsz),
          \dots,
    ...
    \item \(
        \hautl[1]{F''_\clmap}
        \le
        4 \delta \wts[1] \Lambda^{2f(\vdim)} \left(
          4 \cst{vs-ht} + 12 \dimp \cst{vs-deg-prod}^{-2}
        \right) + o(\delta)
      \).
  \end{enumthm}
...
\begin{proof}
  Les deux premiers points ne sont qu'un rappel de la construction.
  Pour le dernier point, on utilise la définition de \( F''_\clmap \) et la
  fait que la majoration donnée par~\eqref{e:hmat-sigma} pour la hauteur de la
  matrice de \( \sigma \) est valable (par construction) pour celle de la
  matrice de \( \rfull \circ \wemba^\clmap \). Comme c'est cette quantité qui
  détermine la hauteur de \( F \), il est clair qu'on peut majorer celle de \(
    F''_\clmap \) par le double de la majoration annoncé pour cette dernière,
  en utilisant si on veut la hauteur \( \Hautl[1] \) par un argument similaire
  à celui du scolie précédent.
\end{proof}
...
sous-section~\ref{sec:siegel-plan}, mais cette fois en un point. Posons
\begin{equation}
  \wtsum( \dermp )
  =
...
\end{equation}
pour tout \( \dermp \in \N^\vdim \).  Si \( h \) est une fraction rationnelle
et \( \point \) un point où elle est définie, on définit son
...
\begin{equation} \label{e:def-f1}
  \ratf1
  = \frac{ F''_{\clmap_\ex}(\vmp) }{ \prod\fctrange (\vmp*[\vdim*])^{\Dir*} }
\end{equation}
où \( \clmap_\ex \in \clmaps^{\puiss-1} \) est une carte contenant \( \ex \).
Cette expression est bien homogène de degré \( 0 \) et définie en \( \cexa \).
Le but de cette section est alors de montrer la proposition suivante.
...
On introduit ensuite le point \( \exi \in \va^{\puiss-1} \) tel que \(
  \wemb(\ex) = (\ex, \exi) \) et on note \( \cexi* = \wembcl*_{\clmap_\ex}
...
Pour tout place \( \place \) on note désormais \( \wembclv* =
  \wembcl*_{\clmap_{\wts*, \wts**, \ex*, \ex**, \place}} \) où \( \wembcl* \)
est défini par~\eqref{e:def-wembclm} et \( \clmap_{\wts*, \wts**, \ex*, \ex**,
    \place} \) donné par le lemme~\ref{l:hclab}.  Ainsi, pour chaque \( \fcti
\) les vecteurs \( \wembclv* (\cexa[\fcti], \cexa[\puiss]) \) sont
proportionnels entre eux quand \( \place \) varie et proportionnels à \(
  \cexi* \). On a alors, pour tout \( \place \) :
\begin{align}
  \ratf2
  & =
  \frac{
...
parcourt en fait un ensemble fini lorsque \( \place \) parcourt l'ensemble des
...
ensemble fini quand \( \place \) varie, car c'est déjà le cas de \( F''_\place
\) et de \( \wembclv \).

Rappelons également que l'on a supposé que \( \wtsum(\dermp) \le \epsi \delta
  / \sigma \) ; en remarquant que \( \inda*(F''_\place) \ge \epsi \delta \)
par le lemme précédent  et que \( \wtsum*(\dermp) \le \wtsum(\dermp) \) par
définition, le dernier point donne
