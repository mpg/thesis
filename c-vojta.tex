...
\nomuse {\wtsum*} {Somme pondérée servant à définir l'indice d'annulation le
  long de \( \divi \)}
\begin{equation}
  \wtsum*( \imp )
  =
...
On définit l'indice le long de \( \divi \) d'une forme \( H = \sum h_\imp
  \vmp^\imp \) par :
\nomuse {\inda*} {Indice d'annulation le long de \( \divi \)}
\begin{equation}
  \inda*(H)
  =
  \max \set{
...
\vmpi]$, son image par $\wemba$ est de multidegré exactement $\delta d'$.  On
...
\begin{prop} \label{p:build-aux}
  ...
  \( \delta \) est suffisamment grand, il existe une forme \( F \in \cdn
    [\vmp, \vmpi] \), multihomogène de degré \( \delta d \), où l'on
  rappelle que
  \begin{equation}
    d = \bigl(
      \epsz \wts[1],
