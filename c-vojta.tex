...
Commençons par rappeler quelques faits évidents : le dimension \( \genre \) de
\( \va \) est au moins \( 1 \) car sinon l'énoncé est vide ; le nombre de
points considérés \( \puiss \) vaut donc au moins \( 2 \) et pour la dimension
de l'espace de plongement on a \( \dimp + 1 \ge 16^\genre \ge 16 \) comme il
est rappelé en section~\ref{sec:plong-mm-def}. Par ailleurs, \todo[référence]
on a \( \deg \va = \dimp + 1 \) dans le plongement considéré. 
...
\begin{equation}
  ...
  5
\end{equation}
et, en constatant que \( 66^2 \ge 16^3 \),
\begin{equation}
  \frac{ \Lambda }{ \ln \Lambda }
  \ge
  \frac{ 16^3 (\dimp + 1)^3 }{ 3 \bigl( \ln(\dimp + 1) + \ln(16) \bigr) }
  \ge
  \frac{ 16^3 (\dimp + 1)^3 }{ 6 \ln(\dimp + 1) }
  \ge
  \frac{ 5 \cdot 16^3 }{ 6 }  (\dimp + 1)^2
  \pmm.
\end{equation}
...
\begin{equation} \label{e:B-siegel}
  ...
  \pmm.
\end{equation}
Pour le premier terme du membre de droite, au vu des estimations précédentes,
on écrit
\begin{equation}
  \ln((\dimp+1)!)
  \le
  ...
  \frac{ (\dimp+1)^2 }5
  \le
  \frac{ \Lambda }{ 5 \cdot 16 \cdot 66^2 }
  \pmm.
\end{equation}
Par ailleurs, en majorant grossièrement \( B \) par \( \Lambda \), il vient
\begin{equation}
  (\dimp + 3) \ln B
  \le
  (\dimp + 1)^2 \ln \Lambda
  \le
  \frac{ 6 }{ 5 \cdot 16^3 } \Lambda
  \pmm.
\end{equation}
