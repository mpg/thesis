...
\begin{prop} \label{p:varset-notmin}
  ...
  \begin{align}
    ...
    \Biggl(
      6 \sum_{\fcti=1}^\puiss \wtw[\fcti] \wts[\fcti] \hautl[1]{\var[\fcti]}
...
\begin{proof}[\proofname\ du théorème~\ref{t:vojta-div}]
  ...
  \begin{equation}
    ...
    \Biggr)
  \end{equation}
  car \( \Lambda > \dimp \) et \( \wts[1] \ge \wtw* \wts* \). Appliquons le
  théorème de \bsc{Bézout} arithmétique qu'on vient de rappeler :
  \begin{align}
    \sum_{\fcti=1}^\puiss
    ...
    \Biggr)
    \\ & \le
    \genre B \Lambda^{2\vdim f(\vdim)}
    \Biggl(
      7 \sum_{\fcti=1}^\puiss \wtw[\fcti] \wts[\fcti] \hautl[1]{\var[\fcti]}
      + 4 \wts[1] \cst{vs-ht} \Lambda^{(1 + \frac1\puiss) f(\vdim)}
    \Biggr)
    \label{e:vs-ht-rec}
  \end{align}
  en utilisant le fait que \( \vdim* \le \genre \).  Il ne reste plus qu'à
  appliquer les hypothèses sur \( \var \) (découlant de son appartenance à \(
    \varset(\ex) \)), plus précisément~\eqref{e:varset-ht}
  puis~\eqref{e:varset-deg} sous la forme \( 11 \genre B \le \Lambda^{f(u)}
  \), pour obtenir :
  \begin{align}
    ...
    \pmm.
  \end{align}
  On remarque enfin que, comme \( \vdim \ge \puiss \), on a \( 2 \vdim + 2 +
    \frac1\puiss \le 2\vdim + 1 + \frac{2\vdim}\puiss + \frac1\puiss =
    (2\vdim + 1)(1 + \frac1\puiss) \), ce qui achève de montrer que \( \var'
...
conserver la dépendance en \( \sum\fctrange \wtw* \wts* \hautl[1]{\var*} \)
...
\begin{align}
  ...
  \Biggl(
    \frac1\delta \hautl[\infty]{ \faux** }
    + \frac12 \wts[1] \Lambda^{(1 + \frac1\puiss) f(\vdim)}
  \Biggr)
  + o(1)
\end{align}
...
\begin{align}
  ...
  \Biggl(
    6 \sum_{\fcti=1}^\puiss \wtw[\fcti] \wts[\fcti] \hautl[1]{\var[\fcti]}
    + \frac32 \wts[1] \cst{vs-ht} \Lambda^{(1 + \frac1\puiss) f(\vdim)}
  \Biggr)
  + o(\delta)
  \\ & \le
  \vdim* \Lambda^{2\vdim f(\vdim)}
...
terme en \( o(\delta) \) de la permière ligne soit plus petit que la quantitée
perdue en remplaçant \( \frac32 \) par \( 2 \) dans le terme précédent, qui ne
dépend pas de \( \delta \).
...
\begin{align*}
  ...
  \pmm.
\end{align*}
On invoque alors~\eqref{e:ht-T'-adhoc} pour conclure que
\begin{align*}
  \wtw* \wts* \hautl[\infty]{ T' }
  & \le
  \vdim* \Lambda^{2\vdim f(\vdim)}
...
successivement par
\begin{equation}
  12 \puiss^2 \genre \ln \Lambda^{f(\vdim)}
  \le
  12 \puiss^3 \genre \Lambda^{\frac1\puiss f(\vdim)}
  \le
  \frac12 \Lambda^{(1 + \frac1\puiss) f(\vdim)}
\end{equation}
