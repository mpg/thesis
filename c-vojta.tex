...
\begin{thm} \label{t:vojta-div}
  ...
  Soit \( \vaemb \colon \va \embedin \projd \) un plongement satisfaisant aux
  hypothèses~\ref{h:vaemb} et \( \divi \) l'hyperplan défini par 
  \( \vp[0] = 0 \) dans ce plongement.  Pour tout réel \( 0 < \eps < 1 \),
...
coordonnées linéaire. Si c'est un hypersurface quelconque, on se ramène à un
...
Commençons par le cas d'un hyperplan quelconque. Le lemme suivant donne un
changement de base de petite hauteur.

\begin{lem}
  Soit \( G \) un hyperplan de \( \Qbar^{\dimp+1} \), d'équation \( L \) dans
  la base canonique. Si \( \dimp \ge 2 \), il existe une base \( (b_0, \dots,
    b_\dimp) \) de \( \Qbar^{\dimp+1} \) telle que \( b_i \in G \) si et
  seulement si \( i \neq 0 \), et
  \begin{equation}
    \hautm[2]{b_0} \cdots \hautm[2]{b_\dimp}
    \le
    \dimp^{\dimp/2} \, \hautm[2]{L}
    \pmm.
  \end{equation}
\end{lem}

\begin{proof}
  On commence par choisir une petite base de \( G \) en invoquant une version
  absolue forte du classique lemme de \TS.
  D'après le lemme~4.7 (p. 523) de~\cite{daphimht} et la remarque qui le suit,
  pour tout \( \eps > \), il existe une base \( (b_1, \dots, b_n) \) de
  \( G \) telle que
  \begin{equation}
    \hautl[2]{b_1} + \cdots + \hautl[2]{b_\dimp}
    \le
    \hautl[2]{L} + \sum_{i=1}^{\dimp-1} \sum_{j=1}^{i} \frac1{2j} + \eps
    \pmm.
  \end{equation}
  Or la double somme du membre de droite est majorée par \( \dimp \ln \dimp /
    2 \) (majoration classique du nombre de \bsc{Stirling}) ; en fait, pour \(
    \dimp \ge 2 \) la différence entre ces deux quantités est même supérieure
  ou égale à \( \ln 2 - 1/2 \). Il suffit alors de choisir \( \eps \)
  inférieur à cette dernière quantité pour avoir une base \( (b_1, \dots, b_n)
  \) de \( G \) telle que
  \begin{equation}
    \hautm[2]{b_1} \cdots \hautm[2]{b_\dimp}
    \le
    \dimp^{\dimp/2} \, \hautm[2]{L}
    \pmm.
  \end{equation}
  Enfin, on choisit pour \( b_0 \) un vecteur de la base canonique
  n'appartenant pas à \( G \). Ainsi, \( (b_0, \dots, b_\dimp) \) est
  une base de \( \Qbar^{\dimp+1} \) possédant les propriétés annoncées.
\end{proof}

\begin{lem}
  Soit \( \Theta \colon \va \to \projd \) un plongement projectif et \( E \)
  un hyperplan d'équation \( L(X) = 0 \) dans ce plongement. Alors il existe
  un plongement \( \Theta' \colon \va \to \projd \) dans lequel \( E \) admet
  ...
  \begin{enumthm}
    ...
    \item \(
      ...
      \).
  \end{enumthm}
  De plus, si \( \Theta \) satisfait les hypothèses~\ref{h:vaemb}, alors c'est
  également le cas de \( \Theta' \) avec les mêmes valeurs de \( \dimp \) et
  \( N \) et
  \begin{enumthm}
    \item \(
        \htcmp[\Theta']
...
\begin{proof}
  Notons \( G \) l'hyperplan d'équation \( L \) dans \( \Qbar^{\dimp+1} \) et
  \( (b_0, \dots, b_\dimp) \) la base donnée par le lemme précédent appliqué à
  \( G \). Quitte à multiplier chacun des \( b_i \) par un scalaire
  convenable, ce qui ne change pas sa hauteur, on peut supposer qu'un de ses
  coordonnées est égale à \( 1 \), de sorte que \( \nv2{ b_i } \ge 1 \) en
  toute place.

  On note de plus \( M \) la matrice dont les colonnes sont les
  vecteurs \( b_i \) (écrits dans la base canonique de \( \Qbar^{\dimp+1} \)).
  Ainsi, si \( x \) a pour coordonnées \( C \) dans la base canonique, ses
  coordonnées dans la base \( (b_0, \dots, b_\dimp) \) sont \( M^{-1} C \).

  On note alors \( \chi \in \GL_{\dimp+1}(\Qbar) \) la transformation linéaire
  associée à \( M^{-1} \) (dans la base canonique) et on note de même la
  transformation linéaire de \( \projd \) qui en découle en passant au
  quotient. On pose alors \( \Theta' = \chi \circ \Theta \). On note
  \( L'(\vp) = L(M(\vp)) = L(b_0) \vp[0] \) ; par construction, \( L' \) est
  une équation de \( \divi \) dans ce nouveau plongement.

  Soit maintenant \( x \) un point de \( \va \), dont on note \( C_x \in
    \Qbar^{\dimp+1} \) les coordonnées dans le plongement \( \Theta \) et \(
    C'_x = M^{-1} C_x \) le coordonnées dans le plongement \( \Theta' \). On a
  alors
  \[
    \distv[\place, \Theta]{x}{\divi}
    =
