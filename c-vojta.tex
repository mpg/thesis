...
\begin{thm} \label{t:vojta-div}
  ...
  \begin{align}
    ...
    \\
    \Vcos & = \stuffout{\dots \text{$<1/9$ pour \ref{l:sum-wts}}}
    \\
...
\begin{proof}
  ...
  \begin{equation}
    ...
    \delta \wts[1] \Bigl(
      \bigl(
        4\puiss \cst{vs-ht}
        + 2 (\dimp + \puiss + \vdim) \cst{vs-deg-prod}^{-2}
      \bigr)
      \Lambda^{2\,f(u)}
      + 2 \hlclab + 4(\dimp+1)^2
    \Bigr)
...
Pour expliciter \( \ratfv1 \), on applique le lemme~\ref{l:par-var} à la
...
\begin{multline}
  ...
  \Biggr)
\end{multline}
puis en se souvenant que \( \sum_\place \degv \wtapx = 1 \) et que \( \sigma >
  4 \), en utilisant~\eqref{e:vadapt-ht} et en notant encore \( B \) un
majorant commun des \( \vdeg* \) :
\begin{equation}
...
\begin{align}
  ...
  \left(
    ...
    + 2 \bigl(
      \htcmp + \dimp^2 + (\dimp + 2) (\genre + 2) B^2
    \bigr)
  \right)
  \\ & \le
  \delta \wts[1] \left(
    \Lambda^{3f(\vdim)} \bigl(
      \cst{vs-ht} \cdot \cst{vs-deg-prod}^{-1}
      + 2(\dimp + 2)(\genre + 2) \cst{vs-deg-prod}^{-2}
    \bigr)
    + 2 \Lambda^{f(u)} (\htcmp + \dimp^2)
  \right)
\end{align}
...
\begin{equation}
  ...
  \eps
\end{equation}
ce qui découle des hypothèses faites sur les différents paramètres, par
exemple~\eqref{e:def-epsz}. Il ne
