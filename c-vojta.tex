...
\begin{proof}
  ...
  monômes unitaires (un argument similaire est d'ailleurs utilisé dans la
  preuve du lemme~4.2 de la référence citée).
\end{proof}
...
\begin{lem} \label{l:cramer}
  ...
  de valeur absolue inférieure ou égale à \( C \), et \( v \) une place de \(
    \Q \). Alors \( \nnv2{R^{-1}} \le r! \cdot C^{r-\dv} \) et \( \hautm[2]{R}
    \le r! \cdot C^{r-1} \).
\end{lem}
...
\begin{proof}
  ...
  que la hauteur de la comatrice est donnée par sa valeur absolue
  archimédienne.
\end{proof}

\begin{lem}
  Soit $\anyvar \colon \embedin \proj\dimp$ un sous-schéma fermé intègre, et
  $\chi \in \GL_{\dimp*}(\Qbar)$ donné par une matrice à coefficients entiers
  dans l'intervalle $[-B, B]$, où $B$ est un réel positif fixé, alors
  \begin{equation}
    \hautm[1]{\chi(\anyvar)}
    \le
    \hautm[1]{\anyvar}
...
\begin{scho} \label{s:plong-adapt}
  ...
  facteur \( \var* \). On notera enfin \( \pden** \) la dérivée de \(
    \poldep** \) par rapport à la dernière variable et \( \pden* =
    \prod_{\indi=\vdim+1}^\dimp \pden** \).
\end{scho}
