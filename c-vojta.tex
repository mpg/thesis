...
\( \ratf1 = F'' / \vmp[][\vdim]^\Dii \). Le but de cette section est alors de
...
préciser leur sens, qu'on espère suffisamment évident.) La définition de \(
  \ratf1 \) au début de cette section, celle de \( F'' \) au
...
\).
Il suffit dont d'estimer l'indice de \( \ratf2 \) ; nous allons commencer par
...
comme à la section~\ref{sec:wemb}, mais en choisissant cette fois une famille
...
application du lemme~\ref{l:indice-inversible} montre que \( \der[\dermp]
  \ratf2(\ex) \) est égal, pour tout \( \place \), à
\begin{equation}
  \der[\dermp]
    \frac{
      R(\vmp) \cdot F( \vmp, \wembcl[\place](\vmp) )
