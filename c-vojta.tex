...
Si \( G \) est une forme multihomogène, on défini \( \inda[\point] G =
  \inda[\point] G/H \) où \( H \) est n'importe quelle forme multihomogène de
même degré ne s'annulant pas en \( \point \). Le deuxième point du lemme
précédent montre que cette définition a bien un sens.

\medskip

On choisit dans l'atlas \( \clmaps^{\puiss-1} \) présenté en
sous-section~\ref{sec:wemb} une carte \( \clmap_\ex \) contenant \( \ex \), et
on considère la forme multihomogène \( F''_{\clmap_\ex} \) donnée par le
scolie~\ref{s:aux-co}. Le but de cette section est alors de montrer la
proposition suivante.

\begin{prop} \label{p:extra}
  On a \( \inda** F''_{\clmap_\ex} \ge \frac{ \epsi \delta }{ \sigma } \) avec
...
que par définition on a \( \inda** F''_{\clmap_\ex} = \inda** \ratf \).
...
La section précédente a montré que \( F''_{\clmap_\ex} \) était d'indice
...
\begin{fact}
  ...
  \begin{enumthm}
    \item \( G \) s'annulle en \( x \) avec un indice supérieur à \( \alpha \)
      par rapport à son degré \( b \) ;
    \item \(
...
Pour appliquer ce fait, nous devons fabriquer à partir de \( \fauxf**
...
\begin{fact}
  ...
  \begin{enumthm}
    \item \( \inda** G \ge \inda** \fauxf** \) ;
    \item \( \deg G = (\prod\fctrange \vdeg*) \deg \fauxf** \) ;
    \item \(
        \hautl[\infty] G
        \le
        (\prod\fctrange \vdeg*) \hautl[\infty]{ \fauxf** } + o(\delta)
      \).
  \end{enumthm}
\end{fact}
