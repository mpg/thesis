...
\begin{scho} \label{s:def-rfull}
  Comme \( \cdn[ \vmp ] \) est la somme directe de ses parties homogènes, les
  applications précédentes donnent lieu à une application linéaire \( \rfull
  \) de \( \cdn[ \vmp ] \) dans \( \cdn[ \vmp ]^{C'''} \) telle \( \rfull(H)
    \equiv RH \mod \varida \) et en particuler \( \ker \rfull = \varida \).

  De même, les familles d'applications \( \rdiv^\alpha \) et \( \relim^\alpha
  \) donnent lieu à des applications \( \rdiv \) et \( \relim \) définies sur
  tout \( \cdn[ \vmp ] \).
\end{scho}
...
\begin{scho}
  ...
  \begin{equation} \label{e:vanish*-def}
    ...
    \right\}
  \end{equation}
  défini en utilisant l'indice plus usuel, qui est évidemment aussi une
  valuation.
\end{scho}

\begin{rem}
  Si une forme \( H = \sum_\im \vmp[][0]^\im H_\im \) appartient à \(
    \vanishv[\beta] \), alors dans la somme précédente les termes correspondant
  aux \( \im \) tels que \( \wtsum*(\im) < \beta \) sont dans \( \varida \), et
  les autres sont dans \( \vanish[\beta] \) par définition. Il est donc clair
  que
  \begin{equation}
    \vanishv[\beta] \subset \vanish[\beta] + \varida
    \quad \forall \beta
    \pmm.
  \end{equation}
  ...
  \) de sorte que le plongement est bien adapté. Alors \( \vmp[1][\dimp] \in
    \vanish[1] + \varid \) mais \( \indav \vmp[1][\dimp] = 0 \).

  On pourrait être tenté d'utiliser l'appartenance à \( \vanish[\beta] + \varida
  \) pour définir une notion d'indice, mais il n'est pas clair que cette notion
  jouirait des propriétés précédentes, ni qu'elle se comparerait de façon
  agréable avec la notion définie précédemment. Nous n'explorerons donc pas plus
  avant cette seconde notion d'indice, dont nous n'avons mentionné l'existence
  qu'à titre de curiosité.
\end{rem}

Explicitons maintenant le lien entre l'indice et l'application de réduction
du scolie~\ref{s:def-rfull}.

\begin{lem}
  Pour tout forme multhomogène \( H \), on a \( \inda* \rfull(H) \ge \indav H
  \).
\end{lem}

\begin{proof}
  On se souvient que les applications \( \rdiv \) et \( \relim \), donc en
  définive \( \rfull \), sont définies par leur action sur les monômes en
  effectuant des substitutions sur les dernières variables et en gardant les
  premières, en particulier les \( \vmp*[0] \) inchangées. Cette observation
  et la linéarité de \( \rfull \) montre immédiatement que \( \rfull(H) =
    \sum_{\im \in \N^\puiss} \vmp[][0]^\im \, \rfull(H_\im) \), où \( H_\im \)
  est définit comme précédemment. Cette écriture de \( \rfull(H) \) n'est pas
  canonique dans le sens où chaque \( \rfull(H_\im) \) peut dépendre de \(
    \vmp[][0] \), mais elle est suffisante pour minorer l'indice. On observe
  alors que \( \rfull(H_\im) = 0 \) dès que \( H_\im \in \varida \) pour
  aboutir au résultat annoncé.
\end{proof}

Remarquons que la réciproque est fausse, même modulo une constante additive ne
dépendant pas du degré de \( H \). En effet, si \( \varida \) est engendré par
des relations de la forme \( (\vmp**)^\indi - (\vmp*[0])^\indi \) pour \( \ind
  > \vdim* \), il est clair que \( \rdiv \) et donc \( \rfull \) feront
croître l'indice de façon proportionnelle au degré de \( H \) en les dernières
variables.
...

\subsection{Stratégie de construction}
...
contrôlée, et d'indice élevé le long de \( \divi \) dans \( \var \). Plus
précisément, en conservant les notations précédentes et
...
\begin{equation}
  ...
  \cap \wemba^{-1}(\vanishv)
\end{equation}
non nulle modulo l'idéal \( \idealim \) de \( \wemb(\var) \), de hauteur
relativement petite.

Ce problème est un cas classique d'application du lemme de \TS. Pour l'espace
de départ, on fixe dans \( \cdn[\vmp, \vmpi]_\Dz \) un sous-espace vectoriel
supplémentaire de \( (\idealim)_\Dz \) engendré par des monômes, que l'on
notera \( \fspace \) et dans lequel on cherchera \( F \). La condition
précédente devient donc simplement \( F \neq 0 \).

Il est commode, pour appliquer le lemme de \TS, de présenter les conditions
sur \( F \) comme l'appartenance au noyau d'une certaine application linéaire.
Pour cela, on introduit, pour chaque \( \im \in \N^\puiss \), l'application
\begin{align}
  \partial_divi^\im \colon \cdn[\vmp] & \to \cdn[\vmp]
  \\
  H & \mapsto
  H_\im
  =
...
\begin{align}
  \partial_E^\stairs \colon \cdn[\vmp]
  & \to \bigoplus_{\im \in \stairs} \cdn[\vmp]
  \\
  H & \mapsto
  (\partial_\divi^\im H)_{\im \in \stairs}
  \pmm.
\end{align}
Par définition, il est clair que \( \vanishv = (\partial_\divi^\stairs)^{-1}
  (\bigoplus_{\im \in \stairs} \varida) \). Or, d'après le
scolie~\ref{s:def-rfull}, on a \( \varida = \ker \rfull \), donc \( \vanishv =
  \ker \sigma \) avec
\begin{align}
  \sigma \colon \cdn[\vmp]
  & \to \bigoplus_{\im \in \stairs} \cdn[\vmp]^{C'''}
  \\
  H & \mapsto
  (\rfull \circ \partial_\divi^\im H)_{\im \in \stairs}
  \pmm.
\end{align}
Plus précisément, compte tenu des conditions de degré on utilisera le fait que
\( \vanishv \cap \cdn[\vmp]_\Di = \ker \sigma_\Di \) où \( \sigma_\Di \) est
une restriction de \( \sigma \) dont l'espace d'arrivée est \( \bigoplus_{\im
    \in \stairs} \cdn[\vmp]^{C'''}_{\Dir - \im} \).

Au final, on appliquera le lemme de \TS à la matrice, dans les bases
monomiales canoniques, de l'application
\( \sigma_\Di \circ \wemba \) restreinte à \( \fspace \).  Pour pouvoir
procéder, nous devons donc :
\begin{enumerate}
  \item Minorer la dimension de \( \fspace \) et majorer celle de l'espace
    d'arrivée de \( \sigma_\Di \) en s'assurant que la seconde est plus petite
    que la première.
  \item Majorer la hauteur de la matrice considérée.
\end{enumerate}
Ces calculs font l'objet des deux sous-sections suivantes.  Comme \( \delta \)
est arbitrairement grand devant les autres paramètres, on n'explicitera à
chaque fois que le terme de plus haut degré en \( \delta \) ; ainsi lorsqu'on
utilisera la notation \( o(\truc) \) ou \( \sim \), il s'agira d'équivalents
quand \( \delta \) tend vers l'infini.


\subsection{Deux calculs de dimension} \label{sec:comp-dim}

La dimension de \( \fspace \), c'est-à-dire celle de \( \cdn[\vmp, \vmpi] /
  \idealim \),  est donnée par le théorème de \bsc{Hilbert} multihomogène dès
qu'on connaît les différents multidegrés de \( \wemb(\var) \). Il est \lat{a
  priori} difficile de tous les calculer, mais comme il suffit en fait de
minorer la dimension, le lemme suivant nous donne tout ce qu'on aura besoin de
savoir sur le degré.
...
Calculons maintenant la dimension de \( \bigoplus_{\im \in \stairs}
  \cdn[\vmp]^{C'''}_{\Dir - \im} \), l'espace d'arrivée de \( \sigma_\Di \).
Pour cela, on commence par calculer la dimension de chacun des facteurs.

\begin{lem}
  Pour tout \( \alpha \in \N^\puiss \) on a
  \( \dim \cdn[\vmp]^{C'''}_\alpha
    =
    \prod\fctrange \vdeg*
    \frac{\alpha_\fct^{\vdim*}}{\vdim*!}
    + o(\prod\fctrange \alpha_\fct^{\vdim*})
  \).
\end{lem}

\begin{proof}
  La base monomiale évidente de cet espace est en bijection avec l'ensemble
  d'indices suivant :
  \begin{equation}
    \Biggl\{
      ...
      \text{ tel que }
      \lgr{\imp*}
      = \alpha_\fct \quad \forall \fct
    \Biggr\}
    \pmm.
  \end{equation}
  Il est clair que les choix des différents \( \imp* \) sont indépendants
  entre eux. Il suffit donc de calculer le cardinal de chaque ensemble
  \begin{equation}
    \Biggl\{
      \imp* = (\imp*[0], \dots, \imp*[\vdim*+1])
      \in
      \N^{\vdim* + 1}
      \times \{ 0, \dots, \vdeg* - 1 \}
      \text{ tel que }
    ...
    \pmm.
  \end{equation}
  Il est clair que pour choisir un élément dans cet ensemble, il suffit de
  choisir une valeur de \( \imp*[\vdim+1] \) entre \( 0 \) et \( \vdeg* - 1
  \), puis de choisir les autre composantes de sorte que leur somme soit \(
    \alpha_\fct - \imp*[\vdim+1] \). Le cardinal de l'ensemble précédent est
  donc
  \begin{align}
    \sum_{\imp*[\vdim*+1] = 0}^{\vdeg*-1}
    \binom{ \alpha_\fct - \imp*[\vdim*+1] + \vdim* }{ \vdim* }
    & =
    \sum_{\imp*[\vdim*+1] = 0}^{\vdeg*-1}
    \frac{
      (\alpha_\fct - \imp*[\vdim*+1] + \vdim*)
      \cdots
      (\alpha_\fct - \imp*[\vdim*+1] + 1)
    }{
      \vdim*!
    }
    \\ & =
    \vdeg* \,
    \frac{ \alpha_\fct^{\vdim*} }{ \vdim*! } + o(\alpha_\fct^{\vdim*})
    \pmm.
  \end{align}
  Le résultat annoncé suit en prenant le produit sur \( \fct \).
\end{proof}

\begin{lem}
  On a
  \begin{equation}
    \dim \bigoplus_{\im \in \stairs} \cdn[\vmp]^{C'''}_{\Dir - \im}
    \le
    \pmm.
  \end{equation}
\end{lem}
...
\begin{prop} \label{p:build-aux}
  ...
  \( \delta \) est assez grand, il existe une forme \( F \in \cdn [\vmp,
    ...
    \idealim \), telle que \( R \cdot \wemba(F) \in \vanish + \varida \) et
  \begin{equation}
    \hautl[\infty] F
    \le
