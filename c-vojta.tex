...
\todo[\( \epsi \) défini plus tard, à remplacer ici après avoir vérifié si
ça vaut la peine de supprimer la dépendance en \( \dimp \)]
\todo[propager dans le théorème principal]
\begin{align}
  \label{e:def-Lambda}
  \Lambda
  & = \max \bigl(
  ...
  \\
  \newcst[\va]{vs-ht} & = \stuffout{\vaht, \max(\dimp^{\puiss^2}, \hlclab)}
\end{align}
...
\begin{proof}[\proofname\ du théorème~\ref{t:vojta-div}]
  ...
  \todo[Vérifier la normalisation de hauteur et reporter ci-dessous.]
  \begin{equation}
    \hautl{ \var*' }
    \le
  ...
  Appliquons le théorème de \bsc{Bézout} arithmétique qu'on vient
  de rappeler :
  \begin{align}
  ...
  certain \( \fct \) ; à ce stade on a \( \vdim \ge \puiss - 1 \). On en
  déduit alors
  \begin{alignat}{2}
    \wts* \hautn{\ex*}
    & \le 2\cst{vs-ht} \Lambda^{2\,f(\puiss-1)} \wts[1]
    &\quad& \text{par \eqref{e:varset-ht} et~\ref{f:comp-h-hn},}
    \\ \label{e:ct-Vbig-final}
    \wts[1] \hautn{\ex[1]}
    & \le 3\cst{vs-ht} \Lambda^{2\,f(\puiss-1)} \wts[1]
...
\todo[Tout réécrire en remplaçant \( \anydim \) par \( 0 \) et, dans les
...
utilisant successivement la proposition~\ref{p:extra} il vient :
\begin{equation}
  \indg b[x] G
  \ge
  ...
  \pmm.
\end{equation}
On utilise alors successivement~\eqref{e:cst-vs-deg-prod} puis
\eqref{e:varset-deg} et \eqref{e:varset-deg-prod} pour écrire
\begin{equation} \label{e:tp-ind}
  ...
  \alpha
\end{equation}
qui est précisément la première condition.
...
\begin{align}
  ...
  \right)
\end{align}
On utilise alors la définition de \( \Lambda \) pour écrire
\begin{equation}
  \puiss \ln \Lambda
  \ge
...
construction d'une forme auxiliaire, extrapolation et application d'un
théorème multiplicité. Cependant, cet ordre n'est pas celui dans lequel les
valeurs des différents paramètres sont déterminés ; la présente section a pour
but de clarifier la façon de choisir ces valeurs qui peuvent paraître,
sinon arbitraires, du moins mystérieuses au moment de leur introduction dans
...
proposition~\ref{p:varset-notmin} produire d'autres formes qui restent dans \(
  \varset(\ex) \). En fait, on peut voir ceci comme la construction d'une
suite de sous-variétés emboîtées, en estimant à chaque étape le degré et la
hauteur, qui dépendent des estimations à l'étape précédente.

En fait, on pourrait tout à fait dans un premier temps se concentrer
uniquement sur le degré, qu'on peut estimer indépendamment de la hauteur mais
qui en revanche détermine de façon cruciale les estimations de hauteur (c'est
d'ailleurs la seule raison pour laquelle on l'estime, en fin de compte). Lors
de la construction de la fonction auxiliaire, on se contenterait de savoir que
la borne de hauteur ne dépend pas de \( \ex \), ce qui permettrait ensuite de
...
stade on a seulement besoin que \( \epsz \) soit suffisamment grand devant \(
  \epsi \) pour que le rapport apparaissant dans~\eqref{e:ct-eps*-siegel} soit
inférieur à \( \frac12 \) ; notons que ce rapport ne dépend pas des degrés de
\( \var \), uniquement de ceux de la fonction auxiliaire. L'estimation de
hauteur en revanche dépend du maximum des degrés de \( \var \) et de ses
hauteurs.

On peut alors mener les calculs de la sous-section~\ref{sec:vojta-extrap-core}
et aboutir à~\eqref{e:prod-lem-final}, où la quantité qui y est notée \(
...
c'est-à-dire de ne pas dériver à un ordre trop élever. Le dernier terme enfin
est déterminé par la condition d'approximation~\eqref{e:Vapx} ; le seul moyen
de le rendre grand en valeur absolue et que l'indice de construction de la
...
suffisamment petit pour pouvoir extrapoler. Un calcul élémentaire montre qu'il
est possible de satisfaire ces deux conditions pour peu que \( \puiss > \max
  \vdim* = \genre \) (exposants apparaissant dans la première condition).
C'est uniquement pour répondre à cette exigence qu'on demande
\( \puiss > \genre \) dans l'énoncé du théorème ; de même les deux conditions
citées déterminent seules les définitions de \( \epsz \) et \( \epsi \)
données en~\eqref{e:def-epsz} et~\eqref{e:def-epsi}. (On utilisera souvent par
ailleurs le fait que \( \puiss \ge 2 \) et que \( \epsz < 1 \) pour simplifier
certaines estimations, mais aucune de ces inégalités n'est cruciale pour
l'aboutissement de la preuve.)

Maintenant que \( \epsi \) est fixé, on en déduit les valeurs souhaitables de
\( \Vcos \), déterminé par la seule condition~\eqref{e:Vcos-ct-extra} et de \(
  \sigma \), seulement déterminé par~\eqref{e:ct-sigma}.
...
c'est également ceci qui détermine sa valeur. Il est plus naturel d'absorber
