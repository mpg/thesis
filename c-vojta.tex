...
Nous allons maintenant expliciter \( \ratfv1 \). Posons d'acord
\( G_\place = \fauxv \cdot
  \prod\fctirange \nnv1{ \wembclp\clmapv\fcti }^{-\delta}
...
\begin{proof}
  ...
  scolie~\ref{s:aux-co}) et \( \dermp \). Le premier est immédiat d'après la
  définition~\eqref{e:def-ratfv12} de \( \ratfv1 \) et le fait que \(
    \deg_\fct \fauxv = \Dir* \).
  ...
  \begin{equation}
    ...
    \pmm.
  \end{equation}
  Pour le degré de \( P_\place \), on écrit alors
  \begin{align}
    \deg_\fct P_\place
    & =
  ...
  logarithme puis la somme pondérée par les degrés locaux sur toutes les
  places :
  \begin{align}
  ...
  \\ & \le
    \hautl[1]{ (G_\place)_\place }
    + 2 \sum\fctrange \lgr{\dermp*} \hautl[1]{ \varfc* }
    \\ & \qquad
    + \sum\fctrange
      \lgr{\dermp*} \bigl(
      ...
      + \genre (\Dir*) \ln 2
  \end{align}
  où l'on a utilisé~\eqref{e:nv-varfca}. Nous allons maintenant majorer
  indépendamment chacun des termes de cette somme. Le premier est donné
  par~\eqref{e:aux-co-htv}. Pour le deuxième, on écrit
  \begin{equation}
  ...
  \end{equation}
  en utilisant la remarque précédente que \( \dermp \) puis
  l'hypothèse~\eqref{e:varset-ht}.

  Enfin, pour le dernier terme\dots
\end{proof}
