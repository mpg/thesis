...
\begin{rem}
  ...
  tout plongement et donne des valeurs admissibles dans un certain nombre de
  cas, par exemple celui où \( \va \) est une courbe elliptique plongée à la
  \bsc{Weierstrass} ou bien le cas plus général où l'on utilisera un
  plongement de \bsc{Mumford} modifié.

  Dans tous les cas considérés, on aura \( \dimp \ge 2 \) et \( N \ge 3 \), et
  le degré de (l'image de) \( \va \) dans le plongement sera au moins \( 3 \).

  Par ailleurs, on peut toujours majorer \( \htcmp \) par \( 4 \hlclab \) en
  suivant la méthode utilisée dans~\cite{daphimhva2} pour déduire la
  proposition~3.9 (p.~665) du lemme~3.11, lequel correspond essentiellement à
  un cas particulier de l'hypothèse~\eqref{e:clab-loc}. On formule néanmoins
  les énoncés avec un \( \htcmp \) indépendant de \( \hlclab
  \) car dans certains cas on sait expliciter de la première constante mais
  pas de la seconde.
\end{rem}

On fixe désormais et jusqu'au début de la section~\ref{sec:vojta-coro} un
plongement projectif de \( \va \) et des constantes comme ci-dessus.  Le but
des prochaines sections est alors de prouver le théorème suivant, où les
notions de distance et de hauteurs découlent du plongement fixé.
