...
\begin{lem} \label{l:hmat-wemba}
  ...
  \begin{equation}
    ...
    \le
    \left(
      \bigl( \hmclab (\dimp+1)!^2 \bigr)^2 \,
      \prod\fctrange (\vdegp*) ^{2\dimp(\puiss-1)}
    \right) ^{\delta \wts[1]}
    \pmm,
  \end{equation}
...
\begin{proof}
  ...
  \begin{align}
    ...
    \\ & \le
    \prod\fctirange
    \bigr( \hmclab (\dimp+1)!^2 \bigr)^{\wtis* + \wts**} \,
    \bigr( \vdegp[\fcti] \bigl)^{2 \dimp \wts[\fcti]} \,
    \bigr( \vdegp[\puiss] \bigl)^{2 \dimp \wts[\puiss]}
  \end{align}
  Le résultat annoncé suit en invoquant le lemme~\ref{l:sum-wts} et en
  majorant largement \( \wt* \) par \( \wt[1] \) et \( 1 \) par \( \puiss-1 \).
\end{proof}
...
\begin{proof}
  ...
  \begin{equation}
    ...
    \Bigl(
      2 \bigl(
        \hlclab + 2(\dimp+1) \ln(\dimp+1)
      \bigr)
      +
      2 \dimp (\puiss-1) \sum\fctrange \ln \vdegp*
    \Bigr)
    \pmm,
  \end{equation}
