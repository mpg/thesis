...
\begin{thm} \label{t:vojta-div}
  ...
  il n'existe pas dans \( \va(\Qbar) \) de famille de points \( \ex[1],
...
\begin{coro} \label{c:vojta-div-dv}
  Soit \( \vaemb \colon \va \embedin \projd \) un plongement satisfaisant aux
  hypothèses~\ref{h:vaemb}, \( \divi \) un hyperplan de \( \projd \) et \( p_0
  \) le point de \( \projd \) de coordonnées homogènes \( (1 {:} 0 {:} \cdots
    {:} 0) \). Pour tout réel \( 0 < \eps < 1 \),
  il n'existe pas dans \( \va(\Qbar) \) de famille de points \( \ex[1],
    \dots, \ex[\puiss] \) avec \( \puiss \ge \genre + 1 \) satisfaisant
  simultanément aux conditions suivantes :
  \begin{align}
...
\begin{proof}
  On considère le plongement \( \vaemb' \) donné par le lemme précédent et on
  prévoit d'appliquer le théorème~\ref{t:vojta-div} à \( E \) et \( \vaemb'
  \) avec \( \expapx' = 85 \expapx / 86 \) pour une raison qui deviendra
  bientôt claire. Il s'agit de vérifier que les hypothèses du corollaire
  impliquent bien celles du théorème.
