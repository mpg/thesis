...
\begin{thm} \label{t:vojta-div}
  ...
  \begin{align}
    ...
    \\
    \newcst[\vaemb]{vs-ht}
    & = \max( \hautl1{ \chow\va }, \hlclab, \htcmp, n\log(n + 1) )
  \end{align}
\end{thm}
...
\begin{nota} \label{n:wt-choose}
  ...
  garantissent qu'aucun \( \ex* \) n'est de torsion) et \( \wt[1] \) assez
  grand\footnote{Ce qui sera utilisé à la section~\vref{sec:vojta-extrap-core},
    voir en particulier~\eqref{e:prod-lem-final} et le dernier paragraphe de
    la section citée.} (ce qui est possible car seuls les rapports entre les
  différents \( \wt* \) importent).
\end{nota}
...
\begin{proof}[\proofname\ du théorème~\vref{t:vojta-div}]
  ...
  notre cas (en notant \( \vdim* = \dim \var* \)) :
  \begin{align}
    \hautl1{ \chow{\var*'} }
    & \le
    ...
    + \deg T \Bigl(
      \hautl1{ \varfc* }
      + (\vdim* + 1) (\deg \var*) \log(n + 1)
    \Bigr)
    \\ & \quad
    + \vdim* (\deg \var*') \log(n + 1)
    \\ & \le
...
\begin{lem} \label{l:adapt-gen}
  Soit \( \iota \colon \anyvar \embedin \projd \) une sous-variété
  de degré \( \anydeg \), non contenue dans l'hyperplan d'équation \( \anyvp[0]
  ...
  que \( \anyvp[0] \) soit invariant par ce changement de coordonnées.
\end{lem}
...
\begin{proof}
  ...
  grâce au théorème fondamental de l'élimination (\eng{Elimilnation theorem},
  page~84 de \cite{phidg}). On peut donc choisir \( L_1,
...
Dans l'énoncé suivant, on rappelle que \( C' = C'_\vdeg \), comme indiqué
en-dessous de~\eqref{e:C-i-iii}.
...
\).\phantomsection\label{pg:clmaps-m}
On introduit alors pour chaque \( \fcti \) la famille de formes
...
\begin{equation} \label{e:def-epsz}
  ...
  \pmm,
\end{equation}
et enfin un entier \( \delta \) (qui sera choisi assez grand) tel que \(
...
\begin{proof}
  Dans le cas d'un monôme, on reprend point par point le début de la preuve
  ...
  majoration ne dépend pas de la carte utilisée ; on obtient ainsi
  \begin{equation}
    \nv1{ \wemba[\clmapv](\vmp^p \vmpi^q) }
    \le
    \prod\fctirange \left(
      \hmclab*^{\wtis* + \wts**} \,
      \nv1{ \vadapt[\fcti]^{-1} }^{2\wtis*} \,
    ...
    \right) ^\delta
  \end{equation}
  puis, \( F \) étant une combinaison linéraire de monômes :
  \begin{equation}
    \nv1{ \wemba[\clmapv](F) }
    \le
    \nv1 F
    ...
    \pmm.
  \end{equation}
  La suite de la preuve est inchangée à par un facteur \( \hautm1 F \)
  supplémentaire.
\end{proof}
...
polynôme de \bsc{Hilbert}-\bsc{Samuel} de \( \idealim \) en \( \Dz \).
...
\bsc{Hilbert}-\bsc{Samuel} multihomogène (voir~\cite[§ 3]{philz} par exemple)
dès qu'on connaît les différents multidegrés de \( \wemb(\var) \). Il est
\lat{a priori} difficile de tous les calculer, mais comme il suffit en fait de
...
\begin{proof}
  Il suffit d'appliquer le théorème de \bsc{Hilbert}-\bsc{Samuel}
  multihomogène en utilisant le lemme précédent, car une somme de nombre
  positifs est minorée par chacun de ses termes. Il vient
  \begin{align}
    \dim \fspace
    & =
    \Biggl(
...
\begin{align}
  ...
  2 \delta \wts[1]
  \bigl(
    \hlclab + 2\log((n+1)!) + 2n \log B
  \bigr)
  \\ & \qquad +
  \sum\fctrange 2\Di* \Bigl(
    \hautl1{ \varfc* }
    + \bigl( B(g+1) + 1 \bigr) \log B + \log 2
  \Bigr)
  + o(\delta)
  \pmm.
\end{align}
On remarque pour commencer que, comme \( \epsz < 1 \) on a \( d'_\fct = \wts*
  (2\wtw* + \epsz) \le 3 \wtw* \wts* \) ; le lemme~\vref{l:sum-wts} implique
alors que \( \sum_\fct d'_\fct \le 6 \wts[1] \). On utilise de plus la
définition~\eqref{e:cst-vs-ht} pour majorer
\( \hautl\infty{ M_{\sigma'_\clmap} } \) par
\begin{align}
  &
  4 \delta \wts[1]
  \Bigl(
    \cst{vs-ht}/2 + \log((n+1)!) + n \log B
    + 3 \log(B) \bigl( B(g+1) + 1 \bigr) + 3 \log 2
  \Bigr)
  \\ & \qquad
  ...
  \pmm.
\end{align}
On invoque alors~\eqref{e:B-siegel}\footnote{\label{fn:adhoc}%
  La section~\vref{sec:estimations} regroupe quelques estimations plus ou
  moins fastidieuses ou inintéressantes pour ne pas perturber le cours du
  texte ; elle est parfaitement indépendante des sections précédentes sauf
  bien sûr pour les définitions de différents paramètres en jeu.} pour
conclure :
...
\begin{fact} \label{f:siegel}
  ...
  \begin{equation}
    \hautl\infty x
    \le
    \frac p{q-p} \bigl( \hautl\infty M + \log q \bigr)
    + \frac q{q-p} c_\cdn
    \pmm,
  \end{equation}
...
\begin{proof}
  ...
  en remarquant de plus que \( \log q = o(\delta) \), on voit qu'il existe une
...
\begin{lem} \label{l:param-any-easy}
  Soit \( \derp = (\derp[1], \dots, \derp[\anydim]) \in \N^\anydim \) et
  posons \( \derp[0] = 0 \) ; on note \( \supp \derp \) l'ensemble des indices
  \( \alpha \) tels que \( \derp[\alpha] \neq 0 \). Alors, pour tout
...
\begin{proof}
  ...
  \) car \(
    ...
    = 0
  \).
  Ceci donne le premier cas ; pour le deuxième, si \( l \neq 0 \) on écrit
  \begin{align}
  ...
  \end{align}
  qui donne bien le résultat annoncé. Le cas \( l = 0 \) est direct.
\end{proof}
...
\begin{lem} \label{l:param-aff}
  ...
  \begin{enumthm}
    \item \( \der[\derp] \omega
        = \pi\left(
          \dfrac{ P_\alpha^\derp }{ R^{2\lgr\dermp - 1} }
        \right)
      \) ;
    ...
    \item \( \nv1{ P_\dv^\derp }
        ...
        \nv1 S ^{2\lgr\derp - 1}
        \cdot \left(
          (8\anydim)^{\lgr\derp} D^{3\lgr\derp -2}
        \right)^\dv \) où \( D = \max(\deg_\vaf S, \deg_\vafi S) \)
  \end{enumthm}
  où le dernier point est valable en toute place \( v \).
\end{lem}
...
\begin{proof}
  ...
  \begin{align}
    \deg_\vaf P^\derp
    & \le
    \deg_\vaf S + (2\lgr\derp - 2) \deg_\vaf S
    =
    (2\lgr\derp - 1) \deg_\vaf S
    \\
    \deg_\vafi P^\derp
    & \le
    \deg_\vafi S + (2\lgr\derp - 2) \deg_\vafi S
    =
...
\begin{lem} \label{l:param-any+1}
  Avec la définition précédente, pour tout \( \ind \in \set{0, \dots, \anydim}
  \), tout \( \derp \in \N^\anydim \minusset0 \) et \( \alpha \in \set{0, 1}
  \), il existe des formes homogènes \( P_{\ind, \alpha}^\derp \) telles que :
  \begin{enumthm}
    \item \(
        \der[\derp] \frac{ \vp[\anydim+1] }{ \vp* }
        =
        \frac{ P_{\ind, \alpha}^\derp }{ (\vp*^{\anydeg+1} Q)^{2\lgr\derp} }
      \) dans \( \cdn(\anyvar) \) ;
    \item \(
        \deg P_{\ind, \alpha}^\derp
        =
        4\anydeg \lgr\derp
      \) ;
    \item \(
...
\begin{proof}
  ...
  \begin{align}
    ...
    \\ & =
    \frac{
      \overline P_{\ind, \dv}^{\,\derp} (\vp[0], \dots, \vp[\anydim+1])
    }{
      \bigl(
  ...
  \end{align}
  où \( \overline P_{\ind, \dv}^{\,\derp} \) est l'homogénéisé de
  \( \tilde P_{\ind, \dv}^\derp \) par rapport à \( \vp[0] \vp* \), de degré
  \( 2\anydeg(2\lgr\derp-1) \),
  et \( P_{\ind, \dv}^\derp
    = \overline P_{\ind, \dv}^{\,\derp} \vp*^{\anydeg+1} Q \).
  ...
  \begin{align}
    ...
    \cdot \left(
      (8\anydim)^{\lgr\derp} \anydeg^{3\lgr\derp - 2}
    \right)^\dv
    \cdot
...
\begin{lem} \label{l:par-anyvar-mono}
  Soient \( \ip \in \N^{\anydim+2} \) et \( M_\ind^\ip \) comme ci-dessus,
  pour \( \ind \in \set{0, \dots, \anydim} \). En conservant les notations
  ci-dessus, pour tous \( \derp \in \N^\anydim \) et \( \alpha \in \set{0, 1}
  \), il existe des formes homogènes \( P^\derp_{\ip, \ind, \alpha} \) telles que
  \begin{enumthm}
    \item \( \der^\derp M_\ind^\ip
        = \frac{
          P^\derp_{\ip, \ind, \alpha}
        }{
          Q_\ind^{2\lgr\derp} \vp*^{2\lgr\derp(\anydeg+1) + \lgr\ip}
        }
      \) dans \( \cdn(\anyvar) \) ;
    \item \( \deg P^\derp_{\ip, \ind, \alpha}
        =
    ...
    \item \( \nv1{ P^\derp_{\ip, \ind, \dv} }
        ...
        \nv1{ \chow\anyvar }^{2\lgr\derp}
        \left(
          \bigl( 16 \anydim \anydeg^{3} \bigr) ^{ \lgr\derp }
          \cdot 2^{ \anydim \lgr\ip }
        \right)^\dv
      \) en toute place \( v \).
  \end{enumthm}
  De plus, \( \ord_{\vp[0]} \bigl( P^\derp_{\ip, \ind, \alpha} \bigr)
    \ge \ip[0] \).
\end{lem}
...
\begin{proof}
  ...
  \end{equation}
  pour tout \( \derp \), même nul. On pose enfin \( \indl[0] = 0 \) pour
  tout \( \indi \) ; en appliquant le lemme~\vref{l:param-any-easy}, il vient
  alors :
  \begin{align}
    \der^\derp M_\ind^\ip
    =
  ...
  \begin{align*}
    \sumset' = \Bigl\{
      ...
      &
      \text{ tel que }
      \gmp\nu[\indi, \alpha_\indi][\indi] = 0
      \\
      &
      \text{ et, pour tout \( (\indi, \alpha_\indi) \) avec \( l \le s \) : }
  ...
  \( \nv1 Q ^{2\lgr{\gmp\nu}} \) admet le même majorant que celui donné par le
  lemme en question pour \( \nv1{ P_{\ind, \dv}^{\gmp\nu} } \)
  pour tout \( \gmp\nu \).
  On remarque alors que l'ensemble de sommation \( \sumset' \)
  ...
  \(
    ...
    =
    \vp[0]^{\ip[0]}
  \),  ce qui prouve l'assertion sur l'ordre.
\end{proof}
...
\begin{lem} \label{l:par-var}
  ...
  multidegré \(
    ...
    \prod\fctrange \set{0, \dots, \vdim*} \). Alors, pour tous \( \dermp \in
    \prod\fctrange \N^{\vdim*} \) et \( \beta \in \set{0, 1} \), il existe une
  forme \( P_{G, \ind, \dv}^\dermp \) telle que
  \begin{enumthm}
    \item \(
        \der^\dermp g
        = \frac{
      ...
      \) dans \( \cdn(\var) \) ;
    \item \(
        \deg_\fct P^\dermp_{G, \ind, \beta}
        =
        \alpha_\fct + 4\vdeg* \lgr{\dermp*}
      \) pour tout \( \fct \), où \( \deg_\fct \) est le degré partiel en \(
        \vmp* \) ;
    \item \(
        ...
        \left(
          \bigl( 16 \vdim* \vdeg*^{3} \bigr) ^{ \lgr{\dermp*} }
          \cdot 2^{ \vdim* \alpha_\fct }
        \right)^\dv
      \) pour tout \( v \).
  \end{enumthm}
  De plus, \( \inda* \bigl( P^\dermp_{G, \ind, \beta} \bigr) \ge \inda*(G)
    \).
\end{lem}
...
\begin{proof}
  ...
  Seul le point sur l'indice reste à vérifier ; on peut clairement se ramener
  au cas où \( G = \vmp^\imp \) est un monôme. Considérons alors \(
    \vmp^{\gmp\nu} \) un monôme apparaissant dans \( P^\dermp_{G, \ind, \beta}
  \) : c'est-à-dire que \( (\vmp*)^{\gmp\nu*} \) apparaît dans \(
    P^\dermp_{\imp*, \ind, \beta}(\vmp*) \). D'après le lemme cité, on alors
  \( \gmp\nu*[0] \ge \imp*[0] \) d'où, en sommant, \( \wtsum*(\gmp\nu) \ge
    \wtsum*(\imp) \) qui est équivalent à l'estimation annoncée vu la
...
\begin{rem} \label{r:ind-basis}
  ...
  la base choisie, car seul compte l'ordre des opérateurs différentiels sur
...
sous-section~\oref{sec:wemb}, \vpageref{pg:clmaps-m} une carte \( \clmape \)
contenant \( \ex \), et on considère la forme multihomogène \( \faux** \)
donnée par le scolie~\vref{s:aux-co}. Le but de cette section est alors de
...
\begin{proof}
  ...
  \begin{equation}
    ...
    \cdot ( 2 \vdegp* )^{\vdeg* (\vdim* + 1) \dv}
  \end{equation}
  pour tout \( \ind \). D'après la note~\oref{fn:varfc} en bas
  de~\vpageref{fn:varfc} on a \( \nv1{ \varfc* } \ge 1 \), donc le résultat
  ...
  relations \( \poldep*[\ind] \) liant chacune des dernières coordonnées aux
  premières,  unitaires en la dernière variable (voir
  page~\pageref{p:def-poldep}). En décomposant \( \poldep*[\ind]
...
\end{align}
puis on utilise la définition de \( \Dir \) (voir~\eqref{e:d-dp-def}
\vpageref{e:d-dp-def} pour \( d' \) ; pour \( r \) il suffit de se souvenir
qu'il ne dépend pas de \( \delta \)) en scindant le premier facteur et
...
Pour \( p \in \set{1, 2, 3, 4} \), posons alors \( \ratfh{p} = \sum_v
  \degv \log \av{\ratfv{p}(\cexa)} \) où \( \degv \) désigne le degré local
...
\begin{align}
  ...
  \\ & \le
  \sum\fctrange \wts* \bigl(
    \hautn{ \ex* } + \htcmp + \frac12\log(n+1) + \hautl\infty{\vadapt*}
  \bigr)
  \pmm.
\end{align}
On remarque alors que \( \hautl\infty{\vadapt*} \le \log B \)
...
\begin{align}
  ...
  + \bigl( \htcmp + \frac12\log(n+1) + \log B \bigr) \sum\fctrange \wts*
  \\ & \le
    2 m \wts[1] \hautn{ \ex[1] }
    + 2 \wts[1] \bigl( \frac32\cst{vs-ht} + \log B \bigr)
  \pmm.
  \label{e:sum-h-cexa}
\end{align}
Au final on a donc
\begin{equation} \label{e:ratfh4}
  ...
  \le
  2 \epsz \delta \wts[1]
  \bigl( m \hautn{ \ex[1] } + \frac32\cst{vs-ht} + \log B \bigr)
  \pmm.
\end{equation}
...
\begin{equation} \label{e:ratfh2}
  \ratfh2 \le
  \delta \bigl( 2 \wts[1] (\cst{vs-ht} + 2 \log B) + o(\wts[1]) \bigr)
\end{equation}
...
\begin{proof}
  ...
  premier est immédiat d'après la définition~\eqref{e:def-ratfv1} de \(
    \ratfv1 \) et le fait que \( \deg_\fct \fauxv = \Dir* \). Le point sur
  l'indice est également clair.
  ...
  \begin{align}
    ...
    + \sum\fctrange \bigl(
      ...
      + \lgr{\dermp*} ( \log(16g) + 3 \log B )
      + g (\Dir*) \log 2
    \bigr)
  \\ & \le
    \hautl1{ (\fauxv)_v }
    + 2 \bigl( \sum\fctrange \lgr{\dermp*} \hautl1{ \varfc* } \bigr)
    + \delta g (\log 2) \sum\fctrange \wts* (2 \wtw* + \epsz)
    + o(\delta)
    \\ & \qquad
    + \bigl(
      2 g B ( \log B + \log( n + 1 ) )
  ...
  On écrit ensuite
  \begin{equation}
    g (\log 2) \sum\fctrange \wts* (2 \wtw* + \epsz)
    \le
    3 g (\log 2) \sum\fctrange \wts* \wtw*
    \le
    6 g (\log 2) \, \wts[1]
    \le
    \frac14 \Lambda \wts[1]
  ...
  lemme~\vref{l:sum-wts}. On invoque alors~\eqref{e:ht-Pv-adhoc}\footnote{Voir
    note~\oref{fn:adhoc} en bas de \vpageref{fn:adhoc}} pour majorer
...
\begin{equation}
  ...
  \le
  2 \wts[1] \bigl(
    2(m-1) \hautn{ \ex[1] } + \frac32\cst{vs-ht} + \log B
  \bigr)
\end{equation}
...
\begin{align}
  ...
  \\ & \le \label{e:ratfh1b}
  \delta \wts[1] \bigl(
    8(m-1) B \epsi \sigma^{-1} \hautn{\ex[1]} + \frac32\cst{vs-ht} + \log B
  \bigr)
  \pmm.
\end{align}
...
\begin{multline}
  \sum_v \degv \Biggl(
    - \frac12 \delta \wts[1] \hautn{\ex[1]} \wtapx \eps \epsi
    + \log \nv1{P_v}
    ...
    \biggl(
      ...
      + \wtapx\eps \htcmp
      + \log \nv1{ \varfc* }
      + (\vdeg* (\vdim* + 1) \dv) \log( 2 \vdegp* (n+1) )
    \biggl)
  \Biggr)
\end{multline}
...
\begin{align*}
  \ratfh{1'}
  & \le
  - \frac12 \delta \wts[1] \hautn{\ex[1]} \eps \epsi
  + 15 \delta \wts[1] \cst{vs-ht} \Lambda^{(1 + \frac1m) f(\vdim)}
...
\begin{align}
  ...
  + (\frac72 + 3\epsz) \cst{vs-ht} + (5 + 2\epsz) \log B
  \\ & \le
...
inférieur à \( - \frac14 \eps \epsi \). La définition~\eqref{e:def-epsz}
de \( \epsz \) assure que
...
\begin{equation} \label{e:ct-sigma}
  ...
  \le \frac{\eps \epsi}{16}
\end{equation}
et on a bien \( \alpha \le \frac3{16} \eps\epsi - \frac12 \eps\epsi \le -
  \frac14 \eps\epsi < 0 \) comme annoncé.
...
\begin{align} \label{e:Vbig-ct-extra}
  \alpha \hautn{\ex[1]} + \beta
  & \le
  - \frac14\eps \epsi \hautn{\ex[1]}
  + 20 \cst{vs-ht} \Lambda^{(1 + \frac1m) f(m)}
  \\ & \le
  - \frac14\eps \epsi \Bigl(
    \hautn{\ex[1]}
    - \frac{80}{\eps \epsi}
...
\begin{fact} \label{f:thm-prod}
  ...
  \begin{enumthm}
    ...
    \item \(
        \frac\alpha m < \left( \frac{\log(\vdim+1)}{2\vdim^2}\right)^\vdim
      \).
  \end{enumthm}
    ...
    \item la hauteur de \( T \) satisfait à
      \begin{align}
        ...
        \left(
          \hautl\infty G
          + \sum_{\fcti=1}^m \bigl(
            b_\fcti (\stoll{\vdim[\fcti]} + \log 2) + \sqrt{\vdim[\fcti]}
          \bigr)
          + \frac{\vdim-1}2 \log \lgr b
        \right)
        \\ & \qquad
        + b_\fct
        \left( \frac m\alpha \right)^\vdim (\vdim* + 1)
        \log \left( \left( \frac m\alpha \right)^\vdim (\vdim* + 1) \right)
        + b_\fct \log \binom{\deg T + \vdim*}{\vdim*}
      \end{align}
...
On considère à cet effet la projection linéaire \( \pi \) d'un ouvert dense de
...
l'anneau des coordonnées homogènes de \( \var \) est une extension finie de
\(
  \cdn[ \vmp[1][0], \dots, \vmp[1][{\vdim[1]}]; \dots;
...
\begin{equation}
  \left( \frac{\log(\vdim+1)}{2\vdim^2}\right)^\vdim
\end{equation}
...
\begin{equation} \label{e:tp-3}
  \left( \frac{\log(mg + 1)}{2(mg)^2}\right)^{mg}
  >
  (\sqrt2 mg)^{-2mg}
  \ge
  \Lambda^{-2}
...
choix de \( \alpha \), et en remarquant que \( \log \lgr b \) et \(
...
\begin{align}
  ...
  \\ & \qquad
  + D \delta \wts* (2\wtw* + \epsz)
  \log \binom{\Lambda^{2\vdim f(\vdim)} + \vdim*}{\vdim*}
  + o(\delta)
  \pmm.
\end{align}
...
\begin{equation}
  \log \binom{\Lambda^{2\vdim f(\vdim)} + \vdim*}{\vdim*}
  \le
  \vdim* \log(\Lambda^{2\vdim f(\vdim)} + 1)
  \le
...
\begin{align*}
  ...
  \Biggl(
    ...
    \\ & \quad
    + m \biggl(
      \sum_{\fcti=1}^m \wts[\fcti] (\stoll{\vdim[\fcti]} + \log 2)
      + \wts* \Bigl(
        2 \log \bigl( \Lambda^{2\vdim f(\vdim)} (\vdim* + 1) \bigr) + 1
      \Bigr)
    \biggr)
  \Biggr)
  + o(1)
...
\( \stoll{\vdim[\fcti]} \le m \log m \).  On utilise de plus le
lemme~\vref{l:sum-wts} et le fait que \( \wts* \le \wts[1] \) pour estimer le
terme de la dernière ligne ci-dessus par
\begin{align}
  2m \wts[1] \left(
    m \log m + \log 2 + 2\vdim \log \Lambda^{f(\vdim)}
    + \log m + \frac12
  \right)
\end{align}
puis on invoque~\eqref{e:ht-T-adhoc}\footnote{Voir note~\oref{fn:adhoc} en bas
  de \vpageref{fn:adhoc}} pour obtenir
\begin{align}
  \wtw* \wts* \hautl\infty T
  & \le
  \vdim* \Lambda^{2\vdim f(\vdim)}
...
\begin{align*}
  ...
  \hautl\infty{ T }
  + \left(
    \deg T \Bigl( \hautl\infty{ \vadapt* } + \log(n + 1) \Bigr)
    + \log \binom{ \deg T + n + 1 }{ n + 1 }
  \right)
  \\ & \le
  \hautl\infty{ T }
  + \left(
    \Lambda^{2\vdim f(\vdim)} \Bigl( \log B + \log(n + 1) \Bigr)
    + 2\vdim (n + 1) \log(\Lambda^{f(\vdim)})
  \right)
  \pmm.
\end{align*}
...
supposée contredire le théorème est fixée en premier, ainsi que la famille \(
  (\wt*)_\fct \) comme indiqué à la sous-section~\vref{sec:wt}. Plus
précisément, cette famille est fixée à un facteur entier près : on prendra
ensuite \( \wt[1] \) assez grand pour que dans~\eqref{e:prod-lem-final} la
partie en \( \delta \, o(\wts[1]) \), qui provient de~\eqref{e:ratfh3}
et~\eqref{e:ratfh2}, soit négligeable devant le terme en \( \delta \wts[1] \).
...
élémentaire \( \log x \le \alpha \, x^{1/\alpha} \), valable pour tout
\( \alpha > 0 \).
...
\begin{equation} \label{e:B-siegel}
  \frac{\cst{vs-ht}}2 + \log((n+1)!) + n \log B
  + 3 \log(B) \bigl( B(g+1) + 1 \bigr) + 3 \log 2
  \le
...
\begin{align}
  & \cst{vs-ht}/2 + \log((n+1)!)
  + 3 (\log B) B(g+1) + n \log B + 3\log B + 3 \log 2
  \\ & \qquad \le
...
\begin{equation}
  ...
  m^2 g \log( \sqrt2 m g )
  \ge
  m^2 \log m + \frac{ m^2 }2 \log 2
  \ge
  m \log m + \log m + \frac12 + \log 2
\end{equation}
...
est majorée par \( 3\vdim \log(\Lambda^{f(\vdim)}) \) et comme de plus \( \vdim
  \le mg \) le membre de gauche de~\eqref{e:ht-T-adhoc} est majoré
successivement par
\begin{equation}
  6 m^2 g \log(\Lambda^{f(\vdim)})
  \le
...
\begin{align}
  & 2 g B ( \log B + \log( n + 1 ) ) + \log(16g) + 3 \log B
  \\ & \qquad \le
...
\begin{lem} \label{l:plin}
  ...
  automorphisme \( \chi \) de \( \projd \) tel que \( \chi(\divi) \)
  admet \( \vp[0] = 0 \) comme équation et que, si l'on note \( \vaemb' = \chi
    \circ \vaemb \) le nouveau plongement \( \va \to \projd \) obtenu par
  composition, on a, pour tout \( x \in \va(\Qbar) \) :
  \begin{enumthm}
    \item \(
    ...
    \item \(
      ...
      \)
  \end{enumthm}
  où les deux derniers points signifient qu'il s'agit de valeurs acceptables
  pour pour ces constantes (les valeurs optimales pouvant être plus petites).
...
\begin{proof}
  ...
  Soit maintenant \( x \) un point de \( \va \), dont on note \( C_x \in
    \Qbar^{n+1} \) des coordonnées dans le plongement \( \vaemb \) et \(
    C'_x = M^{-1} C_x \) des coordonnées dans le plongement \( \vaemb' \). Vu
  ...
  \begin{equation}
    ...
    \le
    \htcmp[\vaemb] + \hautl2 L + \frac12 \log(n + 1)
  \end{equation}
...
\begin{proof}
  ...
  \(
    ...
    \ge
    85 ( \hautl{1, \vaemb}{E} + \frac12 \log (n + 1) )
  \)
...
\begin{proof}
  ...
  \begin{equation}
    \prod\placerange  \distv{p_i}{\divi}^\degv
    \le
    \prod_{v \in M_\cdn} \distv{p_i}{\divi}^\degv
    =
    \hautm2 \divi ^{-1}
  \end{equation}
...
\begin{lem} \label{l:norm-mn}
  ...
  Par ailleurs, on a \( \sum_v \degv \dpv \le 1.26 \adeg / \log \adeg
...
\begin{proof}
  ...
  en sommant sur \( i \) on a \( v_p( \binom\adeg\alpha ) \le n \log_p(\adeg)
  ...
  \( 1.25506 \adeg / \log \adeg \) d'après le corollaire~1 page~69
...
\begin{lem} \label{l:hs-vero}
  ...
  \begin{enumthm}
    ...
    \item \(
        ...
        + 2 \adeg^{g+1} (g + 1) (\deg_{\vaemb} \va) \log(n + 1)
...
\begin{proof}
  ...
  point~\vref{i:hs-vero-ht-va} en comparant les hauteurs utilisées grâce
  à~\eqref{e:ht-pph-1} :
  \begin{align}
    \hautl{1, \vaemb'} \va
    & \le
    \hautl{\htpph, \vaemb'} \va
    + (g + 1) (\deg_{\vaemb'} \va) \log(n' + 1)
    \\ & \le
    \adeg^{g+1} \hautl{\htpph, \vaemb} \va
    + \adeg^{g+1} (g + 1) (\deg_{\vaemb} \va) \log(n + 1)
    \\ & \le
    \adeg^{g+1} \hautl{1, \vaemb} \va
    + 2 \adeg^{g+1} (g + 1) (\deg_{\vaemb} \va) \log(n + 1)
    \pmm.
  ...
  \begin{equation}
    ...
    + \frac54\adeg \log(n + 1) + 1.26 \adeg n
  \end{equation}
  dont le dernier terme se déduit en remarquant que \( \frac54 \log(n +
    1) / n \le \frac58 \log 3 \le 0.69 \) car \( n \ge 2 \).
\end{proof}
...
\begin{proof}
  ...
  et
  \begin{equation}
    \htcmp[\vaemb'] + \hautl{2, \vaemb'} E + \frac12 \log(n' + 1)
    \le
    \adeg \htcmp[\vaemb]
...
\begin{proof}
  ...
  \begin{equation}
    \hautl1{ P_i }
    \le
    \hautl1 \avar + (\adim + 1) \adeg \log(\adeg + 1)
  \end{equation}
  ...
  nuls, tous deux entiers et de valeur absolue au plus
  \( \max(1, \adeg/2) \).
  ...
  car aucun des facteurs n'est nul et l'anneau ambiant est intègre.
  On peut donc choisir un point \( \lambda \in \Z^s \) avec \(
    \abs{\lambda_j} \le m/2 \) en lequel \( \mathcal P \), qui est homogène,
  ne s'annule pas (voir~\cite{remivds} avant la proposition~4.1 page~115).
  ...
  On pose alors \( Q = \lambda_1 P_1 + \dots + \lambda_s P_s \) et \( \hs =
    ...
    \hautl1{\lambda} \le \log s + \log(m/2) \), ce qui donne
  immédiatement
  \begin{equation}
    \hautl1 \hs
    \le
    \hautl1 \avar + (\adim + 1) \adeg \log(\adeg + 1)
    + (\adim+2)(n+1) \log (\adeg + 1)
    + \log(m/2)
  \end{equation}
...
\begin{coro} \label{c:vojta-gen2}
  Soient \( \vaemb \colon \va \embedin \projd \) un plongement projectif,
  \( \avar \) une variété de \( \projd \) de degré \( \adeg \) et de dimension
  \( \adim \), et \( 0 < \eps \) un nombre réel.  Il n'existe dans \(
    \va(\Qbar) \) aucune famille de points \( x_1, \dots, x_m \) avec \(
    m \ge g + 1 \) satisfaisant simultanément aux conditions suivantes :
  \begin{align}
    0 < \distv{x_i} \avar
...
\begin{proof}
  ...
  et dans le cas contraire on remarque qu'il s'agit d'un produit de \(
    mg - m + 1 \) facteurs tous inférieurs à \( 2mg \)
