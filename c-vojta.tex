    ...
    \item Il existe une famille de réels \( \hmclab* \) presque tous égaux à
      ...
      \begin{enumerate}
        ...
        \item pour tout couple de points \( (x, y) \in \va^2 \) et tout place
          ...
          \end{equation}
      \end{enumerate}
      On note de plus \( \hlclab = \ln \hmclab \).
  \end{enumthm}
\end{hypo}
...
que \( \card \clmaps^{\puiss-1} = (\dimp + 1)^{\puiss-1} \)\todo{\( \dimp+1
    \to \nclmaps \)} d'après les hypothèses~\ref{h:vaemb}, on a immédiatement
\begin{equation}
  \dim \ftarget
  \le
  \bigl(\delta^\vdim + o(\delta^\vdim)\bigr)
...
\begin{equation} \label{e:hmat-sigma-p}
  ...
  \pmm.
\end{equation}
Compte tenu des remarque précédentes, on a donc aussi
...
explicite jusqu'à obtenir la relation de récurrence~\eqref{e:vs-ht-rec} ; voir
néanmoins le quatrième point du scolie~\ref{s:aux-co} pour une simplification
de cet ordre.
...
\begin{scho} \label{s:aux-co}
  ...
  \begin{enumthm}
    ...
    \item \( \deg \faux* = \delta d' + o(\delta) \) avec
      \( d'_\fct = \wts* (2 \wtw* + \epsz) \) pour tout \( \fct \) ;
    \item \(
        \hautl[1]{\faux*}
        \le
...
\begin{proof}
  ...
  que \( \hautl[1]{ \faux* } \) est en fait majoré par la même quantité. Il ne
  reste qu'à appliquer l'hypothèse~\eqref{e:varset-ht} pour conclure.
\end{proof}
...
posons \( R_\ind = \poldep[][\anydim+1](1, \vaf[1], \dots, \vaf[\anydim],
  ...
  = \cdn(\anyvar) \) en prenant pour \( \vaf[\indi]' \) et \( \omega \) les
images
...
\end{align}
où \( \bar P_{\ind, \dv}^\derp \) est homogène de degré \( 2 (2\anydeg-1)
  (2\lgr\derp-1) \) en \( \vp[0], \dots, \vp[\anydim+1] \) et \( Q_\ind \)
homogène de degré \( 4\anydeg - 2 \) en ces mêmes variables, en remarquant que
\( \deg R = 2\anydeg \). On pose enfin \( P_{\ind, \dv}^\derp = \bar P_{\ind,
    \dv}^\derp \cdot \bar Q_\ind \) de sorte qu'on a l'énoncé suivant.
...
\begin{lem} \label{l:param-any+1}
  ...
  \begin{enumthm}
    \item \(
        \der[\derp] \frac{ \vp[\anydim+1] }{ \vp* }
        =
        \frac{ P_{\ind, \dv}^\derp }{ Q_\ind^{2\lgr\derp} }
      \) dans \( \cdn(\anyvar) \) ;
    \item \(
        \deg P_{\ind, \dv}^\derp
        =
...
\begin{lem} \label{l:par-var}
  ...
  \begin{enumthm}
    ...
    \item \(
        ...
        =
        \alpha_\fct + 4 (2\vdeg* - 1) \lgr{\dermp*}
      \) ;
    \item \(
        \nv1{ P^\derp_{G, \ind, \dv} }
        \le
...
\( \cexi* = \wembclg\clmape* (\cexa[\fcti], \cexa[\puiss]) \),
qui est un système de coordonnées multihomogènes de
\( \exi \) dans le plongement \( \vaemb^{\puiss-1} \).
...
On note alors \( \wembclv* = \wembclg\clmapv* \) où l'on rappelle que
\( \wembclm* \) est défini par~\eqref{e:def-wembclm}.  De même, on pose \(
  \fauxv = \faux[\clmapv] \).
...
Posons alors \( \ratfh{p} = \sum_\place \degv \ln \av{\ratfv{p}(\cexa)} \) où
\( \degv \) désigne le degré local en \( \place \) ; cette somme converge car
\( \ratfv p \) parcourt en fait un ensemble fini (de cardinal majoré par celui
de l'atlas \( \clmaps^{\puiss-1} \) choisi en sous-section~\ref{sec:wemb})
lorsque \( \place \) parcourt l'ensemble des places de \( \cdn \). La formule
du produit donne alors
...
\end{align}
où la dernière ligne est donnée directement par~\eqref{e:clab-loc}
compte tenu de la définition de \( \clmapv \) ci-dessus. Il ne reste à alors
plus qu'à prendre le logarithme dans l'estimation précédente, multiplier par
\( \delta \) sommer sur \( \place \) et sur \( \fcti \) pour avoir
\begin{equation} \label{e:ratfh2}
  \ratfh2 \le
  2 \delta \wts[1] \bigl( \cst{vs-ht} + \ln B \bigr)
\end{equation}
en utilisant de plus le lemme~\ref{l:sum-wts}.

\medskip

Nous allons maintenant expliciter \( \ratfv1 \) : pour chaque place \( \place
\), on applique le lemme~\ref{l:par-var} avec \( G = \fauxv \cdot
  \prod\fctirange \nnv1{ \wembclp\clmapv\fcti }^{-\delta}
\) et \( \ind =
  \indv \), et on note \( P_\place = P_{\fauxv, \indv, \dv}^\derp \) la forme
obtenue pour l'indice \( \derp \) fixé plus haut (plus précisément, celle des deux formes
obtenues qui correspond à la place considérée).  Remarquons que \( P_\place \)
parcourt un ensemble fini quand \( \place \) varie, car c'est déjà le cas de
\( \wembclv \), donc de \( \fauxv \). Le \( P_\place \) jouissent alors des
...
\begin{proof}
  ...
  tenant compte des informations connues sur \( \fauxv \) et \( \dermp \). Le
  premier est immédiat car \( \deg_\fct \fauxv = \Dir* \).

  Rappelons l'hypothèse faite que \( \dermp \) : on suppose que \(
    \wtsum(\dermp) \le \epsi \delta / \sigma \). D'une part, en remarquant que
  \( \inda*(\fauxv) \ge \epsi \delta \) par construction
  (scolie~\ref{s:aux-co})  et que \( \wtsum*(\dermp) \le \wtsum(\dermp) \) par
  définition, on a
  \begin{equation} \label{e:ind-Pv}
    ...
    \frac34 \epsi \delta
  \end{equation}
  en utilisant la minoration large mais suffisante \( \sigma > 4 \). D'autre
  part, cette hypoithèse implique que pour tout \( \fct \) on a
  \begin{equation}
