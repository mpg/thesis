...
\begin{equation}
  \begin{aligned}
    \pmor \colon \cdn(\anyvar)
    & \to \cdn(\anyvar)\series\psp
    \\
    f
    & \mapsto
...
rationnelle \( f = G/\vp[\anydim] \), nous exhiberons explicitement (au moins)
un polynôme \( R_\derp \) tel que \( R_\derp \cdot \pdiff*^\derp(f) \) soit un
polynôme, de degré et normes locales contrôlées. Nous ne considérons que
certains numérateurs \( G \) ayant une forme particulière ; bien qu'il soit
possible d'obtenir des énoncés plus généraux (pour tout fraction rationnelle
si l'on veut), le fait de se restreindre au cas particulier qui nous intéresse
simplifie un peu les preuves et permet d'obtenir des majorations plus fines.

\begin{rem} \label{r:param-deshom}
  Ci-dessus on a choisi d'utiliser \( \vp[\anydim] \) comme variable de
  déshomogénéisation ; on aurait aussi bien pu utiliser \( \vp* \) pour \(
    \ind \in \set{0, \dots, \anydim} \) à la place. On obtiendrait alors une
  base différente de l'espace des dérivations, donc un morphisme \( \pmor \)
  différent, qu'on pourrait également chercher à représenter par un autre
  morphisme \( \pmor* \).

  On pourrait donc écrire chacun des lemmes qui vont suivre en
  déshomogénéisant par rapport à une variable d'indice quelconque dans \(
    \set{0, \dots, \anydim} \), néanmoins pour simplifier les notations nous
  utiliserons toujours \( \vp[\anydim] \) ; il est clair que l'énoncé général
  se déduit de ce cas par permutation des variables.
\end{rem}
...
(car leur terme constant l'est), ce qui permet d'étendre le morphisme à \(
  \cdn[X]_{(\ideal\anyvar)} \). Pour tout \( \ind \in \set{0, \dots,
    \anydim-1} \), on peut poser \( \pmor*_\dv(\vp*/\vp[\anydim]) =
  \vp*/\vp[\anydim] + \psp* \). Pour \( \ind > \anydim \) on peut exploiter
les relations de dépendance intégrales données par le plongement adapté, comme
dans le lemme suivant, qu'on énonce dans un cadre affine pour commencer.
...
\begin{lem} \label{l:param-aff}
  ...
  qui laisse stable les \( \vaf* \) et envoie \( \vafi \) sur \( \omega \).
  Soit \( R \in \ker \pi \) tel que \( \diff_\vafi R \notin \ker
    \pi \).
...
\begin{proof}
  ...
  \begin{equation}
    ...
    \pmm.
  \end{equation}
  On remplace alors \( R \) par son développement de \bsc{Taylor}, pour
  obtenir les égalités suivantes modulo \( R \) :
  \begin{align}
    0
    & =
  ...
  qui assure bien sa nullité modulo \( R \).
...
Nous sommes maintenant en mesure de définir les morphismes \( \pmor*_\dv \) de
la façon suivante : fixons un \( \ind > \anydim \) et notons \( \tilde\Pi =
  P_\ind(\anyvp[1], \dots, \anyvp[\anydim], 1, Y ) \), où \( P_\ind \) est
donné par le fait~\ref{f:plong-adapt-dep}, puis \( \Pi \) une des dérivées
successives de \( \tilde\Pi \) par rapport à la dernière variable, telle que
\( \diff_Y \Pi \notin \ideal\anyvar \). On pose alors, pour \( \derp \neq 0
\) :
\begin{equation}
  ...
  \pmm,
\end{equation}
où les \( P_\dv^\derp \) sont donnés par le lemme précédent. On étend alors
les morphismes \( \pmor*_\dv \) à \( \cdn[X]_{(\ideal\anyvar)} \) comme
expliqué au deuxième paragraphe précédant le lemme~\ref{l:param-aff}. Ainsi,
\( \pmor*_\dv \) et donc les différents \( \pdiff^\derp \) sont désormais bien
définis.

En pratique, pour l'application considérée (représenter les dérivées d'un
déshomogénéisé de \( \faux* \)) nous n'avons pas besoin d'étudier les images
par \( \pmor* \) des \( \anyvp* / \anyvp[\anydim] \) pour \( \ind > \anydim +
  1 \). Ceci nous permet d'obtenir des estimations légèrement plus précises
car il n'y a qu'une variable liée, dont on connaît en fait le polynôme
minimal.

\begin{lem} \label{l:pmor-dim+1}
  Notons \( \poldep[][\anydim+1] \) le polynôme donné par l'application du
  fait~\ref{f:plong-adapt-dep} à \( \anydim \) et \( \pden \) sa dérivée par
  rapport à la dernière variable ; alors :
  \begin{enumthm}
    \item \( P^\derp_{\anydim+1, \dv}
        = \pdiff^\derp(\frac{\vp[\anydim+1]}{\vp[\anydim]})
        \cdot \pden^{2\lgr\derp} \) est un polynôme ;
    \item \( \deg P^\derp_{\anydim+1, \dv}
        = 2 (\anydeg - 1) \lgr\derp \) ;
    \item \( \nv1{ P^\derp_{\anydim+1, \dv} }
        \le
        \nv1{ \chow\anyvar }^{2 \lgr\derp} \, \bigl(
          4 \anydim \anydeg^{3} \bigr) ^{ \lgr\derp \dv }
      \).
  \end{enumthm}
\end{lem}

\begin{proof}
  D'après le deuxième point de la définition~\ref{d:plong-adapt}, \(
    \vp[\anydim+1] \) est de degré \( \anydeg \) sur les autres variables, de
  sorte que \( \poldep[][\anydim+1] \) est son polynôme minimal (homogène).
  Ainsi, dans les notations des paragraphes ci-dessus, on peut prendre \(
    \tilde\Pi = \Pi \) et appliquer le lemme précédent directement. On a ainsi
  \begin{equation}
    \pdiff^\derp \left( \frac{ \vp[\anydim+1] }{ \vp[\anydim] } \right)
    =
    \frac {
      P_\dv^\derp ( \vp[0], \dots, \vp[\anydim-1], \vp[\anydim+1] )
    }{
      \pden ( \vp[0], \dots, \vp[\anydim-1], \vp[\anydim+1] )
      ^{2\lgr\derp - 1}
    }
    =
    \frac {
  ...
  \end{equation}
  par homogénéité.  On pose finalement\footnote{Ceci n'a pour but que de
    simplifier les calculs ultérieurs en rendant le degré exactement linéaire
    en \( \lgr\derp \).} \( P_{\ind, \dv}^\derp = \pden \cdot P_\dv^\derp \).
  Les estimations de degré et de norme annoncées sont alors immédiates.
\end{proof}
...
monômes en \( \vp / \vp[\anydim] \). En fait, pour les applications
considérées, nous pouvons même nous restreindre aux monômes de la forme
\begin{equation}
  M_\ip
  =
  \prod_{\ind=0}^{\anydim+1}
  \left( \frac{\vp*}{\vp[\anydim]} \right)^{\ip*}
  \quad
  \text{avec \( \ip[\anydim + 1] < \deg \anyvar \).}
\end{equation}

% XXX

\begin{lem} \label{l:par-anyvar-mono}
  Soit \( \ip \in \N^{\dimp+1} \) et \( M_\ip \) comme ci-dessus.  Posons \(
    \pden = \prod_{\ind = \anydim + 1}^{\dimp} \pden_\ind \) où les \(
    \pden_\ind \) sont donnés par le lemme précédent. Alors :
  \begin{enumthm}
    \item \( P^\derp_{\ip, \dv}
        = \pdiff^\derp(M_\ip)
