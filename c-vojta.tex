...
Remarquons que dans l'énoncé précédent, \( \divi \) n'est pas une sous-variété
de \( \va \). On pourrait être tenté de remplacer \( \divi \) par \( \divi
  \cap \va \) dans la condition~\eqref{e:Vapx}, mais c'est l'inégalité de
gauche (condition \( \ex* \not\in \divi \)) qui pose problème. Ceci n'est en
fait pas gênant car, à la fin du chapitre, nous en déduirons un énoncé
similaire en remplaçant \( \divi \) par n'importe quelle sous-variété de \(
  \projd \), donc en particulier par n'importe quelle sous-variété de \( \va
\).
...
\begin{proof}[\proofname\ du théorème~\vref{t:vojta-div}]
  ...
  \). Rappelons comment \( \Hautl\htpph \) se compare à \( \Hautl1 \) dans le
  cas particulier de la forme de \bsc{Chow} \( \chow V \) d'une variété
  projective \( V \) :
...
ceci a en revanche un coût, qui consiste à remplacer \( \eps \) par \(
  \eps / \min(n + 1, \card \placesapx) \) dans les autres hypothèses, comme
nous le verrons au cours de la preuve.

\begin{coro} \label{c:vojta-div-any-s}
  Soient \( \vaemb \colon \va \embedin \projd \) un plongement projectif,
  ...
  \begin{align}
    ...
    \right)^{ \frac{m}{m-g} }
  \end{align}
  avec \( \cst{ht-v-div-dv} \) comme en~\eqref{e:cst-ht-v-div-dv}, \( s =
    \min(n+1, \card \placesapx) \) et
  \begin{align}
    \Lambda_2
    & = \max \bigl(
      (\deg \va)^m,
...
Une autre approche consiste à ne pas se débarasser du facteur \(
  \distv{p_0}\divi \) pour l'instant en prévision du fait que l'on déduira en
section~\vref{sec:ha-prod} des énoncés où l'on prendra le produit sur toutes
les places de \( \placesapx \), et où ce facteur disparaîtra plus
naturellement. Cependant, pour simplifier l'énoncé des corollaires suivants où
l'on changera de plongement, on utilisera l'énoncé suivant.

\begin{coro} \label{c:vojta-div-any-p}
  Soient \( \vaemb \colon \va \embedin \projd \) un plongement projectif,
  \( \divi \) un hyperplan de \( \projd \) et \( \eps > 0 \)
  un nombre réel.  Il n'existe dans \( \va(\Qbar) \) aucune famille de points
  \( x_1, \dots, x_m \) avec \( m \ge g + 1 \) satisfaisant
  simultanément aux conditions suivantes, où les notions de distance et de
  hauteur sont relatives au plongement \( \vaemb \) :
  \begin{align}
    ...
    \right)^{ \frac{m}{m-g} }
  \end{align}
  avec \( \Lambda_1 \) et \( \newcst[\Theta]{ht-v-div-dv} \) comme au
  corollaire~\vref{c:vojta-div-any-s} et \( (\alpha_v)\placerange \) une
  certaine famille de réels supérieurs ou égaux à \( 1 \), satisfaisant
  \( \prod\placerange \alpha_v \le \hautm2 \divi ^2 \cdot \sqrt{n+1} \).
\end{coro}

\begin{proof}
  En reprenant les notations de la preuve du corollaire précédent, on chosit
  cette fois-ci un \( i \) tel que \( p_i \not\in \divi \), ce qui est
  toujours possible. On pose alors
  \begin{equation}
    \alpha_v
    =
  ...
  \begin{equation}
    ...
    \hautm2 \divi ^{-1}
  \end{equation}
  vu l'expression de la distance à une hyperplan.
\end{proof}

Pour éviter de dupliquer les corollaires des sections suivantes, on regroupe
les deux énoncés précédents en un seul, où l'on rappelle de plus les valeurs
de toutes les constantes pour en faciliter l'usage.
...
\begin{coro} \label{c:vojta-gen1}
  ...
  \begin{align}
    ...
    \\
    \hautn{x_1}
    & > \frac4\adeg \cst{ht-v-gen1} \Lambda_3^{(1 + \frac1m)
      \prod_{i=m}^{mg} (2i + 1)}
    \\
    \hautn{x_i} & > \hautn{x_{i-1}} \cdot
    4 m \Lambda_3^{2m \prod_{i=m+1}^{mg} (2i + 1)}
