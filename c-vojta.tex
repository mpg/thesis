...
fait~\ref{f:thm-prod} à la forme \( G \) ainsi obtenue, avec \( \alpha =
  \puiss \Lambda^{-2f(u)} \) et \( b = \deg G = \bigl( D \delta \wts* (2\wtw*
    + \epsz) \bigr)_\fct \). Vérifions que les hypothèses sont bien
satisfaites.  Commençons avec l'indice : pour tout \( \imp \in
  (\N^{\dimp+1})^\puiss \) on a
\begin{align}
...
deuxième condition est satisfaite.
