...
\begin{proof}
  ...
  \begin{equation} \label{e:plin-norm}
    \nv\infty{ C _x } \le \nv2{ L } \nv2{ C'_x }
    \quad\text{et}\quad
    \nv\infty{ C'_x } \le \nv2{ L } \nv2{ C _x }
  \end{equation}
  et la deuxième comparaison donne aussitôt le point~\ref{i:plin-ht-pt}.
  ...
  avec (la transposée de) \( M^{-1} \sigma \) sur chacun des \( \genre + 1 \)
  ...
  \begin{equation}
    ...
    \le
    \htcmp[\vaemb] + \hautl[2] L + \sqrt{\dimp+1}
  \end{equation}
  ...
  Pour la deuxième, on écrit de même
  \begin{align}
    \frac{
...
avec une hypothèse d'approximation modifiée, point sur lequel nous
reviendrons. Le résultat étant énoncé ici sous une forme un peu plus compacte,
précisons pour la commodité du lecteur que les seuls autres changements sont
le remplacement de \( \cst{vs-ht} \) par \( \cst{ht-v-div-dv} \) avec une
définition différente faisant intervenir la hauteur de \( E \), ainsi que le
remplacement de \( 85 \) par \( 86 \) partout où ce nombre apparaît en même
temps que \( \expapx \).
...
est proche de \( E \), jusqu'à obtenir un hypothèse impossible à satisfaire
(donc un résultat vide) lorsque \( p_0 \). Le corollaire suivant évite ce
problème en considérant des permutations de coordonnées ; ceci a en revanche
un coût, qui consiste à remplacer \( \expapx \) par \( \expapx / (\dimp + 1)
\) dans les autres hypothèses, comme nous le verrons au cours de la preuve.
...
\begin{proof}
  ...
  ces ensemble le nombre réel \( \sigma_i = \sum_{\place \in \placesapx_i}
    \wtapx \degv \) où \( \degv \) désigne comme d'habitude le dégré local en
  \( \place \) divisé par le degré global. Par hypothèse, on a
  \( 1 = \sum\placerange \wtapx \degv \ge \sum_{i=0}^\dimp \sigma_i \)
  et il exite donc (au moins) un indice \( i \) tel que \( \sigma_i \ge
    1/(\dimp + 1) \). On fixe un tel indice, désormais désigné par \( i \).

  On note alors \( \placesapx' = \placesapx_i \) et, pour tout \( \place \in
    \placesapx' \), on pose \( \wtapx' = \wtapx / \sigma_i \). Enfin, on pose
  ...
  appliquer le corollaire~\ref{c:vojta-div-dv} avec le plongement \( \vaemb' \) le réel \(
    \expapx' \), l'ensemble de places \( \placesapx_i \) et les poids \(
    \wtapx' \), car ses hypothèses sont impliquées par celle du présent
  corrolaire compte tenu de la comparaison ci-dessu et du fait que \( \expapx'
    \ge \expapx / (\dimp + 1) \).
\end{proof}
...
\begin{lem} \label{l:hs-vero}
  ...
  le plongement de \bsc{Veronese} modifié de degré \( \delta \), qui va donc
  ...
  \begin{enumthm}
    \item \(
      ...
      \) ;
    \item \(
        \distpv[\place, \vaemb']{x}{\divi}
        \le
        \distpv[\place, \vaemb]{x}{F}
        \cdot \todots
      \) ; \label{i:hs-vero-dv}
    \item \(
        \hautm[2, \vaemb']{x}
...
\begin{proof}
  ...
  indices et les plongements de \bsc{Veronese} modifiés. Pour le deuxième
  ...
  modifié) puis on effectue un développement limité\todo.
