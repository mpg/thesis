...
\begin{lem}
  Soit \( \vaemb \colon \va \to \projd \) un plongement projectif et \( \divi
  \) un hyperplan d'équation \( L(\vp) = 0 \) dans ce plongement, où \( L \)
  est normalisée de sorte qu'un de ses coefficients soit égal à \( 1 \).
  Alors il existe un plongement \( \vaemb' \colon \va \to \projd \) dans
  lequel \( \divi \) admet \( \vp[0] = 0 \) comme équation et tel que :
  \begin{enumthm}
    \item \(
...
\begin{proof}
  Par hypothèse, un des coefficients de \( L \) vaut \( 1 \). Notons \( \sigma
  \) la transformation de \( \projd \) donnée par l'échange de la coordonnée
  correspondante avec \( \vp[0] \). Cette transformation ne change clairement ni
  les distances, ni les hauteurs. Par ailleurs, une équation de \( \divi \) dans
  le plongement \( \sigma \circ \Theta \) est \( L_\sigma(\vp) =
    L(\sigma^{-1}(\vp)) = \vp[0] + l_1 \vp[1] + \dots + l_\dimp \vp[\dimp] \).
  Posons alors
  \begin{equation}
    M = \begin{pmatrix}
  ...
  coordonnées homogènes (dans le plongement \( \sigma \circ \Theta \)) de
  ...
  quotient. On pose alors \( \Theta' = \chi \circ \sigma \circ \Theta \). On
  note \( L'(\vp) = L_\sigma(M(\vp)) = \vp[0] \) ; par construction, \( L' \)
  est une équation de \( \divi \) dans ce nouveau plongement, qui satisfait
  donc à la première condition.
  ...
  conditions sont satisfaites avec \( \hmclab[\vaemb'] = \nv1 L ^{5/2}
    \hmclab[\vaemb] \) comme annoncé.
