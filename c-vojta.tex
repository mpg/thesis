...
\begin{equation} \label{e:def-epsz}
  \frac{\eps \eps}{33\puiss}
  \le \epsz \le
  \frac{\eps \eps}{32\puiss}
  \pmm,
\end{equation}
...
\begin{align}
  ...
  + o(1)
\end{align}
puis on remarque que la définition \eqref{e:def-epsi} de \( \epsi \) signifie
précisément que le premier facteur vaut \( 1 \) ; en utilisant alors le fait
que \( \puiss \ge \genre + 1 \ge 2 \), il vient enfin
...
\begin{scho} \label{s:aux-co}
  ...
  \begin{enumthm}
    \item \( F''_\clmap = R \cdot F(\vmp, \wembcl_\clmap(\vmp)) \mod \varida
      \) ;
...
\begin{proof}
  Les trois premiers points ne sont qu'un rappel de la construction, en se
  souvenant du lemme~\ref{l:rfull}, de la définition de \( \wemba^\clmap \) et
  de ses propriétés (sous-section~\ref{sec:wemb}). Le dernier point découle
  des propriétés classiques de la norme ainsi que du lemme~\ref{l:rfull}.
\end{proof}
