...
mais pas leur produit. Rappelons qu'on note \( \degv \) le degré local en \(
  \place \) et posons \( \hautl{\ratf{p}(\cexa)} = \sum_\place \degv
  \av{\ratfv{p}(\cexa)} \) ; il est alors clair que \( \hautl{ \der[\dermp]
    \ratf2(\ex) } = \sum_{p=3}^5 \hautl{\ratf{p}(\cexa)} \).% XXX
