...
\begin{lem} \label{l:par-anyvar-mono}
  Soient \( \ip \in \N^{\anydim+1} \) et \( M_\ip \) comme ci-dessus, pour \(
    \ind \in \set{0, \dots, \anydim} \). En conservant les notations
...
\begin{proof}
  ...
  \begin{equation}
    ...
    \sum_{\gmp\nu \in N}
    \prod_{\indi=0}^{\anydim+1}
    \prod_{ \alpha_\indi = 1 }^{ \ip[\indi] } \left(
      \der^\indl \biggl( \frac{ \vp[\indi] }{ \vp* } \biggr)
      \pden_\ind^{2\lgr\indl}
      \vp* ^{\lgr\indl + 1}
    \right)
  \end{equation}
  dont on vérifie facilement que c'est bien un polynôme :
  pour chaque facteur non nul de chaque terme, si \( \indl = 0
  \) alors \(
    \der^\indl ( \frac{ \vp* }{ \vp[\anydim] } )
    \vp*
  \) est un polynôme, sinon si \( \indi \le \anydim \) on sait que \(
    \der^\indl ( \frac{ \vp* }{ \vp[\anydim] } )
    \vp* ^{\lgr\indl + 1}
  \)
  est un polynôme, et sinon
  \(
    \der^\indl ( \frac{ \vp* }{ \vp[\anydim] } )
    \pden_\ind^{2\lgr\indl}
  \) en est un, d'après les paragraphes précédents. Notons qu'on choisit \(
    2\lgr\indl \) comme exposant du dénominateur, bien qu'il soit légèrement
  sous-optimal, pour simplifer la sommation.
