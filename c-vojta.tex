...
= \deg \var*$, et $\varfc*$ une\footnote{\label{fn:varfc}Cette forme est
  unique à multiplication par un scalaire près ; au besoin on la supposera
  normalisée de sorte que toutes ses normes locales soient au moins \( 1 \).}
forme de \bsc{Chow} de $\var*$. On note enfin $\varid$ l'idéal multihomogène
saturé de $\var$ et $\varid*$ ceux de ses facteurs.
...
\(
  \wembcl*
  = L\pexp{\wti*, \wti**}
  [{\vadapt[\fcti](\ex[\fcti]), \vadapt**(\ex**), \place}]
\)
...
de\footnote{\label{fn:coord-deshom}Dans toute cette section on prendra \(
  ...
  clair qu'on pourrait utiliser n'importe quel indice entre \( 0 \) (puis \( 1
  \) à partir du lemme~\ref{l:par-anyvar-mono}) et \( \anydim \) (et \( \vdim*
  \) en multiprojectif).  Les morphismes et formes obtenus dépendront bien sûr
  de ce choix, mais pas leurs propriétés.}
\begin{equation}
  \cdn\Big(
...
\( f_1 = F'' / \vmp[][\vdim]^\Dii \). Le but de cette section est alors de
montrer la proposition suivante.

\begin{prop}
  On a \( \inda**(f_1) \ge \epsi \delta / \sigma \), où \( \sigma \) est tel
  que \dots
\end{prop}
...
théorème~\ref{t:vojta-div} et qu'on peut tirer parti du
scolie~\ref{s:part-cases}.  On choisit, pour tout \( \fct \), un indice \(
  \indv* \in \set{0, \dots, \vdim*} \) de sorte que \( \av{ \cexa*[\indv*] } \)
soit maximal sous cette condition. Le lemme suivant montre que cette valeur
absolue représente à peu de chose près la norme de \( \cexa* \).
...
\begin{lem} \label{l:coord-norm}
  ...
  \begin{equation}
    ...
    \nv\infty{ \cexa }
    \cdot \nv1{ \varfc* }^{-1}
    \cdot ( 2 \vdegp* \dimp** )^{- \vdeg* (\vdim* + 1) \dv}
    \pmm.
  \end{equation}
\end{lem}

\begin{proof}
  Il suffit de montrer qu'on a
  \begin{equation}
    \av{ \cexa** }
    \le
    \av{ \cexa*[\indv] }
    ...
    \cdot ( 2 \vdegp* )^{\vdeg* (\vdim* + 1) \dv}
  \end{equation}
  pour tout \( \ind \in \set{\vdim* + 1, \dots, \dimp} \). On utilise le fait
  qu'on connaît des relations \( \poldep** \) liant chacune des dernières
  coordonnées aux premières et de norme contrôlée par~\eqref{e:nv-poldep}.  On
  décomposant \( \poldep** \) suivant les puissances de \( \vmp** \) :
  \begin{equation}
    (\vmp**) ^{ \vdeg* }
    =
    \sum_{ \alpha=1 }^{ \vdeg* }
    ...
    \le \nv1{ \poldep** }
  \end{equation}
  On spécialise alors en \( \cexa \) et on prend les valeurs absolues :
  \begin{equation}
    \av{ \cexa** }
    \le
    \sum_{ \alpha=1 }^{ \vdeg* }
    \poldep*[\ind, \alpha]
    \left(
      \frac{ \av{\cexa*[\indv]} }{ \av{\cexa**} }
    \right) ^{ \vdeg* - \alpha }
  \end{equation}
  Si le quotient apparaissant dans le membre de droite est inférieur à \( 1
  \), notre assertion initiale est vérifiée compte tenu
  de~\eqref{e:nv-poldep}.  Sinon, elle l'est aussi, car
  \( \varfc* \) est normalisée (note~\ref{fn:varfc} page~\pageref{fn:varfc})
  de façon à avoir \( \nv1{ \varfc* } \ge 1 \) en toute place.
\end{proof}

On introduit ensuite le point \( \exi \in \va^{\puiss*} \) tel que \(
  \wemb(\ex) = (\ex, \exi) \) et on note \( \cexi \) un système de coordonnées
multihomogènes de ce point dans le plongement \( \Theta^{\puiss*} \).  On
choisit alors pour tout \( \fcti \) un indice \( \indiv* \in \set{0, \dots,
    \dimp} \) maximisant \( \av{ \cexi*[\indiv*] } \), ainsi que \( \indig*
  \in \set{0, \dots, \dimp} \) tel que \( \cexi*[\indig*] \neq 0 \). Il est
alors clair que \( \av{ \cexi*[\indiv*] } \ge \nv1{ \cexi* } \dimp**^{-\dv}
\).
...
\begin{equation}
  \wembcl[][\indig](\vmp) ^{ \delta }
  =
  \prod\fctirange \wembcl*[\indig](\vmp[\fcti], \vmp[\puiss]) ^{ \delta }
  \qquad\text{et}\quad
  \vmp[][\vdim] ^{\epsz \wts \delta}
  =
  \prod\fctrange ( \vmp*[\vdim*] )^{\epsz \wts* \delta}
  \pmm.
\end{equation}
(Par la suite, on utilisera ce genre de notations compactes sans toujours
préciser leur sens, qu'on espère suffisamment évident.) La définition de \(
  f_1 \) au début de cette section, celle de \( F'' \) au
scolie~\ref{s:aux-co}, le choix des \( l_j \) et le scolie~\ref{s:part-cases}
montrent que \( f_1 / f_2 \) est défini et inversible sur un voisinage de \(
  \cex \) dans \( \var \).  Le lemme~\ref{l:indice-inversible} s'applique
donc, montrant que \( f_1 \) et \( f_2 \) ont le même indice en \( \cex \).
Il suffit dont d'estimer l'indice de \( f_2 \) ; nous allons commencer par
décomposer cette fonctions en des facteurs sur lesquels nous aurons un bon
contrôle.

On commence par choisir pour chaque place \( \place \) une famille de formes
\(
  \wembcl[\place, \fcti]
  = L\pexp{\wti*, \wti**}
  [{\vadapt[\fcti](\ex[\fcti]), \vadapt**(\ex**), \place}]
\)
donnée par l'application du lemme~\ref{l:hclab} au plongement \( \Theta \)
comme à la section~\ref{sec:wemb}, mais en choissisant cette fois une famille
par place. Indépendamment de \( \place \), les estimations de la section citée
restent valables et bien sûr \( \wembcl[\place, \fcti] (\cexa[\fcti],
  \cexa[\puiss]) \) est proportionnel à \( \cexi* \). On écrit alors, pour
tout \( \place \) :
\begin{align}
  f_2
  & =
  ...
  \left(
    ...
    \frac{
      ...
      \wembcl[\place, \fcti][\indiv*](\vmp[\fcti], \vmp[\puiss])
    }
  \right)^\delta
  \left(
...
précédente sont tous définis et inversibles en \( \cex \) grâce aux
différents choix d'indices et au scolie~\ref{s:part-cases}, une nouvelle
application du lemme~\ref{l:indice-inversible} montre que \( \der[\dermp]
  f_2(\ex) \) est égal, pour tout \( \place \), à
\begin{equation}
  \der[\dermp]
    \frac{
      R(\vmp) \cdot F( \vmp, \wembcl[\place](\vmp) )
  ...
  \left(
    \prod\fctirange
    \frac{
      ( \cexa[\fcti ][{\indv[\fcti ]}] )^{ 2\wts[\fcti ] }
      ( \cexa[\puiss][{\indv[\puiss]}] )^{ 2\wts[\puiss] }
    }{
      \wembcl[\place, \fcti][\indiv*](\cexa[\fcti], \cexa[\puiss])
    }
  \right)^\delta
  \left(
    \frac{ \wembcl[][\indiv](\cexa) }{ \wembcl[][\indig](\cexa) }
  \right) ^{ \delta }
  \left(
    \frac{ \cexa[][\indv] }{ \cexa[][\vdim] }
  \right) ^{ \epsz \wts \delta + r }
\end{equation}
Pour démontrer la proposition, procédons par l'absurde et supposons que
\( \wtsum( \dermp ) < \epsi \delta / \sigma \).  En estimant pour chaque \(
  \place \) la valeur absolue \( \place \)-adiques de chacun des facteurs de
l'expression ci-dessus, en distinguant selon que
\( \place \in \placesapx \) ou pas pour le premier, nous allons montrer que
l'on contredit la formule du produit sous cette hypothèse.

Rappelons qu'on note \( \degv \) le degré local de \( \cdn \) en \( \place \)
et traitons les facteurs un par un de droite à gauche. Pour le premier, on a
\begin{equation}
  \prod_\place
  \prod\fctrange
  \left(
  ...
  \le
  \hautm[\infty]{ \ex[1] }^{2\epsz \delta \wts[1]} \cdot \expb^{o(\delta)}
\end{equation}
en remarquant d'abord que le dénominateur disparaît en prenant le produit sur
toutes les places et que le numérateur est évidemment majoré par \(
  \nv\infty{\cexa[][\indv]} \), puis en appliquant le lemme~\ref{l:epsiii-def}
et en majorant généreusement \( \epsiii \) par \( 1 \) dans son résultat.

Pour le second, on commence avec les mêmes arguments pour écrire
\begin{equation}
  \prod_\place
  \prod\fctirange
  \left(
...
\end{equation}
puis on majore le logarithme de cette quantité comme suit
\begin{align}
  \delta \sum\fctirange \hautl[\infty]{ \exi* }
  & \le
  \delta \sum\fctirange \Bigl(
    \hautl[\infty]{ \exi* }
    + (4^\genre - 1) \vaht + 3\genre \ln2
  \Bigr)
  && \text{par le fait~\ref{f:comp-h-hn}}
  \\ & \le
  \delta (\puiss*) \bigl(
    3 \Vcos \wts[1] \hautn{\ex[1]}
    + 4^\genre \, \vaht + 3\genre
  \bigr)
  && \text{par~\eqref{e:hautn-wt-diff}}
\end{align}
Pour le facteur suivant, les définitions de \( \wembcl[\place, \fcti] \) et de
\( \indiv* \) ainsi que des majorations évidentes donnent
\begin{align}
  \frac{
    \av{ \cexa[\fcti ][{\indv[\fcti ]}] }^{ 2\wts[\fcti ] }
    \av{ \cexa[\puiss][{\indv[\puiss]}] }^{ 2\wts[\puiss] }
  }{
    \av{ \wembcl[\place, \fcti][\indiv*](\cexa[\fcti], \cexa[\puiss]) }
  }
  & \le
  \frac{
    \nv1{ \cexa[\fcti ] }^{ 2\wts[\fcti ] }
    \nv1{ \cexa[\puiss] }^{ 2\wts[\puiss] }
  }{
    \nv1{
      L\pexp{\wti*, \wti**}
      [{\vadapt[\fcti](\ex[\fcti]), \vadapt**(\ex**), \place}]
      (\cexa[\fcti], \cexa[\puiss])
    }
  }
  (\dimp*)^\dv
\end{align}
En prenant le produit sur \( \fcti \) et \( \place \) et en élevant à la
puissance \( \delta \), puis en applicant le résultat du lemme~\ref{l:hclab}
on majore la contribution de ce facteur par
\begin{align}
  \prod\fctirange \Bigl(
    \hclab^{\wtis* + \wts**}
    \dimp**
    \prod_\place
    \nv1{ \wembcl[\place, \fcti] }^\degv
  \Bigr)^\delta
  & \le
  \Bigl(
    \hclab^{2\wts[1]}
    \dimp**^{\puiss*}
    \prod_\place \prod\fctirange
    \nv1{ \wembcl[\place, \fcti] }^{-\degv}
  \Bigr)^\delta
\end{align}
Avant de passer au dernier facteur, remarquons de suite que
\begin{equation}
  \nv1{ R(\vmp) \cdot F( \vmp, \wembcl[\place](\vmp) ) }
  \le
  \nv1{ F }
  \prod\fctirange \nv1{ \wembcl[\place, \fcti] }^\delta
  \cdot \expb^{o(\delta)}
\end{equation}
et que les facteurs \( \prod\fctirange \nv1{ \wembcl[\place, \fcti] }^\delta
\) qui apparaîtront se simplifieront avec ceux de l'estimation précédente.

\worknote{Voir que \( \indv \neq 0 \).}
Le lemme~\ref{l:par-var} appliqué\footnote{En utilisant les \( \vmp*[\indv]
  ...
  note~\ref{fn:coord-deshom} page~\pageref{fn:coord-deshom} et le choix de \(
    \indv \).} à \( R(\vmp) \cdot F( \vmp, \wembcl[\place](\vmp) ) \) nous
...
\begin{equation}
  ...
  \frac{ P^\dermp_\place(\cexa) }{
    \cexa[][\indv]^\Diii \pden(\cexa)^{\vlg\dermp} }
  =
  \frac{ P^\dermp_\place(\cexa) }{
...
En fait, aux places de \( \placesapx \), on peut obtenir une estimation plus
...
où \( \gmp\nu \) est tel que \( \wtsum*(\gmp\nu) \ge \delta \epsi (1 -
  \frac1\sigma) \) d'après la fin du lemme~\ref{l:par-var} et l'hypothèse sur
...
\begin{align}
  ...
  -\wtapx\expapx \sum\fctrange
  \gmp\nu*[0] \hautn{\ex*}
  + \sum\fctrange \deg_\fcti P^\dermp_\place \left(
    \frac\dv2 \ln(\dimp*) + \ln \nv\infty{\vadapt^{-1}}
    + \wtapx\expapx (4^\genre \vaht + 3\genre)
  \right)
\end{align}
et on majore enfin le premier terme de cette somme par
\begin{equation}
  - \delta \wts[1] \hautn{\ex[1]}
  \wtapx\expapx \epsi (1 - \frac1\sigma) (1 - \sqrt{\Vcos})
  \pmm.
\end{equation}

En regroupant tout les termes qui constituent
\( \frac{ P^\dermp_\place(\cexa) }{ \cexa[][\indv]^{\deg P^\dermp_\place } } \)
puis en sommant sur \( \place \), on majore la hauteur logarithmique de ce
terme par la quantité suivante, où l'on note \( \wtapx = 0 \) si \( \place
  \notin \placesapx \) :
