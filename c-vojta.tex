...
\begin{hypo} \label{h:vaemb}
  \begin{enumthm}
    ...
    \item Il existe une famille de réels \( \hmclab* \) presque tous égaux à
      ...
      \( (L\pexp{a, b, \clmap})_{\clmap \in \clmaps} \) de \( (\dimp+1)
      \)-uplets de formes telles que :
      \begin{enumerate}
        \item la famille \( (L\pexp{a, b, \clmap}[\ind])\indrange \) représente
          le morphisme \( (x, y) \mapsto ax - by \) sur \( \clmap \) ;
        \item chaque forme \( L\pexp{a, b, \clmap}[\ind] \) est bihomogène de
          bidegré \( (2a^2, 2b^2) \) ;
        \item pour tout place \( \place \), toute carte \( \clmap \) et tout
          indice \( \ind \), on a
          \( \nv1{ L\pexp{a, b, \clmap}[\ind] } \le \hmclab* \) ;
...
\end{equation}
où \( L\pexp{\wti*, \wti**, \clmap_\fcti} \) est donnée par les
hypothèses~\ref{h:vaemb}, de sorte que chacune des applications
...
\begin{scho} \label{s:aux-co}
  ...
  \begin{enumthm}
    ...
    \item \(
      ...
      \) ; \label{i:aux-co-ht}
  \end{enumthm}
  De plus, si \( (\clmap_\place)_\place \) est une famille de carte indexée
  par les places de \( \cdn \), on a
  \begin{equation}
    \frac{
      \nv1{ \faux[\clmap_\place] }
    }{
      \prod\fctirange \nnv1{ \wembcl[\clmap_\place, \fcti] }^\delta
    }
    \le
    \cdots
  \end{equation}
...
\( \exi \) dans le plongement \( \vaemb^{\puiss-1} \). (Ci-dessous on notera
...
est défini par~\eqref{e:def-wembclm} et \( \clmap_{\wts*, \wts**, \ex*, \ex**,
    \place} \) donné par les hypothèses~\ref{h:vaemb}. De même, on note \(
...
compte tenu de la définition de \( \wembclv* \) ci-dessus. Il ne reste à alors
