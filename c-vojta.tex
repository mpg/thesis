...
\begin{proof}
  ...
  Compte tenu du fait que \( \sum_{\fct=1}^{\mu} 2 \lgr{\gmp\nu*} - 1 = 2
    \lgr\derp - \lgr\ip - \mu \le 2 \lgr\derp - 2 \) sur l'ensemble de
  sommation, on peut définir les polynômes \( P^\derp \)  par la relation de
  récurrence
  \begin{equation}
    - P^\derp
    =
  ...
  \begin{align}
    ...
    - R \, \diff_{\ind_0} S \, \diff_\vafi Q_{\derp'}
    \\ & \qquad
    + (2\lgr{\derp'} - 1) Q_{\derp'}
    (\diff_{\ind_0} S \, \diff_\vafi R - R \diff_{\ind_0} R)
    \pmm.
  \end{align}
