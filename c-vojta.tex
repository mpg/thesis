...
d'un plongement projectif \( \vaemb \colon \va \embedin \projd \) fixé, auquel
on associe les constantes suivantes.

\begin{nota} \label{n:vaemb} \todo[Déplacer dans l'intro et/ou l'annexe]
  \begin{enumthm}
    \item On note \( \htcmp \) une constante réelle telle que
      ...
      \end{equation}
    \item \label{i:clab}
      On note \( \hmclab* \) une famille de réels presque tous égaux à
      \( 1 \), \( \nclmaps \) un entier, \( \clmaps \) un ensemble de \(
      ...
      \begin{enumerate}
        ...
        \item pour tout couple de points \( (x, y) \in \va^2 \) et tout place
          ...
          (Le membre de droite est par ailleurs majoré par \( \hmclab*^{a^2 +
              b^2} \) d'après l'inégalité précédente les propriétés de la
          norme utilisée.)
      \end{enumerate}
      On note enfin \( \hmclab = \prod_\place \hmclab*^\degv \) et
      \( \hlclab = \ln \hmclab \).
  \end{enumthm}
\end{nota}

\begin{rem} \label{r:vaemb}
  L'annexe~\ref{chap:plong-mm} montre que de telles constantes existent pour
  tout plongement et donne des valeurs admissibles pour tout ou partie de ces
  constantes dans un certain nombre de cas, par exemple celui où \( \va \) est
  une courbe elliptique plongée à la \bsc{Weierstrass} ou bien le cas plus
  général où l'on utilisera un plongement de \bsc{Mumford} modifié. Elle
  établit par ailleurs des relations entre \( \htcmp \) et \( \hlclab \) ; on
  formulera néanmoins les énoncés suivants avec une dépendance explicite en
  chacune de ces constantes pour des raisons de clarté et d'optimalité pour
  les cas où l'une serait mieux contrôler que l'autre.

  Dans tous les cas considérés, on aura \( \dimp \ge 2 \) et \( N \ge 3 \), et
  le degré de (l'image de) \( \va \) dans le plongement sera au moins \( 3 \).
\end{rem}

On fixe désormais et jusqu'au début de la section~\ref{sec:vojta-coro} un
plongement projectif de \( \va \) et des constantes associées à ce plongement
comme ci-dessus. Le but des prochaines sections est alors de prouver le
théorème suivant, où les notions de distance et de hauteurs découlent du
plongement fixé.
...
\begin{thm} \label{t:vojta-div}
  ...
  plongement fixé et \( 0 < \eps < 1 \) un nombre réel.  Il n'existe dans \(
    \va(\Qbar) \) aucune famille de points \( \ex[1], \dots, \ex[\puiss] \)
  avec \( \puiss \ge \genre + 1 \) satisfaisant simultanément aux conditions
  suivantes :
  \begin{align}
    0 < \distv{\ex*}{\divi}
...
On dispose (notations~\ref{n:vaemb}) d'un atlas \( \clmaps \) de \( \va^2 \)
...
notations~\ref{n:vaemb}, de sorte que chaque application
...
\begin{proof}
  ...
  utilisant les notations~\ref{n:vaemb}. En prenant le
...
\begin{proof}
  Pour le cas où \( F \) est un monôme, on reprend point par point le preuve
...
\begin{proof}
  ...
  hauteur de \( \faux* \) sauf qu'on utilise le corollaire~\ref{c:ht-wemba} à
  la place du lemme~\ref{l:hmat-wemba}, ce qui donne une majoration identique
  à la précédente :
  \begin{align}
    \sum_\place \degv \ln \nv1{ \faux[\clmapv] }
    \le
...
\( \clmapv* \) la carte donnée par les notations~\ref{n:vaemb} avec \( (a, b,
...
lemme~\ref{l:par-var} avec \( G = \fauxv \) et \( \ind = \indv \), et on
...
remarque~\ref{r:vaemb}, puis \( \genre \ge 1 \) et \( \puiss \ge \genre + 1
...
\begin{lem} \label{l:plin}
  ...
  par les notations~\ref{n:vaemb} et la dimension ambiante sont inchangées.
\end{lem}
...
\begin{proof}
  ...
  des notations~\ref{n:vaemb}, on pose
  \begin{equation}
...
\begin{lem} \label{l:norm-mn}
  ...
  divise un premier inférieur ou égal ) \( \Delta \) et \( 0 \) sinon. On a
  alors
  \begin{equation}
...
\begin{lem} \label{l:hs-vero}
  ...
  \end{enumthm}
  Par ailleurs le cardinal \( \nclmaps \) de l'ensemble de cartes introduit
  par les notations~\ref{n:vaemb} est inchangé.
\end{lem}
...
\begin{proof}
  ...
  point~\ref{i:hs-vero-htcmp} est une conséquence immédiate de ce dernier
  ...
  Pour~\ref{i:hs-vero-dv}, pour chaque place, introduisons un point
  ...
  même type d'argument que \( \nv{}{ \chi(x) \wedge \chi(y) } \le \nv{}{ x
      \wedge y }^\Delta \) de sorte que
  \(
    \distv[\place, \vaemb'] x{y_\place}
    \le
    \distv[\place, \vaemb] x{y_\place}^{\Delta}
  \) ce qui implique largement l'estimation annoncée, la distance étant plus
  petite que \( 1 \).
  ...
  notations~\ref{n:vaemb}. Pour tout \( \beta \in \N^{\dimp+1} \) de longueur
...
\begin{proof}
  ...
  corollaire~\ref{c:vojta-div-any} dans ce cadre avec \( \expapx' =
  ...
  corollaire~\ref{c:vojta-div-any} comme prévu et conclure.
\end{proof}
...
\begin{proof}
  ...
  \begin{equation}
    \mathcal P
    =
    \prod\fctrange P_1(\ex*) T_1 + \dots + P_s(\ex*) T_s
  \end{equation}
...
On énonce maintenant une version du précédant corollaire avec des constantes
...
\begin{proof}
  ...
  rappellée pour référence, et l'apparition de \( \cst{v-gen} \) ne change
  ...
  et \( \deg \va \ge 3 \) (c'est évidant pour \( \puiss \) assez grand et on
