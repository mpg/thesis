...
\begin{lem} \label{l:param-aff}
  ...
  Soit \( S \in \ker \tilde\pi \) ; on note \( R = \diff_\vafi S \) et on
  suppose que \( R \notin \ker \pi \).
  ...
  \begin{enumthm}
    \item \( \der[\derp] \omega
        ...
        \right)
      \) ;
    \item \( \deg P_\dv^\derp \le (\deg S - 1) (2\lgr\derp - 1) \) ;
    \item \( \nv1{ P_\dv^\derp }
        \le
        \nv1 S ^{2\lgr\derp - 1}
        \cdot \left(
          (4\anydim)^{\lgr\derp -1} (\deg S)^{3\lgr\derp -2}
        \right)^\dv \).
  \end{enumthm}
\end{lem}
...
\begin{proof}
  ...
  récurrence sur la longueur de \( \derp \), en partant à chaque fois de \(
    P_\dv^\derp = - \diff_{\ind_0} S \) quand \( \derp_{\ind_0} = 1 \) et \(
  ...
  \begin{align}
    \deg P^\derp
    & \le
    \deg S - \lgr\ip - \mu + (\deg S - 1) (2\lgr\derp - 2)
    \\ & \le
    1 - \lgr\ip - \mu + (\deg S - 1) (2\lgr\derp - 1)
    \\ & \le
    (\deg S - 1) (2\lgr\derp - 1)
    \pmm,
  \end{align}
  ...
  avec \( Q_\derp = P^\derp \cdot \derp! \) et où, rappelons-le, \( \derp' \)
  est tel que
  \( \derp[\ind_0] = \derp[\ind_0]' + 1 \) et \( \derp* = \derp*' \) sinon :
  \begin{align}
    Q_\derp
    & =
    R^2 \, \diff_{\ind_0} Q_{\derp'}
    - R \, \diff_{\ind_0} P \, \diff_\vafi Q_{\derp'}
    \\ & \qquad
    + (2\lgr{\derp'} - 1) Q_{\derp'}
    (\diff_{\ind_0} P \, \diff_\vafi R - R \diff_{\ind_0} R)
    \pmm.
  \end{align}
  On en déduit immédiatement l'estimation de degré suivante :
  \begin{equation}
    \deg P^\derp
    \le 2 (\deg S - 1) + \deg P^{\derp'}
    \le (\deg S - 1) (2\lgr\derp - 1)
    \pmm.
  \end{equation}
  Pour la norme, on prouve que \(
    \nv1{Q_\derp}
    \le
    \nv1 S^{2\lgr\derp-1} 4^{\lgr\derp-1} (\deg S)^{3\lgr\derp-2}
    (\lgr\derp - 1) !
  \)
  (ce qui implique le résultat annoncé vu que \( \binom{\lgr\derp}{\derp}
    \le \anydim^{\lgr\derp-1} \)) en exploitant la majoration de degré
  sous la forme \( \deg P^\derp \le 2\lgr\derp \deg S \).
  \begin{align}
    \nv1{ Q_\derp }
    & \le
    2(\deg S)^2 \cdot \deg Q_{\derp'} \cdot \nv1 S^2 \nv1{ Q_{\derp'} }
    + 2 (2\lgr{\derp'} - 1) (\deg S)^3 \nv1 S^2
    \\ & \le
    \nv1 S^2 \nv1{ Q_{\derp'} } \cdot 4 (\deg S)^{3\lgr{\derp'}}
    \\ & \le
    \nv1 S^{2\lgr\derp-1} \cdot 4^{\lgr\derp-1} (\deg S)^{3\lgr\derp-2}
    (\lgr\derp - 1) !
    \qedhere
  \end{align}
\end{proof}
...
dernier, ainsi que \( S = S_\ind \) car ce choix convient étant donné que \(
  \poldep[][\anydim+1] \) est le polynôme minimal de \( \vp[\anydim+1] \).
On note \( \tilde P_{\ind, \dv}^\derp \) les polynômes obtenus et \( \tilde
  Q_\ind = \diff_\vafi S_\ind \). On a alors, dans \( \cdn(\anyvar) \) :
\begin{align}
...
\( \deg S = 2\anydeg \). On pose enfin \( P_{\ind, \dv}^\derp = \bar P_{\ind,
