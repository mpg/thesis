...
dans ce système de coordonnées. Nous préciserons ensuite les notions d'indice
utilisées, puis présenterons un morphisme associé aux poids \( \wt* \) qui
permettra par la suite d'exploiter~\eqref{e:hautn-wt-diff}.
...

\subsection{Indice le long d'un hyperplan}
\label{sec:inda}

Nous définissons ici une notion d'indice d'annulation le long de l'hyperplan
\( \divi \). Une autre notion d'indice, en un point cette fois-ci, sera
définie de façon similaire au début de la
sous-section~\ref{sec:vojta-extrap-core}.
...
précédemment s'annule avec un indice élevé en \( \ex \). Commençons par
préciser la notion d'indice utilisée, similaire à celle définie à la
sous-section~\ref{sec:inda}, mais cette fois en un point. Posons
\begin{equation}
  \wtsum( \dermp )
  =
