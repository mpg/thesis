...
\) positif, qu'on utilise en conjonction avec~\eqref{e:varset-deg} pour
obtenir
\begin{equation}
  \ln B
  \le
  ...
  \pmm.
\end{equation}
On se contentera parfois de l'estimation un peu grossière suivante :
\begin{equation}
  \ln B
  \le
  B
  ...
  \pmm.
\end{equation}
Enfin, on utilisera à l'occasion la remarque élémentaire suivante : \( \alpha
  + \beta \le \alpha\beta \) dès que \( \alpha \ge 2 \) et \( \beta \ge 2 \).

Avant de passer aux différentes estimations \lat{ad hoc}, commençons par une
estimation qui nous sera utile plusieurs fois :
\begin{align}
  B (\genre + 1) \ln (B (\dimp+1))
  & \le
  B (\genre + 1) \bigl(
...
précision possible sur le facteur constant, on se contentera d'obtenir celui
qui est utile dans chaque situation.
...
\begin{equation}
  2 \vdim (\dimp + 1) \ln \Lambda^{f(\vdim)}
  \le
  2 \cst{vs-ht} \puiss^2 \genre \Lambda^{\frac1\puiss f(\vdim)}
  \le
  \frac12 \Lambda^{(1 + \frac1\puiss) f(\vdim)}
\end{equation}
...
\begin{align}
  ...
  \\ & \qquad \le
  \frac2{255} \cst{vs-ht} \Lambda^{(1 + \frac1\puiss) f(\vdim)}
  + 4 \puiss \Lambda^{\frac1\puiss \, f(\vdim)}
...
\begin{align}
  ...
  + \cst{vs-ht} \puiss \Lambda^{\frac1\puiss\,f(\vdim)}
  + \frac1{255} \cst{vs-ht} \Lambda^{(1 + \frac1\puiss) f(\vdim)}
  \\ & \qquad \le \label{e:ratfh1a-adhoc}
  \frac16 \cst{vs-ht} \Lambda^{(1 + \frac1\puiss) f(\vdim)}
  \pmm.
\end{align}
En particulier, l'inégalité précédente implique
