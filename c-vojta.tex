...
restent valables et bien sûr \( \wembcl[\place, \fcti] (\cexa[\fcti],
  \cexa[\puiss]) \) est proportionnel à \( \cexi* \). On écrit alors
\begin{align}
  f_2
  & =
  \frac{ R(\vmp) }{ \vmp[][\vdim]^r }
...
\( \wtsum( \dermp ) < \epsi \delta / \sigma \).  En estimer les valeurs
absolues \( \place \)-adiques de chacun des facteurs, en distinguant selon que
...
\) se simplifieront avec ceux de l'estimation précédente. Appliquons
maintenant le lemme~\ref{l:par-var} à cette forme :
% XXX
