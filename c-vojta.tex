...
\begin{prop} \label{p:varset-notmin}
  ...
  \begin{align}
    ...
    \Biggl(
      \sum\fctrange \wtw* \wts* \hautl[1]{\var*}
      + 17 \wts[1] \cst{vs-ht} \Lambda^{2f(\vdim)} \ln \Lambda^{f(\vdim)}
    \Biggr)
    \pmm.
  \end{align}
\end{prop}
...
\begin{proof}
  ...
  définitions~\eqref{e:d-dp-def} de \( d \) et \( d' \).  Pour le quatrième
...
\end{equation}
et au final :
\worknote{Facteur \( 2 B \dimp \) oublié ! Puis à reporter\dots}
...
fonction de \( \sum\fctrange \wtw* \wts* \hautl[1]{\var*} \). Pour \( \delta
\) assez grand, on peut, lorsqu'on remplace dans l'estimation ci-dessus,
supprimer les termes \( o(\delta) \) dans le membre de droite en perdant par
exemple le facteur \( 2 \) dans le membre de gauche ; on obtient ainsi
\begin{align}
  \wts* \hautl[\infty] T
  & \le
  \vdim* \Lambda^{2\vdim f(\vdim)}
...
Passons maintenant à \( T' \). Par construction et vu les propriétés
classiques de la hauteur, on a
\begin{align}
  \hautl[\infty]{ T' }
  & \le
  \hautl[\infty]{ T }
  + \deg T \cdot \hautl[\infty]{ \vadapt* }
  + \ln \binom{ \deg T + \dimp + 1 }{ \dimp + 1 }
  \\ & \le
  \hautl[\infty]{ T }
  + \Lambda^{2\vdim f(\vdim)} \ln \Lambda^{f(\vdim)}
  ...
  \Biggr)
\end{align}
en observant \todo que \( 2\vdim (\dimp + 1) \le \Lambda^2 \). Ceci achève la
preuve de la proposition~\ref{p:varset-notmin}.
