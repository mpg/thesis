...
\) fixé (\lat{cf.} section~\ref{sec:vaemb}) l'hyperplan \( \vp[0] = 0 \) ;
nous verrons plus tard (section~\ref{sec:vojta-coro}) que ce cas implique le
cas général.
...
jusqu'à la section~\ref{sec:vojta-coro} non incluse.
...
On rappelle pour commencer que d'après la proposition~\ref{p:dv-hp}, on a
...
particulièrement commode à manipuler dans ce cas (section~\ref{sec:siegel}).
...
\begin{proof}
  ...
  \begin{alignat*}{2}
    ...
    && \text{d'après~\eqref{e:wt-diff-normn-gen} et~\eqref{e:wt-ratio-gen}}
  \end{alignat*}
...
\begin{proof}
  ...
  du plongement implique que \( \poldep*[\vdim*+1] \) irréductible et de degré
  \( \vdeg* \) en \( \vp*[\vdim*+1] \) donc \( \pden* \) étant de degré plus
  petit, ne peut pas s'annuler identiquement sur \( \var* \).
\end{proof}
...
\begin{proof}
  ...
  pour tout \( \gamma \in \N \) et tout \( \alpha \in \set{1, \dots,
      \Delta_\fct} \) un polynôme \( U_{\gamma, \alpha, \Delta_\fct} \) tel que
  si l'on pose, pour tout \( \fct \in \set{1, \dots, \puiss} \) et tout \(
  ...
  \begin{equation}
    ...
    \begin{dcases*}
      ...
      \\
      \sum _{\alpha=1}^{\Delta_\fct}
      U_{\gamma, \alpha, \Delta_\fct}
      ( P\mexp*[\ind, 1], \dots, P\mexp*[\ind, \Delta\mexp*] )
      (\vmp**)^{\Delta_\fct - \alpha}
      & sinon,
    \end{dcases*}
  \end{equation}
  alors \( R_{\fct, \ind, \gamma} = ( \vmp*[\ind] ) ^\gamma \mod \Ideal \) et
...
On dispose (section~\ref{sec:vaemb}) d'un atlas \( \clmaps \) de \( \va^2 \)
...
section~\ref{sec:vaemb}, de sorte que chaque application
...
\begin{lem} \label{l:hmat-wemba}
  ...
  \begin{align}
    ...
    \pmm,
  \end{align}
  où l'on rappelle que \( \hmclab \) est donné par la section~\ref{sec:vaemb}.
\end{lem}
...
\begin{proof}
  ...
  utilisant la section~\ref{sec:vaemb}. En prenant le
...
\( \clmapv* \) la carte donnée par la section~\ref{sec:vaemb} avec \( (a, b,
...
\begin{lem} \label{l:plin}
  ...
  à la section\ref{sec:vaemb} et la dimension ambiante sont inchangées.
\end{lem}
...
\begin{proof}
  ...
  de la section~\ref{sec:vaemb}, on pose
  \begin{equation}
...
\begin{lem} \label{l:hs-vero}
  ...
  \end{enumthm}
  Par ailleurs le cardinal \( \nclmaps \) de l'ensemble de cartes introduit
  à la section~\ref{sec:vaemb} est inchangé.
\end{lem}
...
\begin{proof}
  ...
  section~\ref{sec:vaemb}. Pour tout \( \beta \in \N^{\dimp+1} \) de longueur
