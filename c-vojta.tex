...
\begin{proof}
  ...
  \( G \). Quitte à multiplier \( L \) par un scalaire, ce qui ne change pas
  sa hauteur, on suppose de plus que \( L(b_0) = 1 \).

  On note de plus \( M \) la matrice dont les colonnes sont les
  vecteurs \( b_i \) (écrits dans la base canonique de \( \Qbar^{\dimp+1} \)).
  Ainsi, si \( x \) a pour coordonnées \( C \) dans la base canonique, ces
  ...
  \( L'(\vp) = L(M(\vp)) = L(b_0) \vp[0] = \vp[0] \) ; par construction, \( L'
  \) est une équation de \( \divi \) dans ce nouveau plongement.

  Soit maintenant \( x \) un point de \( \va \), dont on note encore \( x \in
    \Qbar^{\dimp+1} \) les coordonnées dans le plongement \( \Theta \) et \(
    x' = M^{-1} x \) le coordonnées dans le plongement \( \Theta' \). On a
  alors
  \[
    \distv[\place, \Theta]{x}{\divi}
    =
