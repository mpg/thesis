...
des formes \( R(\vmp) \cdot F( \vmp, \wembclv[](\vmp) ) \).  Comme l'indique la
...
les polynômes fournis par le lemme dépendent de ce choix, qui n'est pas gênant
car la forme à la quelle on applique le lemme dépend déjà de \( \place \).
Par ailleurs, chaque application du lemme founit
...
\begin{equation}
  ...
  =
  \frac{ P_\place(\vmp) }{
    \prod\fctrange (\vmp*[\indv*]) ^{ \Dir* } \, \pden*(\vmp*)^{\lgr{\dermp*}}
  }
  \pmm.
\end{equation}
...
% pour définir \ratfv{1a} et 1b
%  \frac{ P_\place(\vmp) }{
%    \vmp[][\indv]^\Dir \pden(\vmp)^{\vlg\dermp} }
%  =
%  \frac{ P_\place(\vmp) }{
%    \vmp[][\indv]^{\deg P_\place } }
%  \cdot
%  \frac{
%    \vmp[][\indv]^{\vlg\dermp \cdot \deg \pden}
%  }{
%    \pden(\vmp)^{\vlg\dermp}
%  }
