...
\begin{lem} \label{l:hmat-wemba}
  ...
  \begin{equation}
    ...
    \le
    \left(
      \hclab^{\wts[1] (1 + \epsiii)} \,
      \Bigr( \prod\fctirange (\vdegp[\fcti])^{2\dimp\wtis*} \Bigl) \,
      (\vdegp**)^{2\dimp(\puiss*)\wts**} \,
      (\dimp**!)^\puiss
    \right) ^\delta
    \pmm.
  \end{equation}
...
\begin{proof}
  ...
  en remarquant que, par définition de \( d \), on a \( \lgr{q\mexpi*} =
    \delta \) pour tout \( \fcti \) (voir \eqref{e:d-dp-def}). Le résultat
  annoncé suit en prenant le produit sur toutes les places, en invoquant le
  lemme~\ref{l:epsiii-def} et en utilisant~\eqref{e:vadapt-inv-ht}.
\end{proof}
