...
de $\epsz$ (voir fin de la section~\1{sec:comp-dim}). Considérons par
...
\begin{proof} \later
  ...
  fait~\1{f:addsub}, qui n'était pas disponible au moment de la
...
\subsection{Deux calculs de dimension} \1{sec:comp-dim}
...
\begin{fact} \label{f:dep-int}
  ...
  \begin{enumthm}
    ...
    \item \( \nv1{ P\mexp*[\ind] }
      \le \nv1{ \varfc* } \cdot 2^{ (\vardim*+1) \vardeg* \dv } \).
  \end{enumthm}
\end{fact}
...
\begin{lem}
  ...
  \end{equation}
  qui est l'identité modulo \( \Ideal \) (voir~\1{e:C-spaces}
  et~\1{e:C-i-iii} pour la définition de l'espace d'arrivée). De plus les
...
\begin{coro} \label{c:hmat-rdiv}
  ...
  faisant commuter le diagramme~\1{e:diag-f-aux}, dont les matrices dans
...
\begin{coro} \label{c:hmat-relim}
  ...
  diagramme~\1{e:diag-f-aux} commute. De plus, les colonnes de la matrice
...
\begin{proof}
  ...
  (scholie~\1{s:plong-adapt}).

  En effet, le deuxième point de la définition~\1{d:plong-adapt} dit que
...
\begin{proof}
  On reprend le diagramme~\1{e:diag-f-aux}, dont la première ligne est
