...
\[
  \sum_{\fct=1}^\puiss (\wt*^2 + 1) < \wt[1]^2 (1 + \epsiii)
  \pmm.
\]
...
\[
  \stairs = \left\{
    ...
    \text{ tels que }
    \frac {\lambda\pexp1[0]} {\wt[1]^2}
    + \dots +
    \frac {\lambda\pexp{\puiss-1}[0]} {\wt[\puiss-1]^2}
...
\begin{lem}
  ...
  \[
    ...
    \le
    \bigr(
    \alpha_1 + \frac94 \beta_1 \wt[1]^2,
    \dots,
    \alpha_{\puiss-1} + \frac94 \beta_{\puiss-1} \wt[\puiss-1]^2,
...
\begin{proof}
  ...
  globalement par des formes de degré $\wt*^2$ en tirant en arrière par
  ...
  degré $\wt*^2$ qui coupent donc $\var*$ en $\vardeg*
  \wt*^{2\vardim*}$ points.
...
Nous allons maintenant calculer la codimension de $\vanish$
dans $\cdn[\cmmh]\ph{ \delta d' + r }^{ C''' }$. On introduit à cet effet le
sous-ensemble de $\stairs$ suivant :
